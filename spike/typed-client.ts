...
function param<K extends string, C extends Record<K, string>, Res, M extends TMeta>(
  name: K,
  child: TNode<C, Res, M>,
): TNode<Omit<C, K>, Res, TParamMeta<K, M>> {
  return {
...
function path<T extends Record<string, TNodeShape>>(
  table: T,
): TNode<Record<string, never>, unknown, TPathMeta<T>> {
  return {
...
function methods<T extends Record<string, TNodeShape>>(table: T): TNode<any, any, TMethodsMeta<T>> {
  return {
...
let nextId = 1;
const store: Todo[] = [{ id: nextId++, title: "fractal todo example", done: false }];

// Leaves

const listAllLeaf = leaf<Record<string, never>, Todo[]>(async (_req) => [...store]);
...
const createTodoNode = body<CreateInput, Record<string, never>, Todo>(parseCreate, async (req) => {
...
const idSubtree = param("id", methods({ GET: getByIdLeaf }));
