...
type FlatChoice<Alt, Pre extends string> = {
  ...
  Alt extends PrefixMeta<infer Pre2, infer R>
    ? Walk<R, `${Pre}${PreKey<Pre2>}`>
    : Record<never, never>
>;

// the structural key an EndMeta alt contributes.
type AltKey<A, Pre extends string> =
  A extends EndMeta<infer Segs, string, unknown, unknown> ? NormKey<`${Pre}${KeyOf<Segs>}`> : never;
...
function build(
  ...
  if (m.tag === "choice") {
    for (const alt of (meta as ChoiceMeta<readonly unknown[]>).alts)
      build(alt, pre, surface, dispatch);
  }
}
