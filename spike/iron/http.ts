...
export function route(
  ...
  Ctx<unknown>,
  Reply<unknown> | null,
  EndMeta<readonly unknown[], string, unknown, unknown>
> {
  ...
  const fn = (hasBody ? maybeFn : schemaOrFn) as (
    ctx: unknown,
  ) => Reply<unknown> | Promise<Reply<unknown>>;
  return handler({ tag: "end", segs, method, hasBody }, async (ctx: Ctx<unknown>) => {
...
export function mount<const Pre extends readonly string[], U, MI>(
  ...
  return handler({ tag: "prefix", pre: prefix, rest: sub.meta }, async (ctx: Ctx<unknown>) => {
