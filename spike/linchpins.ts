...
type Handler<Vars extends Record<string, unknown>> = (ctx: HttpCtx<Vars>) => Promise<Response>;

type Middleware<Vars extends Record<string, unknown>, Extra extends Record<string, unknown>> = (
  ctx: HttpCtx<Vars>,
  next: (ctx: HttpCtx<Vars & Extra>) => Promise<Response>,
) => Promise<Response>;
...
function toHandler(router: Router<NoVars>): (req: Request) => Promise<Response> {
...
const profile = await hit("GET", `${BASE}/admin/profile/u-99`, {
...
console.log(
  `withValidation(createUser) — valid body:   [${validRes.status}] ${await validRes.text()}`,
);
...
console.log(
  `withValidation(createUser) — missing email:[${invalidRes.status}] ${await invalidRes.text()}`,
);
...
console.log(
  `withValidation(paginateItems) — coerce "5":[${coerceRes.status}] ${await coerceRes.text()}`,
);

// 2d. meta.validator exposed
console.log(
  `meta.validator exposed:     ${typeof createUserNode.meta.validator["~standard"].validate === "function" ? "yes" : "no"}`,
);
