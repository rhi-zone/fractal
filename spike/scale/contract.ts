...
type CallArgs<P extends string, I> = (HasParams<P> extends true
  ? { params: PathParams<P> }
  : Record<never, never>) &
  (HasInput<I> extends true ? { body: I } : Record<never, never>);

type CallSig<P extends string, I, O> = keyof CallArgs<P, I> extends never
  ? () => Promise<BodyOf<O>>
...
export type ClientOfDescs<T extends readonly RouteDesc<string, string, unknown, unknown>[]> = {
  readonly [P in Patterns<T>]: {
    readonly [
      D in Extract<T[number], { pattern: P }> as Lowercase<D["method"]>
