...
async function main() {
  ...
  log.push(
    `openapi /users/{id}.get params: ${JSON.stringify(doc.paths["/users/{id}"]?.get?.parameters)}`,
  );
  log.push(
    `openapi /users.post requestBody present: ${doc.paths["/users"]?.post?.requestBody !== undefined}`,
  );
