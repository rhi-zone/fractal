...
type Middleware<
  ...
  Res,
> = (next: CoreHandler<Ctx & AddedCtx, Res>) => CoreHandler<Ctx, Res>;
...
type Req<TypedVars extends Record<string, unknown> = Record<string, never>> = CoreReq<
  HttpCtx<TypedVars>
>;
...
interface StandardSchema<In, Out = In> {
  readonly "~standard": {
    validate(
      ...
      | { issues: Array<{ message: string }>; value?: undefined };
  };
  readonly _in?: In;
  readonly _out?: Out;
}
...
function route<TypedVars extends Record<string, unknown>, S extends RouteSchema>(
  r: Route<TypedVars, S>,
): Route<TypedVars, S> {
  return r;
}
...
class Router<TypedVars extends Record<string, unknown> = Record<string, never>> {
  ...
  async _dispatch(ctx: HttpCtx<Record<string, unknown>>): Promise<Response | null> {
    ...
    return this._runWithMiddlewares(ctx, this._middlewares, (ctx2) => this._dispatchInner(ctx2));
  }
  ...
  private async _dispatchInner(ctx: HttpCtx<Record<string, unknown>>): Promise<Response | null> {
    ...
    for (const mount of this._mounts) {
      ...
      if (seg === prefix) {
        ...
        const result = await this._runWithMiddlewares(subCtx, mount.middlewares, (ctx2) =>
          mount.router._dispatch(ctx2),
        );
    ...
    for (const r of this._routes) {
      ...
      if (r.schema?.body !== undefined) {
        ...
        if (result.issues !== undefined) {
          return new Response(JSON.stringify({ error: "Bad Request", details: result.issues }), {
            status: 400,
...
function crud<T extends Record<string, unknown>>(
  ...
  r.add(
    route({
      ...
      schema: { body: opts.schema },
      handler: async (req) => {
        const item = await opts.store.patch(
          req._ctx.params["id"]!,
          req.validatedBody as Partial<T>,
        );
        return item !== null ? json(item) : notFound();
      },
    }),
...
report(
  "  GET /admin/users/",
  await hit("GET", `${BASE}/admin/users/`, { headers: { "x-user": "alice@example.com" } }),
);

section("2. CRUD: create user");
report(
  "  POST /admin/users/",
  await hit("POST", `${BASE}/admin/users/`, {
...
report(
  "  POST /admin/orgs/",
  await hit("POST", `${BASE}/admin/orgs/`, {
    headers: { "x-user": "alice@example.com" },
...
report(
  ...
  await hit("PATCH", `${BASE}/config/branding`, {
...
report(
  "  POST /integrations/stripe/test",
  await hit("POST", `${BASE}/integrations/stripe/test`, {
    body: { apiKey: "sk_test_abc123" },
  }),
);
report(
  "  POST /integrations/slack/test",
  await hit("POST", `${BASE}/integrations/slack/test`, {
    body: { webhookUrl: "https://hooks.slack.com/abc" },
  }),
);
report(
  "  POST /integrations/unknown/test",
  await hit("POST", `${BASE}/integrations/unknown/test`, {
    body: {},
  }),
);

section("9. SSE endpoint — read event-stream chunks");
const sseRes = await handle(
  new Request(`${BASE}/admin/events`, {
    headers: { "x-user": "alice@example.com" },
  }),
);
...
report(
  "  GET /admin/me",
  await hit("GET", `${BASE}/admin/me`, { headers: { "x-user": "alice@example.com" } }),
);
...
console.log(
  `  GET /blob: [${blobRes.status}] ${blobRes.headers.get("content-type")} len=${blobBuf.byteLength} bytes`,
);
