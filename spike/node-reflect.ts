...
function choice<P extends Record<string, unknown>, Res>(...ns: FNode<P, Res>[]): FNode<P, Res> {
  return {
...
function typed<
  ...
  Res = unknown,
>(parse: (raw: Record<string, unknown>) => Out, inner: FNode<P & Out, Res>): FNode<P, Res> {
  return {
...
const getTodoLeaf = leaf<{ id: string }, Todo>(async (req) => ({
  id: req.params.id,
  title: "Example",
}));
...
const createTodoNode: FNode<{}, Todo> = typed<{ title: string }, {}, Todo>(
  ...
  leaf<{ title: string }, Todo>(async (req) => ({ id: "new", title: req.params.title })),
);
...
const makeReq = (method: string, pathStr: string): Req<Record<string, never>> => ({
  method,
