...
for (const form of FORMS) {
  for (const res of RES) {
    ...
    const row: Row = {
      ...
      res,
      routes: ROUTES[res]!,
      inst: best!.instantiations,
      checkMs: best!.checkMs,
      tsgoErrors: best!.errors,
      tscOk: !tsc.crash && tscP.errors === 0,
      tscCrash: tsc.crash,
      tscErrors: tscP.errors,
    };
    rows.push(row);
    console.log(
...
function tbl(pick: (r: Row) => string | number, label: string) {
  ...
  for (const res of RES) {
    const cells = FORMS.map((f) => {
      const r = rows.find((x) => x.form === f && x.res === res);
      return r ? String(pick(r)) : "—";
    });
