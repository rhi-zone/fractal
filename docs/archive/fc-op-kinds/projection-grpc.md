...
| Field types & `optional`/`repeated` | proto types                                  | inferred-from-type (`Option`→optional, `Vec`→repeated)                                        | **[MIXED]** — nullability/multiplicity are agnostic axes; the proto _scalar mapping_ is gRPC-specific.                                                                                                                                 |
...
| Status codes                        | enum result on the wire                      | agnostic-property (op's error taxonomy / safety) partially                                    | **[MIXED]** — the _mapping_ of errors→codes recurs (HTTP status, exit codes), but the specific gRPC status enum values are gRPC-form.                                                                                                  |
...
| Metadata (headers)                  | k→v envelope                                 | caller/runtime                                                                                | **[GRPC-SPECIFIC]** at the wire level; conceptually every protocol has an envelope, but the concept is too generic to be a shared _op_ distinction.                                                                                    |
...
_operation's semantics_: whether calling it changes state, and whether calling it twice is
...
side-effects × idempotent decomposition), and the gRPC projection should _read_ it to emit
...
- Error taxonomy (maps to status codes here, HTTP codes / exit codes elsewhere) — the _taxonomy_ is agnostic even if the _values_ are not.
...
- **Field numbers** = sequential 1-based by parameter order; **no way to pin them**. A
  comment (~lines 148–155) explicitly warns that presence-validation does _not_ catch
...
validation anchor) as authoring. It captures the _structural_ gRPC concepts (service,
