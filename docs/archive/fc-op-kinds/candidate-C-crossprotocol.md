...
Anything only one protocol cares about is that protocol's _projection detail_, not a kind.
...
- **Verb selection** (GET/PUT/DELETE/PATCH/POST) — _HTTP-only surface detail_.
- **Body vs. no body** — _HTTP-only surface detail_.
...
- **Flag naming / subcommand shape** — _CLI-only surface detail_.
...
- **Query vs. action** → the model plans differently: a query is "gather information, cheap,
  ...
  deliberately". This is _the_ MCP distinction (readOnlyHint / destructiveHint annotations).
...
- **Tool name / JSON schema** — _MCP-only surface detail_.
...
- **Method naming / streaming shape** — _gRPC-only surface detail_.
...
reads-vs-mutates (it's what HTTP _does_ with "reads"), so it is not an independent kind.
...
one kind defined by _invocation semantics_ rather than the effect axes).
...
| **amend**   | Mutates a _part_ of existing state; NOT idempotent in general (depends on current value), non-destructive.                        | increment a counter; append to a log; apply a diff |
| **call**    | _(anchor)_ Invokes a procedure/method for its effect and/or return; not assumed safe, idempotent, or reversible.                  | send email; run a job; charge a card               |
...
- `call` = the anchor. It is deliberately the _catch-all_ invocation kind: anything whose
...
The mapping _looks_ 1:1, which is exactly the trap. Discriminating checks:
...
- **Does a kind exist that HTTP can't cleanly express?** `amend` maps to PATCH, but PATCH is
  HTTP's _weakest_ verb (semantics undefined by spec). Our `amend` has a sharper agnostic
...
- **Do two kinds ever share a verb / does one kind span verbs?** `call` deliberately
  absorbs many things HTTP would spread across POST _and_ sometimes PUT (non-idempotent
...
- **Weakness admitted:** the _count_ (5) and the visual near-alignment with 5 HTTP verbs is
...
1. **The 5≈5 coincidence.** Five kinds landing on GET/PUT/DELETE/PATCH/POST is _exactly_
   ...
   gRPC's non-idempotent retry level — and gRPC's non-idempotent is the _default_, not a
...
2. **`set` vs `call` boundary is a promise, not a type.** Idempotency of `set` is almost
   ...
   actually be the _true_ cross-protocol invariant (reads-vs-mutates is the only ✅×4 row;
   idempotent and destructive are ✅×4 and ✅×2 as _hints_ but degrade gracefully to
   defaults). A red team could argue the principled minimum is **observe + call**, with
   set/erase/amend demoted to optional authored _refinements/annotations_ on those two —
