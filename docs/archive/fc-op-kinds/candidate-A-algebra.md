...
The tree IS the addressing/grouping hierarchy. So the _address_ is supplied by a
...
address collapses several naive distinctions into the _arguments_, which is also
...
| invertibility (iso / mono / epi)                                    | **REAL but RELATIONAL** | a property of a _pair_ of arrows, not of one arrow in isolation                                    |
...
So the surviving _kind-bearing_ axes are: **(domain trivial?)** × **(codomain
...
- **direction/authority** (for Endo & Lens): input is _supplied-whole_ vs
  _computed-from-old_. The type CANNOT tell these apart, so this is AUTHORED.
...
- (a) **Authored / relational**: requires a declared inverse partner. Can be
  _inferred_ only if two co-located ops have mutually inverse types and lens-law
...
| Fold      | —                | GET _or_ DELETE (authored)             | one kind spans two verbs → not a verb       |
...
| —         | PATCH            | **nothing** maps here                  | PATCH re-expressed as _partiality modifier_ |
...
- **Produce, Lens-forward, and pure Folds ALL project to GET** (3 kinds → 1 verb):
  the categorical layer is _finer-grained_ than HTTP, so it wasn't back-derived
...
- **No kind maps to PATCH.** PATCH is re-expressed as a _modifier_ (partiality) on
...
(view-vs-element, replace-vs-update-vs-invoke) migrate into the _arguments_ and
into _authoring_. That is the same locus where the HTTP verb distinction lives, so
...
if the load-bearing distinctions are all authored, the _algebra_ isn't doing the
