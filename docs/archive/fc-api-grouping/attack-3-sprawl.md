...
_Red-team pass against `design.md` (Carrier-Grouped Function Core v1). Target: the
...
reifies whenever "no single one owns the property it maintains" — which is the _normal_ case
...
it is a _second population the same size as the first_, made entirely of nouns that do not
...
relation count _above_ the entity count.
...
**two** subjects' fields and no single subject is a superset → the rule _mandates_
reification.
...
_conservative_ (I docked Comment.authorId, Task.milestoneId, File.folderId to their owning
...
The design's whole justification for reifying (§2c/§2d, §3.2) is that _a real conserved
property lives on the pair_. But 11 of these carriers copy-paste the _same_ placeholder
...
A dev wants "assign a user to a project." They reason from what they _have_ in hand: a
...
instantly. **The carrier grouping makes the reified half of the API _less_ greppable and
_less_ navigable than the flat list it replaced**, because the entry point (an invented noun)
...
This is the crux of the "moves the sprawl" charge: grouping reduces _bucket count_ (37 < 100)
...
Every one of the 17 carriers needs a noun _invented and agreed_. The same user-belongs-to-a-
...
chosen by _my_ taste in one sitting. Across a team and a year you get `ProjectAssignment` in
...
anti-pattern, and the reified subset is _less_ discoverable than the flat list it replaced.

But it is **not FATAL**, for one honest reason: total bucket count still _drops_ (37 vs 100),
and the 3 genuinely-lawful carriers (Settlement, TaskDependency, OrgSeat) _are_ real
...
1. **Reification threshold — a carrier must earn its noun.** Amend §3.1 step 4: reify a
   relation carrier **only if** the §3.2 breaking predicate reads ≥2 subjects' fields _and is
   non-trivial_ (i.e. not reducible to "the edge row exists"). A carrier whose only invariant
...
2. **Dual-home discovery index (reuse the capability "index, never a home" pattern).** When
   an edge _is_ reified, index it under **both** endpoint nouns: `User.edges` and
   `Project.edges` both surface `ProjectAssignment`. The edge still has one home (no second
   source of truth), but is _discoverable_ from either subject the dev is holding. This is the
...
3. **Deterministic edge naming.** Name a reified edge from its ordered subject pair + primary
   ...
   no "no-suggesting-names" violation — the name is _derived_, and it is greppable from either
