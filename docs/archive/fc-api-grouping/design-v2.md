...
_Reviser pass over `design.md` (v1) against attacks 1–5 (same directory). This is the
...
fix preserves it, the claim is narrowed or retracted honestly rather than fudged._

_Provenance: everything here is design decision. The per-attack disposition is in §7._
...
whose _non-trivial conserved predicate_ the op is responsible for — or the **effect/service
...
- v1 said _"filed under exactly one **carrier**"_ → v2 says _"exactly one **home**; carriers
  are one kind of home."_ (The totality-over-carriers claim is **retracted**; see §2.)
- v1 said _"the carrier + ops is the **protocol-agnostic truth**; HTTP is computed FROM a
  carrier"_ → v2 says _"the core is the single **domain** truth; a surface is a projection
  that **adds** surface-only facts."_ (The generate-the-surface-unaided claim is
...
_load-by-key_ (`GET /users/:id` mis-projected to POST) and _remove_ (DELETE had no role).
...
v1 §0 / §3.1 claimed _"the procedure is total and deterministic — every op lands in exactly
one carrier."_ Attack 2 (Break A) refuted it by counterexample: zero-subject ops
...
the effect/service tier.** The decision procedure (§3) is _total over homes_, and
_deterministic_ in which home. It is total over **carriers** only for the invariant-bearing
...
over-claimed it as total over _all_ ops.
...
optional and _no step ever read it_ (attack 2 Break B). Unified statement:
...
`invariant` is no longer optional _for a carrier claim_: to file an op under a carrier via
...
The v1 phrasing _"the carrier is the type it is responsible for keeping true"_ — read as _one
op single-handedly owns all invariants it touches_ — is **false for this class, and no fix
preserves it.** So it is narrowed, honestly:

> **Filing (home) is single. Invariant _enforcement_ is per-type and automatic.** An op has
...
invariant" by **separating the two**: an op has one _home_ (filing/discovery/projection), and
every carrier _enforces its own point-invariant at its boundary_ (correctness), independent of
...
credentials") reads an **input subject plus the ambient credential store** — it does _not_
...
- **(V1a) The domain is a single source of truth, never re-declared per surface.** — _Defensible._
- **(V1b) A surface is generated from the core unaided; zero per-op surface config; role alone
  yields the wire.** — _Refuted by counting (attack 4). Overreach._

v2 **preserves V1a and retracts V1b.** The framing downgrades exactly as attack 4 proposed —
to _"typed core + a thin per-surface projection layer that adds surface-only facts"_ — and I
judge this **preserves the original vision's load-bearing half and concedes only its
overreach.** The load-bearing intent was always _"do not maintain N copies of the domain."_
...
rule. Adding them in a projection is _decoration_, not a second declaration of the domain.
...
surface needs. This is stronger honesty for a _weaker_ word, and it is the version that ships.
...
>    change.
> 2. **Additive only.** The manifest carries _only_ surface-only facts the domain lacks:
...
>    surface. This is the precise redefinition of _"no second source of truth"_: **no
...
>    thereafter the manifest is the source of _surface identity_. A re-filed op keeps its
...
1. **Multi-output op with no relational law — where is the home?** When an op returns two
   ...
   "file at the strongest predicate" gives no unique answer. _Recommendation:_ permit one
...
2. **`a.active` edge collapse target — higher-cardinality subject vs a shared `Edges` host?**
   §3 docks flag-edges back onto a real subject, but which. _Recommendation:_ default to the
...
3. **Streaming/subscription — service tier now, or a `subscribe` role?** _Recommendation:_
...
4. **Derived edge names vs human URL aliases.** Canonical `opId` is derived
   (`User×Project:assign`); URLs may want a friendlier noun. _Recommendation:_ keep the
...
5. **Is idempotency a domain fact or a surface fact?** §5.1 files it as a manifest flag
   (surface), but `f(f(x))===f(x)` is arguably a property of the function. _Recommendation:_
   treat it as a one-bit op-level annotation that lives _with the op_ but is _consumed by_
...
6. **Do `load`/`remove` belong in the carrier, or in a separate persistence layer?** They are
   persistence-facing (they concern stored identity, not value transformation). _Recommendation:_
