...
_Red-team of `design.md` (same dir). Thesis under test: carrier-derivation couples internal
code organization to the public projected surface, so you cannot refactor one without
breaking the other. Default assumption: broken; below is the proof._
...
3. **The procedure is designed to RE-FILE ops when the signature/invariant changes.** §3.1
   ...
   whole point of the rule is that discovering a new cross-subject law _moves the home_.
...
Internal filing and public URLs have _opposite_ stability requirements — the design
_celebrates_ re-filing as understanding improves (reify when you find a law), while public
...
Product asks: archiving must be auditable — record _who_ archived, immutably, and the
...
- **Old route `PATCH /documents/:id (archive)` VANISHES. New route `POST /archivals`
  ...
  **complete client rewrite**, triggered by an _internal invariant refinement_, with **zero
...
- Internal call sites: every `Document.archive(doc)` → `Archival.archive(doc, byUser)`
  (module move + new arg). These _are_ caught by the compiler — ordinary, acceptable churn.
...
Note the trap: the _safe_ version of this change — storing `archivedBy` as a field on
...
- **Role gains a cardinality law** ("a project has exactly one lead"): the breaking predicate
  for `promote(assignment,"lead")` reads the _whole set_ of Assignments for a project — a
...
- **Every existing client of `/documents/*` breaks.** The split is a purely _internal
  organizational_ decision (two invariants shouldn't share a type) — and the design's own
  hygiene rule thereby _mandates a breaking public API change_. Following the design's
...
does not move ops off their carrier … the impls _are_ the carrier's own ops."

- **Internal callers:** IF `implement` keeps `Document.archive` resolvable as a direct method,
  `Document.archive(doc)` still works → low internal churn. This part is fine — _provided_
...
is a _pure, un-indirected function_ of internal filing decisions. The decision procedure's
central feature — re-filing ops as invariants are discovered — is _by construction_ a public
...
- **Following the design's own advice triggers it.** §3.1 step 4 and the "split on two
  invariants" hygiene rule _mandate_ the re-filings that break the surface.
...
Not fatal, because the design _already_ isolated the projector as a separate downstream
...
projector being a _pure function_ with no pinning. Remove the purity at the surface boundary
...
things**: (a) a second source of truth for the _op's definition/behavior_ — genuinely bad,
keep forbidding it; and (b) a record of the _public contract's identity_ — which is an
...
2. **A generated, append-only surface manifest ("route lock").** On first publish, the
   projector _freezes_ `opId → { method, path }` for every currently-projected op. It is
...
3. **Projector reads the lock first, defaults second.** A re-filed op keeps its locked
   `{method, path}` because the lock keys on `opId`, which the move preserved. Its _internal_
   home changed (compiler-caught call-site churn — fine); its _public URL_ did not.
...
values — single op definition, no per-op HTTP annotation _at authoring time_, projector stays
...
the _silent public_ break; the lock converts it to a reviewable one. Ship the lock before any
