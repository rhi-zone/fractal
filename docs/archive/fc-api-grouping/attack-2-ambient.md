...
close it. This attack shows the wound is deeper than #2 admits: it is not only _junk
reification_, it is also _non-totality_ and an _internal 3.1-vs-3.2 conflict_. The headline
...
`isEmail` is the cleanest kill: it is the design's _own_ example of an invariant predicate
...
For ambient ops that _do_ materialize state, the procedure files them **confidently into junk
carriers, with zero signal that they are junk**, because the procedure never inspects
invariant _quality_ and `CarrierDef.invariant` is **optional**.

Real systems always give ambient state a type (a Redis rate-limit record `{count,
windowStart, limit}`, an audit row, a cache entry). Once typed, it _has fields_, so:
...
- **rate-limit**: breaking predicate "requests-in-window ≤ limit" reads exactly **one**
  ...
  (read-set _cardinality_) is **orthogonal** to the genuine-vs-junk distinction.
...
- **cache invalidation** `invalidate(key) => void`: breaking predicate "cache == source"
  ...
  So even the ≥2-subject branch — the design's supposed signature of a _real_ relation
...
The design's worked examples all _happen_ to carry clean invariants, which disguises that
the _procedure_ never requires or checks one. The stop condition exists only in the author's
...
They are not intro (step 1: not yielding a _new_ valid entity, just fetching an existing one)
...
over-reification of the most common read in any system, and it is _forced by the rule_, not a
...
- **§3.2** says the carrier is "the type whose invariant the breaking predicate reads." The
  ...
  credentials" — which reads **`User.passwordHash`**, a _different_ subject than the produced
  `Session`. So §3.2 points at User (or a two-subject Auth relation), while §3.1 points at
  Session.

The design asserts (§3.2 tie-break) that §3.2 _resolves_ the step-1-vs-step-4 judgment call.
Here the two mechanisms give **different answers** for a mainstream op. The design only ever
tests the case where the producer's breaking predicate reads its _own_ produced fields
...
- **full-text `search(q) => Result[]`, dashboard, activity feed, cross-N report**: no `t:T`
  source (step 3 needs a first-arg subject; these range over _all_ carriers), no single
...
- **batch/cron `nightlyBilling() => void`**: sequences many carriers' ops; correctness is
  _temporal/sequential_, not a static conserved predicate. If `=> void`, falls through
...
- The carrier core is genuinely sound for **invariant-bearing domain ops** (endomaps, real
  ...
  claims totality over _all_ ops while the abstraction only fits the invariant-bearing
  subset.
...
1. **Add an explicit non-carrier tier** ("service"/"effect" ops) and **retract the totality
   ...
   pure utilities). This tier is _not_ projected by the carrier→HTTP verb-kind rule; it needs
...
2. **Make the stop condition explicit and make `invariant` load-bearing for reification.**
   ...
   read of ambient/global state. Insert as step 0: _"if no such predicate exists → it is a
   service op, not a carrier."_ This is the discriminator the design lacked:
