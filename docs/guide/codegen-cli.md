...
`@rhi-zone/fractal-type-ir` includes a CLI (`packages/type-ir/src/cli.ts`,
bin name `fractal-type-ir` (alias `fractal-codegen`)) that generates a standalone, AOT (ahead-of-time)
...
```sh
bun packages/type-ir/src/cli.ts <command> [options]
```

Or, once the package is installed/linked, via its `bin` entry:

```sh
npx fractal-type-ir <command> [options]
```
...
```sh
bun packages/type-ir/src/cli.ts build src/api.ts -o src/generated/validators.ts
```

Force a rebuild even when the output looks up to date:

```sh
bun packages/type-ir/src/cli.ts build src/api.ts -o src/generated/validators.ts --force
```
...
```sh
bun packages/type-ir/src/cli.ts watch src/api.ts -o src/generated/validators.ts
```
...
```sh
bun packages/type-ir/src/cli.ts stub -o src/generated/validators.ts
```
...
```sh
bun packages/type-ir/src/cli.ts check src/api.ts -o src/generated/validators.ts
```
