...
deployment (busiless)'s own augmentation.
...
required" (busiless's `packages/fractal-support/src/meta.ts` does exactly
...
```ts
// e.g. busiless's packages/fractal-support/src/meta.ts
...
busiless's `scopes`) errors locally at each scope-less `op()` call via the
...
Under this spec, busiless's file becomes the deployment's single
augmentation (§3): `scopes` moves to `LeafMeta`, loses its `?` (becomes
required), and merges in whichever projector fragments busiless actually
...
is busiless's own call, not decided by this spec.
...
- Implementation order / rollout sequencing across the five projector
  packages.
- The exact shape of busiless's "explicitly no scope required" marker
