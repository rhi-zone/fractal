...
Busiless's own `packages/fractal-support/src/rawRequest.ts` is the
...
one-root-fractal-tree-2026-08-02.md` §3.3/§7 (busiless) already names the
...
(`build.ts`) and `wrapScopes` (busiless's `scopes.ts`) are both exactly this
...
`docs/decisions/one-root-fractal-tree-2026-08-02.md` §3.2 (busiless) found
...
`callerContextStorage`, busiless's `callerContext.ts`) is currently GLOBAL
