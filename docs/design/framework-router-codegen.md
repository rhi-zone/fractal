...
_inside_ an existing Express/Hono/Elysia app (rather than as a standalone
...
different target: instead of a fetch-based _client_, it emits a
framework-native _router_.
...
second, closer precedent for the _family_ shape this idea implies: one IR,
...
needed. Keeps `http-api-projector`'s own scope — being fractal's _own_
...
_not_ yet knowable: the call was to size and justify the new package
...
"turns an `api()`/`op()` tree into an HTTP router" that _runs_ (via the
...
not any curated subset of the frameworks that _do_ have real usage. Priority
...
as a starting point for sequencing implementation work, _not_ a locked
...
representation _is_ `SchemaMap`/`ToolSchema` (JSON-Schema-shaped) and framed
...
- `TypeRef` → Zod and `TypeRef` → TypeBox source-text generation already
  ...
  validator _builder source text_ (e.g. `z.object({ ... })`,
...
catch in the _current_ shipped codegen that would make eject-by-abandonment
...
_embedded in the generated file itself_, if one existed, could make
...
Eject-as-the-model doesn't rule out _also_ shipping a `regenerate` (or
...
- **Embed the hash as a comment in the generated file**, computed over the
  file's content _excluding_ that comment line (generate the body first,
...
- **Refuse and require an explicit override.** Print what changed (a diff
  ...
  where the user _does_ want the refresh.
...
a _standing_, per-file "never touch this again" declaration, orthogonal to
...
- **A default write path plus a separate read-only path**, matching a split
  ...
  flag-to-override are two settings of the _same_ write attempt
