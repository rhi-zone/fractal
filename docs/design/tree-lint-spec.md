...
Grounded directly in the consumer repo, `busiless`
...
Each `serveX(composed)` in busiless independently produces a `Fetch`
(`packages/fractal-support/src/fetch.ts`'s `createBusilessFetch`), and
...
- Not a projector (`http-api-projector`, `mcp-api-projector`, …) — every
  ...
  "pure function over the composed service" framing, which busiless's own
...
(read in busiless, not fractal, but the shape recurs identically across every
...
busiless's own CLAUDE.md "select adapters ... by direct import + direct
...
NOTHING in busiless collects its ~18 `buildXTree(composed)` functions into
...
- **Standalone CI command:** the same array, built by a small script that
  ...
  script in busiless, callable in CI without booting the real app.
...
default rule set, and leaves the call site (busiless's composition root
...
- `LintRule` (§4a) is a plain function type, not a closed union or a
  ...
  rule from a runtime string a caller holds (busiless's own CLAUDE.md draws
...
- A consumer-authored custom rule (e.g. busiless wanting a
  busiless-specific check — "every admin-only leaf declares
...
- **`wrapScopes` coverage / required-scope enforcement** — whether every
  leaf declares `meta.scopes` at all (busiless's own
...
own status block notes for its own consumer story). Busiless is where §4b's
...
- CI: a new script (naming and exact location busiless's own call, matching
...
- **Whether/how a stub `ComposedSurface` (§4b's CI path) is generated
  mechanically versus hand-maintained per slice** — busiless's own
  implementation-time call, not decided by this spec.
- **Exact severity/exit-code/output-format conventions for the CI command**
  (§7) — left to whatever convention busiless's other lint scripts
  (`lint:custom-display-count` and siblings, per busiless's own CLAUDE.md)
...
4. **A closed rule enum or a string-keyed rule registry.** §5 — rules are
   ...
   runtime-dynamic dispatch, per busiless's own composition-time-selection
