...
validator module covering every leaf under it, or, in busiless's batch
...
- **Local hash comparison at this scale costs nanoseconds; Merkle's payoff
  ...
  single entry's leaf count is O(10¹–10²) per the busiless reference —
...
already-certified consumer decision): busiless's `codegen-fractal-validators.ts`
