...
/`not`) is structural set algebra over _schemas as predicates_, not over _types as
sets of constructors_ — a schema is fundamentally "a predicate over instances," and
...
2. **Type form** — `{"type": "..."}` with the value being one of: `boolean`,
   `string`, `timestamp` (RFC 3339 string, the _only_ built-in semantic type),
...
"additionalProperties": bool}` — struct with required vs. optional fields as
   _separate keyword buckets_ rather than a `required` name-list (a distinct design
...
validation-level refinement is out of scope; it's a _shape_ language, not a
_constraint_ language. Extension point: `metadata` (a free-form, ignored-by-
...
**Generics:** none.
**Extensibility:** the `metadata` keyword on every form is the _only_ sanctioned
...
- **Unit struct** — a named zero-field struct, e.g. `struct Marker;` — distinct from
  `Unit` because the _name_ is semantically meaningful to some formats (self-
...
- **Newtype struct** — single-field tuple struct, e.g. `struct Meters(f64)` — a
  ...
  serialize it transparently (this is essentially a built-in _branded/nominal
  wrapper_ concept, notable because most of the surveyed systems don't have one).
...
_nominal_ information (the type or variant's name) through serialization — most
...
data model, so the data model itself is not parametric; genericity is a _source_-
...
_named_ tuple elements as of TS 4.0 purely for DX/labels, no runtime meaning),
...
"UserId" }`), i.e. nominal typing is _encoded_, not primitive. Worth noting because
...
survey construct: a _type-level string pattern_, effectively a compile-time regex-
...
as resolved:** critical scoping note for the IR — these are _source-level type-level
computation_, and the survey only needs their **resolved/reified output**, since
...
/`+readonly`/`-readonly`). The _inputs_ to these computations (generics, `infer`,
...
(closed, versioned by TS releases); _values_ within it (interfaces, type aliases)
...
constructor may carry zero or more _positional_ (tuple-shaped) arguments; this is
...
newtype) — OCaml has _both_ an erased alias and a reified wrapper, which is a useful
...
**GADTs (Generalized ADTs):** constructors whose return type can be _instantiated_
...
pin `'a` differently. Notable because it's a form of _type-indexed_ sum where each
...
**Nullability:** no null; `'a option = None | Some of 'a` is the _only_ absence
...
runtime representation (another idea worth carrying: a _phantom_/erased-at-runtime
...
checking) — _except_ OCaml has **extensible variant types**
...
functions; a Haskell record's fields are, notably, _top-level functions_ sharing the
...
single-field wrapper that is _guaranteed by the compiler to have zero runtime
representation cost_ (unlike a single-constructor `data`, which still boxes). This
...
checker fiction with no runtime tag at all, so also zero-cost, but _unenforced_ —
...
`Functor`, ...) is an _open, ad-hoc extensible_ set of "does this type support
...
_cases_ to a sum after the fact," type classes let you "add new _behavior_ to an
...
distinguishing for the IR: extending the _set of type constructors_ vs. extending
the _set of things a projection can compute about_ an existing type constructor —
...
recognizable idea: _factor recursion itself out as a reusable combinator_ rather
...
example of _encoding concerns leaking into the type vocabulary itself_.
...
set at a time, and setting one clears any previously-set sibling. This _is_
...
(`Int32Value`, `StringValue`, ...) which exist _purely_ to recover explicit
...
precision+scale as _type parameters_, not constraints — `MONEY` in Postgres) vs.
...
qualifiable to `YEAR`/`MONTH`/`DAY`/... field-range subsets — i.e. the _type itself_
...
having multiple _physical representations_ with different tradeoffs," which the IR
...
**Array:** Postgres arrays are a first-class, arbitrarily-nestable _type modifier_
...
_named_ product types usable as a column type, i.e. arbitrary struct nesting inside
...
**Range types:** Postgres `INT4RANGE`/`TSRANGE`/etc. — a _type constructor_
...
a constraint that reaches _outside_ the type/table itself, unlike every other
...
recursion lives in the _data_, not the _type_ — SQL has no native recursive
...
functions supplied by the implementation — the schema only declares the _name_, all
...
_field-level parameterization_ orthogonal to the type's own generic-ness, which
GraphQL otherwise lacks entirely).

**Interfaces:** a named set of fields that implementing object types must include —
structural-contract nominal typing (an object type must _declare_ which interfaces
...
**Unions:** `union SearchResult = Photo | Person` — a union of _object types only_
...
arguments/variables, notably _cannot_ contain interfaces/unions and cannot have
...
(`[T!]!`, `[T]!`, `[T!]`, `[T]`) — nullability is a _wrapper type constructor_
...
here applied to _type_ definitions instead of operations.
...
**Nullability:** as above — nullable is the _default_, `!` opts into required,
...
notably, no mechanism exists to add wholly new _composite_-type _kinds_ (you cannot
invent a ninth top-level type kind beyond scalar/object/interface/union/enum/input/
list/non-null) — a clean example of a system that's extensible in its _leaves_
(scalars) and _annotations_ (directives) but closed in its _structural vocabulary_
...
- **Groups**: fields nested under a named group behave like a struct for the
  _value_ constructor (`person.name.first`-style access) but do **not** introduce a
  ...
  grouping for _human/API organization_ decoupled from grouping for _physical
  layout_.
- **Unions**: must be declared _embedded within_ a containing struct (no free-
...
- **Generics**: real parametric types, but restricted to pointer-typed parameters
  ...
  Proto's layout model needs a parameter's _size_ to be uniform (a pointer) to
...
_shapes_, which is itself a useful signal: some of what "extensibility" needs to
mean for the IR is about _schema versioning discipline_, not just "can I add a new
kind of node."
...
**Logical types (verified via spec):** an _annotation layered on top of_ a
...
fields: months, days, milliseconds — notably _not_ reducible to a single scalar
...
architectural bets (Zod: method-chaining, primary object is the _validator_, TS type
is _derived_ via `z.infer`; Valibot: standalone tree-shakeable functions composed by
pipe, otherwise conceptually close to Zod; TypeBox: schemas _are_ JSON Schema
...
_named_ format validators, i.e. reified versions of JSON Schema's soft `format`
strings, but _enforced_ here rather than advisory), `.refine()`/`.check()` (Zod)
...
_changes_ the value/type during parsing, e.g. string→Date — a genuinely distinct
...
**Nullability/optionality:** `.optional()` (adds `| undefined` to the _output_
...
**Generics:** none of these libraries offer _schema-level_ type parameters the way
...
for an IR's _node vocabulary_ to have no native parametric-type node, if
...
- String, with an open sub-vocabulary of semantic string-shapes layered on top as
  _annotations, not new leaf kinds_ (JSON Schema `format`, GraphQL `ID`, Avro
...
- Untagged union (TS `|`, JSON Schema `anyOf`, GraphQL union with server-resolved
  `__typename`, Zod `union`) — a union is a _value-set_ union with no dedicated tag
...
- Tagged/discriminated union with an explicit literal field (TS discriminated-
  ...
  _named field_ whose value selects the branch.
- Implicit-tag union where the tag _is_ which slot is populated rather than a
...
- Boolean-algebra composition (JSON Schema `allOf`/`anyOf`/`not`) — worth keeping
  conceptually distinct from "sum type": this is composition _of schema-
  predicates_, applicable even to non-sum shapes, not a value-carrying tagged
  branch.
...
- Nullable — three observed encodings: (1) dedicated per-type flag, defaulting to
  non-nullable (JTD, SQL's `NOT NULL` opt-out has the _opposite_ default polarity
...
- Optional/presence — genuinely orthogonal to nullable in the most careful systems
  ...
  case study of what goes wrong when this axis is _not_ kept explicit).
...
- Zero-cost, compiler-enforced wrapper (Haskell `newtype`) vs. real-allocation
  ...
  recurs independently in every system that has _any_ nominal-wrapper concept,
  ...
  kinds.
- SQL `CREATE DOMAIN` — named + constrained alias, a nominal wrapper _with a
  constraint bundled in_ at declaration time, one more point on the same spectrum.
...
- No refinement at all, pushed entirely to an external layer (protobuf core,
  GraphQL core, JTD, Cap'n Proto, FlatBuffers, Avro core) — notably the _majority_
  ...
  _about_ a type node, not a distinct type-node kind).
...
- New base types addable at the engine level by third parties (Postgres
  ...
  need to go this far.
- Fallback-to-nearest-known-ancestor on unrecognized annotation, _spec-mandated_
...
- Type-widening/promotion lattice among primitives, spec-defined (Avro: `int` →
  ...
  _stricter supertype_ but to a _different leaf the target format already
  understands_, which matters for scalar leaves that don't naturally form an
...
- some map/dictionary mechanism (all but the most minimal — SQLite/MySQL lack a
  ...
  need)
- optional/nullable as a wrapper concept (though _which_ default polarity and
  _whether_ optional and nullable are unified varies — this variance itself
...
- schema-evolution-specific machinery (Avro reader/writer resolution, protobuf
  field numbers, FlatBuffers deprecation) — real, but a _process/versioning_
...
The recurring shape across nearly every system that _does_ have a formal
...
3. **Constraint/annotation axis** (an open bag hung off any node, never a new
   ...
   _dropped_, not guessed at, and the underlying shape-tree node is still valid).
