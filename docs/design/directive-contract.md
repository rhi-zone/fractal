...
| `paginated`         | `{ style?, inputCursorParam?, inputOffsetParam? }`                   | at-most-one                | Pagination hints overriding the client's shape-derived defaults — see `paginated()` (verbs.ts)                    | `extensions/pagination.ts`'s client extension                                           | Style/field names derived from response shape                  |
