> **Status: Superseded, and its central claim is CONFIRMED RESOLVED in shipped code** — replaced by [function-core-and-projection.md](./function-core-and-projection.md), which is itself partially superseded by [invariants.md](./invariants.md) (see its "STALE (2026-07-10)" note under the `methods({})` invariant) and [router-model.md](./router-model.md). Retained for the reasoning history (semiring framing, matcher insight, the names-not-verbs / tree×binding exploration — much of which the converged model adopts). NOTE: the converged model REJECTS the bidirectional view/review optic pairs proposed here as the core abstraction (transforms are one-directional); it keeps the protocol-neutral tree × per-protocol binding insight. The intervening `Handler<P>` model (handler-model.md) is also superseded.
>
> **What this doc got right (verified against current code, 2026-08-16):** `methods` has in fact dissolved. `packages/api-tree/src/node.ts`'s `Node` type has no node-kind discriminant and no `methods`/verb-dispatch variant — a node is exactly `{ handler?, children?, fallback?, meta }`. There is no `methods()` combinator anywhere in `packages/api-tree` or `packages/http-api-projector`; the only `methods()` combinator left in the repo lives in `spike/std/` (exploratory code, not shipped). Verb assignment happens per-leaf via metadata bundles (`op(fn, http.put)`, `packages/http-api-projector/src/verbs.ts`) that set `meta.http.verb`/`meta.http.method` plus implied behavioral tags, or is derived from the tag lattice by `verbFromTags` (`packages/http-api-projector/src/tags.ts`) when no explicit verb is pinned — `GET`/`PUT`/`DELETE`/`POST` from `readOnly`/`idempotent`/`destructive`. Verb never appears as a tree-level dispatch key; it is exactly the "binding-side rendering, never a tree key" this doc argued for.
>
> **What this doc got wrong (the mechanism):** the actual mechanism is not the bidirectional-optics / explicit `op-name → (verb, path)` table this doc converges on. `invariants.md` explicitly REJECTS an explicit name→verb table and any CRUD-taxonomy verb scheme ("the `read→GET / replace→PUT / remove→DELETE / partial→PATCH` verb table is REJECTED... assistant-invented, not a designed operation kind"). What shipped instead is per-leaf open metadata (tags + optional explicit verb pin), not a table indexed by operation name, and not optics of any kind — `router-model.md`'s dispatch DU + `verbFromTags`'s tag lattice. Treat the historical value of this doc as: it correctly predicted `methods` would dissolve and verb would become binding-side, but its proposed replacement mechanism is not what was built.
