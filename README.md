...
| `@rhi-zone/fractal-http` | HTTP kit: `path`, `methods`, `param`, `query`, `header`, `body`, `validate`, `route`, `serve` |
...
| `@rhi-zone/fractal-client` | Typed client: `client(node, transport?)` → `ClientOf<typeof node>`; `inProcess` (Hyper unification) + `http(baseUrl)` transports |
