...
- **JSON inference**: clustering/union splitting design is a blocker for release but no explicit ordering in roadmap. Clustering algorithm candidates now implemented and measured (see below) — remaining open question is whether to keep `clusteringMethod` as a caller-facing choice (current state) or pick one default via further real-corpus validation.
