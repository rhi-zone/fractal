...
- ~~**`bun test` is red on master, one test**~~ — RESOLVED (2026-08-16). Root-caused to a broken measurement, not a real regression: `createExtractorProgram`'s deterministic file-count assertion (the actual mechanism check) always passed; only the informative `.heapUsed`-based measured assertion in `packages/type-ir/src/from-typescript.memory.test.ts` was flaky (~30-40% fail rate across ~25 local runs). Confirmed via a minimal repro (allocate ~600MB of plain objects, drop the reference, force `Bun.gc(true)`) that on Bun 1.3.9, `process.memoryUsage().heapUsed` does not reliably fall back down after a forced GC even when the objects are provably collected — `.heapTotal` (the VM's committed heap size) does shrink correctly in the same repro. That's a Bun-version measurement quirk, not anything `createExtractorProgram` does. Fixed the test: switched the measured assertion to `.heapTotal`, and replaced the "sample a peak during an unretained loop" approach (inherently a race with background GC) with explicit retention of all N independent Programs followed by a forced GC — a deterministic worst-case bound instead of a timing-dependent sample. Re-verified the actual memory benefit is real and large: ~850MB (12 independent Programs, retained) vs ~15-150MB (1 shared Program covering the same 12 roots), ~6-50x, with zero overlap across 20 in-process reruns + 6 fresh-process runs. No production code changed.
...
- **RESOLVED (2026-08-16): `graphql-api-projector/src/codegen.ts`'s `buildTree` didn't handle a bare-leaf `fallback.subtree`** (found 2026-08-01 while fixing the MCP/JSON-RPC/CLI/GraphQL/HTTP walk-blind-spot family — see aa28952/its sibling fixes). `client.ts`'s runtime `buildClientNode` was fixed so a bare-`op()` `fallback.subtree` (Node model explicitly allows this) resolves to the leaf's own caller directly instead of an empty nested client object (no further tree position to descend into); `codegen.ts`'s STATIC client generator had the identical gap — `buildTree` always recursed into `subtree.children` (`undefined` for a leaf), silently producing an empty object type / no-op runtime member instead of a real call signature. Fixed by making `ClientTreeNode.param` the union the original entry called for: `{ readonly name: string } & ({ readonly subtree: ClientTreeNode } | { readonly operation: OperationEntry })`. `buildTree` now checks `isLeaf(subtree)` (mirroring `buildClientNode`'s own check) and builds an `OperationEntry` directly via `buildLeafEntry(subtree, path, name, ...)` instead of recursing. The render path (`nodeTypeLiteral`/`nodeRuntimeLiteral`/`collectOperations`) branches on `"subtree" in node.param`; the operation-signature/call-body logic used for a normal member entry was extracted into `operationTypeSig`/`operationRuntimeLiteral` so the bare-leaf-param case reuses it verbatim (wrapped in the same `(name: string) => ...` closure `buildClientNode` already uses for the fallback capture). New coverage: `codegen.test.ts`'s "generateGraphQLClient — bare-leaf fallback.subtree" suite (structural assertion the generated type/call signature is real, not `{}`, plus an eval end-to-end test proving the generated `createClient`'s fallback member is directly callable) — confirmed to fail pre-fix (empty-object type, non-function member) and pass post-fix; full `graphql-api-projector` suite (145 tests) and package typecheck both green.
