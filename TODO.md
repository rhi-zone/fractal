...
- **Service stores are not yet threaded into each projector's per-request bag** — `StoreRegistry` can now carry a REQUIRED, deployment-provided service store, and `ServiceStores` (api-tree's input.ts) is the single registration site that structurally checks one is supplied. What does NOT exist yet is the wiring that merges a registered service store into the `stores` bag each projector builds per request — `docs/design/typed-store-spec.md` §8 explicitly defers that as "an implementation-order question across five projector packages' preset options". Until it lands, each projector's bag is typed `ProjectorStores & XStores` (`Omit<Stores, RequiredServiceStoreKeys>` plus its own fragment), which is honest about the fact that a projector genuinely doesn't have the service stores in hand when it builds a request. Landing it means threading a `ServiceStores` value through each projector's own options into its store-builder — plausibly with the `HasRequiredKeys` conditional-arity technique `op()`/`api()` already use, so the option is only required when the merged registry actually declares a service store — after which each `XStoreBag` widens back to `Stores & XStores` and `ProjectorStores` goes away.
