...
//     `createFetch`/`toDropInFetch` fetch implementation over the same
//     tree, so the generated pages' embedded <ApiExplorer/> instances hit a
//     real in-process backend that answers with the routes the MDX
//     documents.
//
// Deliberately tiny: one GET with a path param (bookId), one POST with a
// request body (title/author) — enough to exercise both
// requestSchema/responseSchema embedding and path-param rendering, per this
// fixture's scope (examples/doc-site-verification's README).
//
// Imported by package name (`@rhi-zone/fractal-api-tree/node`, not a
// repo-relative src path): unlike examples/library-api, this site is not a
// bun-workspace member (see examples/doc-site-verification/README.md), so
// its own package.json's `file:`/`overrides` wiring is what makes these
// package-name imports resolve, against the workspace packages' built
// `dist/` output.
