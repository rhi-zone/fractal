// Library API — authored on the HttpRoute pipeline (naiveTransform +
// applyMethods/applyMoveTo/applyResponse, see
// packages/http-api-projector/src/route.ts and
// docs/design/routing-and-transforms.md). Each leaf carries its own tags;
// there is no ancestor tag inheritance (see docs/design/router-model.md —
// "Tags"): a node-level tag does not flow down to its descendants.
//
// Callables are leaf nodes stored in `children` via `op(fn, meta?)`. A
// node's `fallback` option (shape `{ name, subtree }`) captures the
// wildcard-capture subtree.
//
...
//
// Co-location is expressed with the `moveTo` rewriter directive
// (`applyMoveTo`, see route.ts and docs/design/routing-and-transforms.md §
// "Motivating example"): `read`/`replace`/`remove` stay nested inside the
// fallback subtree (alongside `checkout` — this is what gives them the
// `books_bookId_read` etc. MCP/CLI names, since those projections read raw
// tree position with no moveTo pass), each with `moveTo: ".."` — go up to
// the parent position (paths resolve relative to the node's own position;
// `..` moves up one level to the parent, the fallback subtree's own root).
// Method assignment is a second, independent directive (`{kind:"method"}`,
// read by `applyMethods`) — `http.get`/`http.put`/`http.delete` bundle both
// the verb and the tags that verb implies.
//
...
export const api = api_({
  ...
  // node (see docs/design/router-model.md — "Tags").
  catalog: api_({
...
// docs/design/wire-profiles-and-staged-validation.md). `bun run codegen`
// (package.json, `build`) scans this file for 3-arg
// `applyValidation(key, treeExpr, protocol)` invocations and emits
// `examples/library-api/src/generated/apply-validation.ts` via
// `WireValidatorMap` — the third argument names the wire protocol (`"http"`
// here) so codegen derives each leaf's per-field encoding: query/path params
// decode from numeric/strict-boolean/ISO-date strings (`queryProfile`-style),
// a JSON body's `Date` fields decode from an ISO string (`jsonProfile`-style),
// everything else in the body arrives already typed with no coercion at all —
// see `packages/api-tree/src/wire-derive.ts`'s `deriveFieldProfiles("http",
// ...)`.
//
// Applied to `api` (the raw Node), before any protocol-specific projection.
// This still works for HTTP despite `read`/`replace`/`remove` below being
// `moveTo`-relocated onto the fallback's own root during HTTP projection:
// `applyValidation` wraps each leaf's handler (a function reference), and the
// wrap travels with that reference wherever `httpProjection`'s `applyMoveTo`
// later moves it — the path keys this call's own walk uses (matching what
// codegen extracted) never need to agree with HTTP's post-projection dispatch
// paths. A leaf with no matching generated entry keeps its original handler
// untouched (permissive by default — `assertValidationCoverage` is the
// opt-in loud check, not run here).
//
...
// to claim here. A tree dispatched over two divergent wires (e.g. HTTP and
// CLI) would claim one key per protocol instead — see
// wire-profiles-and-staged-validation.md's "Implementation trace" section
// for the pattern this example would follow if a second live protocol were
// added.
// ============================================================================
