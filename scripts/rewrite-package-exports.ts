...
function rewriteExportsEntry(key: string, entry: unknown): unknown {
  ...
  if (entry === null || typeof entry !== "object" || Array.isArray(entry)) {
    throw new Error(`exports["${key}"]: expected an object with "types"/"import", got ${JSON.stringify(entry)}`)
  }
  const obj = entry as Record<string, unknown>
  const keys = Object.keys(obj)
  const unexpected = keys.filter((k) => k !== "types" && k !== "import")
  if (unexpected.length > 0) {
    throw new Error(`exports["${key}"]: unhandled condition(s) ${JSON.stringify(unexpected)} — extend this script before rewriting`)
  }
  if (!isSrcTsPointer(obj.types) || !isSrcTsPointer(obj.import)) {
    throw new Error(`exports["${key}"]: expected both "types" and "import" to point at "./src/*.ts", got ${JSON.stringify(obj)}`)
  }
  if (obj.types !== obj.import) {
    throw new Error(`exports["${key}"]: "types" and "import" point at different files (${obj.types} vs ${obj.import}) — unhandled shape`)
  }
...
export function rewritePackageJson(pkg: Record<string, unknown>): Record<string, unknown> {
  ...
  out.publishConfig = {
    ...(typeof out.publishConfig === "object" && out.publishConfig !== null ? out.publishConfig : {}),
    access: "public",
  }
...
async function main() {
  ...
  if (pkgDirs.length === 0) {
    console.error("usage: bun scripts/rewrite-package-exports.ts [--check] <package-dir> [...more]")
    process.exit(1)
  }
  ...
  for (const pkgDir of pkgDirs) {
    ...
    console.log(`${pkgJsonPath}: rewrote exports (${Object.keys((rewritten.exports as object) ?? {}).length} entries)`)
  }
