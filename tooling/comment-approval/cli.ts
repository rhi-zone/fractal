...
import { approveComments, isApproved, loadManifest, saveManifest, type Manifest } from "./manifest.ts"
...
async function cmdCheck(args: string[]): Promise<number> {
  ...
  const explicitFiles = flag && !flag.startsWith("--") ? [flag, flagArg, ...rest].filter((f): f is string => Boolean(f)) : undefined
  ...
  console.log(`comment-approval: ${unapproved.length} unapproved comment(s) in ${files.length} scanned file(s):\n`)
...
async function cmdApprove(args: string[]): Promise<number> {
  const [flag, flagArg, ...rest] = args
  if (!flag) {
    console.error("usage: bun tooling/comment-approval/cli.ts approve <--all|--staged|--diff-base <ref>|<files...>>")
    return 1
  }
  const explicitFiles = !flag.startsWith("--") ? [flag, flagArg, ...rest].filter((f): f is string => Boolean(f)) : undefined
  ...
  console.log(`comment-approval: approved ${total} comment(s) across ${files.length} file(s) (manifest now has ${Object.keys(manifest.approved).length} approved hash(es))`)
  return 0
}
