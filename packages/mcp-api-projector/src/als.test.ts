...
async function connectedClient<T>(
  tree: Node,
  opts: Omit<CreateMcpServerOptions<T>, "name" | "version">,
) {
...
describe("CreateMcpServerOptions.als — tools", () => {
  ...
  it("concurrent tool calls stay isolated — each sees its own context value", async () => {
    ...
    let counter = 0;
    const { client } = await connectedClient(tree, {
      als: { storage, init: () => ({ requestId: `req-${counter++}` }) },
    });
...
describe("CreateMcpServerOptions.als — resources", () => {
    ...
    users: api_(
      {},
      {
        fallback: {
          name: "userId",
          subtree: api_({
            profile: op((_: unknown) => ({}) as { requestId?: string }, {
              mcp: { as: "resource" },
            }),
          }),
        },
  ...
  it("a fixed resource read runs inside the ALS context", async () => {
    const storage = new AsyncLocalStorage<{ requestId: string }>();
    const alsTree = api_({
      config: op((_: unknown) => ({ requestId: storage.getStore()?.requestId ?? "none" }), {
        mcp: { as: "resource" },
      }),
    });
    const { client } = await connectedClient(alsTree, {
      als: { storage, init: () => ({ requestId: "req-fixed" }) },
    });
      ...
      greet: op(
        (_: unknown) => ({
          messages: [
            {
              role: "assistant",
              content: { type: "text", text: storage.getStore()?.requestId ?? "none" },
            },
          ],
        }),
        { mcp: { as: "prompt" } },
