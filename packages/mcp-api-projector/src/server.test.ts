...
const promptTree = api_({
  ...
  docs: api_({
    critique: op(
      (input: { text: string }) => ({
        messages: [
          { role: "user", content: { type: "text", text: `critique this: ${input.text}` } },
        ],
      }),
      {
        mcp: { as: "prompt" },
...
async function connectedRichContentClient() {
  const server = createMcpServer(richContentTree, {
    name: "rich-content-server",
    version: "1.0.0",
  });
  const [clientTransport, serverTransport] = InMemoryTransport.createLinkedPair();
  const client = new Client({ name: "test-client", version: "1.0.0" });
...
async function connectedRichResourceClient() {
  const server = createMcpServer(richResourceTree, {
    name: "rich-resource-server",
    version: "1.0.0",
  });
  const [clientTransport, serverTransport] = InMemoryTransport.createLinkedPair();
  const client = new Client({ name: "test-client", version: "1.0.0" });
...
describe("createMcpServer — sourceMap support (tools)", () => {
  ...
  async function connectedSourceMapClient() {
    const server = createMcpServer(sourceMapTree, {
      name: "sourcemap-tool-server",
      version: "1.0.0",
    });
    const [clientTransport, serverTransport] = InMemoryTransport.createLinkedPair();
    const client = new Client({ name: "test-client", version: "1.0.0" });
...
async function connectedStreamingClient() {
  const server = createMcpServer(streamingTree, {
...
describe("createMcpServer — tools/call streaming", () => {
  it("sends a progress notification for each StreamProgress yield when the caller supplies a progressToken", async () => {
    ...
    const result = await client.callTool({ name: "progressAndChunks", arguments: {} }, undefined, {
...
describe("createMcpServer — detection", () => {
  ...
  async function connectedClientFor(
    tree: ReturnType<typeof api_>,
...
describe("createMcpServer — logging", () => {
  it("advertises the logging capability only when opts.logging is set", async () => {
    ...
    await Promise.all([
      offServer.connect(offServerTransport),
      offClient.connect(offClientTransport),
    ]);
    expect(offClient.getServerCapabilities()?.logging).toBeUndefined();

    const onServer = createMcpServer(tree, {
