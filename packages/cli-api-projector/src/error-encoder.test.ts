...
function makeIO() {
  ...
  return {
    ...
    err: () =>
      writes
        .filter((w) => w.stream === "err")
        .map((w) => w.text)
        .join(""),
    io: {
      stdout: {
        write: (s: string) => {
          writes.push({ stream: "out", text: s });
        },
      },
      stderr: {
        write: (s: string) => {
          writes.push({ stream: "err", text: s });
        },
      },
      confirm: async () => true,
    },
...
describe("cliErrors", () => {
  ...
  it("a mapping entry with only `exit` keeps the default message", async () => {
    ...
    expect(caught?.message).toBe(
      `Error: ${JSON.stringify({ kind: "conflict", message: "already exists" })}`,
    );
  });
  ...
  it("no errorEncoder configured — current exit-1 default behavior unchanged", async () => {
    ...
    expect(caught?.exitCode).toBe(1);
    expect(caught?.message).toBe(
      `Error: ${JSON.stringify({ kind: "notFound", message: "Book not found" })}`,
    );
  });
