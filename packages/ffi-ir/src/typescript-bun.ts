...
//      `{ [name]: { args: FFIType[], returns: FFIType } }`.
//   2. The complete `FFIType` token vocabulary, by value: `char`(0) `i8`(1)
//      `u8`(2) `i16`(3) `u16`(4) `i32`/`int`/`c_int`(5)
//      `u32`/`uint`/`c_uint`(6) `i64`/`isize`(7) `u64`/`usize`(8)
//      `f64`/`double`(9) `f32`/`float`(10) `bool`(11)
//      `ptr`/`pointer`/`void*`(12) `void`(13) `cstring`(14) `i64_fast`(15)
//      `u64_fast`(16) `function`/`fn`/`callback`(17) `napi_env`(18)
//      `napi_value`(19) `buffer`(20).
//   - `"void"` (13) is a real, distinct `FFIType` token, separate from
//     `ptr`(12) — void returns are emitted as `returns: "void"` below, not
//     omitted.
//   - No struct type exists in this enum; struct-by-value has no direct
//     mapping and must be passed via manual pointers. `toBunFfiType` below
//     throws explicitly for any shape that would require struct-by-value (or
//     any other non-token-representable encoding) rather than guessing a
//     lossy one.
//
...
//   - `"refcount"` — also `"ptr"`, and not gated the way `rust-c-abi.ts`
//     gates it. `rust-c-abi.ts` throws on `refcount` because that file has to
//     generate the free-function/bookkeeping implementation itself, and
//     plain C has no native refcount mechanism to generate correct code for.
//     This file generates no implementation at all — only a `dlopen`
//     symbol-type declaration — and at that level a refcounted handle
//     crosses the C ABI the same way an opaque handle does: a raw pointer
//     (e.g. `Arc::into_raw`/`Box::into_raw` are both `*const/*mut T` at the
//     ABI). The loader code is identical regardless of which of these two
//     disciplines a given handle uses. (Same "same reasoning, different
//     conclusion per file" latitude `gleam.ts`/`rescript.ts` take relative to
//     `wasm-bindgen.ts`/`melange.ts` for the identical four-discipline
//     question, documented in their own file headers.)
//   - `"resource"` (WIT's own/borrow) — throws. WIT's Canonical ABI crosses a
//     resource as an `i32` handle-table index into a per-instance table with
//     lend-count tracking, not a raw memory pointer — a different wire
//     representation than anything a plain `dlopen`'d C shared library
//     (which never speaks the Canonical ABI) can produce or consume. Matches
...
// Minimal primitive-kind -> FFIType mapping, not a full TypeShape Converter
// table the way wasm-bindgen.ts's `handlers` is — bun:ffi has no
// compound-type tokens at all (see file header), so there is no
// struct/enum-hoisting path to build; every non-primitive kind is
// unsupported.
...
function buildSymbolEntry(symbolName: string, shape: FfiFunctionLike, selfParam: boolean): string {
  ...
  // `dlsym` — the key in the symbols object must exactly match the native
  // exported symbol name; no separate lookup-name field exists, unlike
  ...
  // A quoted string key sidesteps that risk and the separate
 ...
 * symbol map — the aggregation step the `module` case of `toBun` needs for
 * its single `dlopen` call, reused by the standalone `function`/`resource`
 * cases of `toBun` too. */
function collect(ref: FfiRef, name: string): { entries: string[]; wrappers: string[] } {
  ...
  if (kind === "resource") {
    ...
    // (`buildFreeFunction`, `<resource>_free`) is a real exported symbol in
    // the compiled library this file's whole job is to bind against, so it
    // gets its own symbol entry and wrapper here alongside the methods.
 ...
 * models the boundary shape, not deployment paths).
 *
 ...
 *     no way to know that resource's other methods or its free function, so
 *     — mirroring `rust-c-abi.ts`'s `method`-requires-`name` precedent — a
 ...
 *     every contained resource's methods' symbols, following bun:ffi's own
 *     constraint that `dlopen` loads the whole shared library in one call,
 *     with one entry per exported symbol needed from it.
 */
