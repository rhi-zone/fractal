...
//
// Scope and decisions this file follows, all already made in
// docs/design/ffi-ir-architecture-options.md (Fork B follow-up, Fork C
// "discipline-per-target: decided") — not re-derived here:
//
//   - WIT is one more *target* for fractal (a `.wit` emission projector,
//     the same shape as the ~120 type-ir projectors), not a shared IR
//     fractal's own C/JS backends sit on top of. See the doc's Fork B
//     follow-up, "Net finding" (line ~469-489).
...
//     free). `opaque-handle` and `refcount` THROW — explicitly excluded
//     for this target per the doc's "Fork C, discipline-per-target:
//     decided" subsection: opaque-handle+free-fn is redundant with
//     `resource` (which provides strictly more — a lend-count/trap safety
//     net `opaque-handle` lacks), and refcount is a confirmed structural
//     mismatch (WIT's Canonical ABI is lend-count-and-trap, not
//     reference counting).
//
// `module` -> WIT `interface`, not `world`: verified from
// component-model.bytecodealliance.org/design/worlds.html (fetched
// 2026-08-03, quoted in the design doc line ~371-378) — "an interface
// groups named types+functions" while "a world describes the
// functionality a component provides, and the functionality it requires
// in order to work" via import/export declarations. ffi-ir's `module`
// kind (per index.ts) is a flat `{ functions, resources }` map with no
// import/export/binding concept at all — structurally an `interface`,
// not a `world`. This mapping is a straightforward read of the spec, not
// a judgment call between two live options.
//
// `resource` methods use WIT's own nested method syntax — verified
// (fetched 2026-08-03, component-model.bytecodealliance.org/design/wit.html):
// a resource block's methods "implicitly take a `self` (AKA `this`)
// parameter that is a handle" and are written `name: func(params) ->
// ret;` INSIDE the `resource { ... }` block, with no explicit receiver
// parameter — e.g. `resource blob { write: func(bytes: list<u8>); }`, not
// `write: func(self: borrow<blob>, bytes: list<u8>);` as a free function.
// This lines up with ffi-ir's own `method` shape, whose `params` already
// excludes the receiver (the receiver is carried separately, as
// `FfiKinds.method.receiver`) — so `function` and `method` emit the exact
// same func-line syntax here; the only difference is where that line is
// placed (top-level in an `interface`, vs. nested in a `resource` block).
//
// WIT identifiers are ASCII kebab-case ("sequences of words, separated by
// single hyphens" — verified, wit.html, fetched 2026-08-03); `toKebabCase`
// below converts ffi-ir/type-ir's camelCase-by-convention names to match.
//
// Data-shape (type-ir TypeRef -> WIT type syntax) coverage is DELIBERATELY
// MINIMAL here — primitives, `record` (from type-ir's `object`), `list<T>`
// (from `array`), `option<T>` (from `meta.optional`/`meta.nullable`), plus
// `ref`/resource-handle syntax for the ownership cases above. No
// standalone `type-ir -> WIT` data-shape projector exists yet in
// packages/type-ir/src/ (confirmed by listing that directory — no `wit*.ts`
// file there); building the full same-shape addition other targets get
 ...
 * (component-model.bytecodealliance.org/design/wit.html, fetched
 * 2026-08-03: `"add: func(a: u64, b: u64) -> u64;"`). The return-type arrow
 * is omitted entirely for a void/null return — WIT's function grammar has
 * no unit/void return type to spell out; a function that returns nothing
 * simply has no `-> ...` clause (matches the `draw-line: func(...);`
 * example in the same fetch, which has no arrow at all). */
function funcLine(
  name: string,
 ...
 * (same fetch: `"resource blob { constructor(init: list<u8>); write:
 * func(bytes: list<u8>); ... }"`). ffi-ir's `resource` schema (index.ts)
