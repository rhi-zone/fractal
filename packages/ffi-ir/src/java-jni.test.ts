...
describe("toJniType", () => {
  ...
  test("resource discipline (own/borrow) throws — WIT-only, no JNI/Android NDK convention enforces it", () => {
    expect(() =>
      toJniType(withOwnership(t({ kind: "ref", target: "FileHandle" }), ownership.resource("own"))),
    ).toThrow(/unsupported ownership discipline "resource" for JNI target/);
    expect(() =>
      toJniType(
