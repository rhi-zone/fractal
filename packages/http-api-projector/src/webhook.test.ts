...
describe("webhookSignatureLayer", () => {
  ...
  it("accepts a base64-encoded signature", async () => {
    ...
    for (let i = 0; i < bytes.length; i++)
      bytes[i] = Number.parseInt(hexSig.slice(i * 2, i * 2 + 2), 16);
    let binary = "";
...
describe("webhookSignatureLayer + replayPreventionLayer composed", () => {
  it("rejects a bad signature before replay tracking runs", async () => {
    const handler = webhookSignatureLayer({ secret: SECRET })(replayPreventionLayer()(innerFetch));
    const res = await handler(
      post("{}", { "X-Webhook-Delivery-ID": "evt_7", "X-Webhook-Signature": "bad" }),
    );
    expect(res.status).toBe(401);
  });
