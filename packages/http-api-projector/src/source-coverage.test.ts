...
import { SourceCoverageError, checkRouteSourceCoverage, findRouteSourceCoverageProblems, httpRoute, makeRouterFromRoute } from "./route.ts"
...
describe("findRouteSourceCoverageProblems", () => {
  ...
  it("accepts a declaration-merged store name passed via knownStores", () => {
    const route = leaf("GET", { paramNames: ["session"], sourceMap: { session: { store: "cookie" } } })
