...
describe("restCrud() tree shape", () => {
  ...
  it("wires list/create as children carrying moveTo('..'), not sibling keys", () => {
    ...
    expect((tree.children?.create as Node).meta.http).toMatchObject({ method: "POST", moveTo: ".." })
  })
  ...
  it("after httpProjection + applyMoveTo, get/update/delete land at the id fallback root", () => {
    ...
    expect(Object.keys(routes.fallback?.subtree.methods ?? {}).sort()).toEqual(["DELETE", "GET", "PUT"])
  })
})
...
describe("restCrud() end-to-end over HTTP", () => {
  ...
  it("GET / lists, POST / creates, GET/PUT/DELETE /:id co-locate onto the id fallback", async () => {
    ...
    // create
    const createRes = await fetch(jsonReq("POST", "http://localhost/widgets", { name: "Widget One" }))
  ...
  it("no path collision between /widgets (collection) and /widgets/:id (item)", () => {
    const tree = api({ widgets: restCrud({ list: (_: unknown) => [], get: (_: { id: string }) => ({}) }) })
    const routes = httpProjection(tree)
    const collectionMethods = routeCandidatesForUrl(routes, "http://localhost/widgets").map((c) => c.method)
    const itemMethods = routeCandidatesForUrl(routes, "http://localhost/widgets/abc").map((c) => c.method)
