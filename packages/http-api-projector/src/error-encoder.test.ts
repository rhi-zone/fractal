    ...
    const router = makeRouterFromRoute(
      ...
      (error) => {
        if (
          typeof error === "object" &&
          error !== null &&
          (error as { kind?: unknown }).kind === "notFound"
        ) {
