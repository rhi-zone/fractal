...
function extractSignatureBytes(headerValue: string): Uint8Array<ArrayBuffer> | undefined {
  ...
  const unprefixed =
    eq > 0 && /^[a-zA-Z0-9-]+$/.test(headerValue.slice(0, eq))
      ? headerValue.slice(eq + 1)
      : headerValue;

  return (
    hexToBytes(unprefixed) ??
    base64ToBytes(unprefixed) ??
    hexToBytes(headerValue) ??
    base64ToBytes(headerValue)
  );
}
...
export function createInMemoryReplayStore(
  defaultTtlMs: number = DEFAULT_REPLAY_TTL_MS,
): ReplayStore {
