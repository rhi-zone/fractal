...
describe("listRoutes", () => {
  ...
  it("meta propagates through per method entry, independent of sibling methods", () => {
    ...
    const route = httpRoute({
      methods: {
        GET: {
          handler: (_: unknown) => null,
          meta: { openapi: { summary: "get it" } } as RouteLeafMeta,
        },
        POST: {
          handler: (_: unknown) => null,
          meta: { openapi: { summary: "post it" } } as RouteLeafMeta,
        },
      },
      meta: {},
    });
...
describe("mergeOpenApiDocs", () => {
  ...
  it("throws when two documents define the same security scheme name", () => {
    ...
    const b = baseDoc({
      components: { securitySchemes: { bearer: { type: "apiKey", in: "header", name: "X" } } },
    });
...
describe("schema correlation under composition — path-keyed schemas resolve correctly where bare-name-keyed schemas would collide", () => {
  ...
  it("bare-name-keyed schemas neither collide nor correlate once composed — both leaves silently degrade to the placeholder schema", async () => {
    ...
    const bareNameKeyedSchemas = {
      list: {
        ...
        outputSchema: { type: "object" as const, title: "A" },
      },
    };
  ...
  it("path-keyed schemas (schemaKey / buildPathMap) resolve each slice's `list` independently, no collision", async () => {
    const pathKeyedSchemas = {
      ...
      "b/list": {
        inputSchema: { type: "object" as const },
        outputSchema: { type: "object" as const, title: "B" },
      },
    };
...
describe("toOpenApi(n, { sourceFile }) — auto-discovery treeId resolution", () => {
  ...
  const twoTreesFixture = new URL("./__fixtures__/openapi-two-trees.fixture.ts", import.meta.url)
    .pathname;

  function inputSchemaAt(doc: OpenApiDoc, path: string, method: string): unknown {
    return (
      doc.paths[path]?.[method]?.requestBody as
  ...
  it("a single-tree file resolves the real schema with no opts.treeId — no silent degrade to the placeholder", async () => {
    ...
    const schema = inputSchemaAt(doc, "/widgets", "post") as
  ...
  it("opts.treeId disambiguates — catalogTree resolves its OWN schema, not inventoryTree's", async () => {
    const doc = await toOpenApi(catalogTree, {
      sourceFile: twoTreesFixture,
      treeId: "catalogTree",
    });
    const schema = inputSchemaAt(doc, "/widgets", "post") as
      | { properties?: Record<string, unknown> }
  ...
  it("opts.treeId disambiguates — inventoryTree resolves its OWN schema, not catalogTree's", async () => {
    const doc = await toOpenApi(inventoryTree, {
      sourceFile: twoTreesFixture,
      treeId: "inventoryTree",
    });
    const schema = inputSchemaAt(doc, "/widgets", "post") as
      | { properties?: Record<string, unknown> }
      | undefined;
