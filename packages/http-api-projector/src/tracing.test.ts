...
import {
  ...
  OtelSpanStatusCode,
  parseTraceParent,
} from "@rhi-zone/fractal-api-tree/otel";
import type {
  OtelAttributes,
  OtelSpan,
  OtelSpanContext,
...
function makeFakeTracer(): { tracer: OtelTracer; spans: RecordedSpan[] } {
  ...
  const tracer: OtelTracer = {
    ...
    startActiveSpan(
      name: string,
      a: OtelSpanOptions | ((span: OtelSpan) => unknown),
      b?: (span: OtelSpan) => unknown,
    ) {
...
describe("tracingLayer", () => {
  ...
  it("defaults projector.type to 'http' and honors an override", async () => {
    ...
    await tracingLayer({ tracer }, { projectorType: "custom" })(inner)(
  ...
  it("records ERROR status and rethrows when inner throws", async () => {
    ...
    await expect(tracingLayer({ tracer })(inner)(new Request("http://localhost/"))).rejects.toBe(
      boom,
    );
    expect(spans[0]!.ended).toBe(true);
  ...
  it("parses an incoming traceparent header and hands it to contextFromTraceParent", async () => {
    ...
    await tracingLayer(integration)(inner)(
      new Request("http://localhost/", { headers: { traceparent: remoteHeader } }),
    );
