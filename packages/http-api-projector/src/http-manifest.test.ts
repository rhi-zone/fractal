...
describe("HttpManifest type safety", () => {
  it("a bare leaf (no method directive) defaults to POST, matching naiveTransform's baseline", () => {
    ...
    expectTypeOf<Manifest>().toEqualTypeOf<{
      readonly "/ping": {
        readonly POST: { readonly input: { n: number }; readonly output: number };
      };
    }>();
  });
  ...
  it("distinct verbs across siblings each resolve their own literal method", () => {
    ...
    expectTypeOf<Manifest>().toEqualTypeOf<{
      readonly "/books/list": {
        readonly GET: { readonly input: unknown; readonly output: string[] };
      };
      readonly "/books/add": {
  ...
  it("a leaf carrying moveTo resolves to its runtime (applyMoveTo) path, not its raw authored position", () => {
    const bookItem = api({
