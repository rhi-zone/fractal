...
export function serveFastlyCompute(handler: FetchHandler): void {
  addEventListener("fetch", (event) => {
    const fastlyEvent = event as unknown as { request: Request; respondWith(response: Promise<Response>): void }
...
const TEXT_CONTENT_TYPE = /^(text\/|application\/(json|xml|javascript|x-www-form-urlencoded)|.+\+(json|xml)$)/i
