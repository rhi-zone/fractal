...
function customCursorList(input: {
  readonly limit?: number;
...
describe("pagination() — cursor style", () => {
  ...
  it("for await walks every item across every page", async () => {
    const client = testClient();
    const seen: Item[] = [];
    for await (const item of (await client.items.listCursor({
      limit: 10,
    })) as unknown as PageIterator<CursorPage<Item>>) {
      seen.push(item);
    }
  ...
  it("getPage() fetches one page at a time, advancing the cursor across calls", async () => {
    const client = testClient();
    const iter = (await client.items.listCursor({ limit: 10 })) as unknown as PageIterator<
      CursorPage<Item>
    >;
...
describe("pagination() — offset style", () => {
  it("for await walks every item across every page using offset/total", async () => {
    const client = testClient();
    const seen: Item[] = [];
    for await (const item of (await client.items.listOffset({
      limit: 7,
    })) as unknown as PageIterator<OffsetPage<Item>>) {
      seen.push(item);
    }
    expect(seen).toHaveLength(25);
    expect(seen.map((i) => i.id)).toEqual(ALL_ITEMS.map((i) => i.id));
  ...
  it("preserves extra query params (limit) across every follow-up request", async () => {
    const client = testClient();
    let count = 0;
    for await (const _item of (await client.items.listOffset({
      limit: 3,
    })) as unknown as PageIterator<OffsetPage<Item>>) {
      count++;
    }
...
describe("pagination() — paginated() directive customization", () => {
  it("honors a custom inputCursorParam name", async () => {
    const client = testClient();
    const seen: Item[] = [];
    for await (const item of (await client.items.listCustom({
      limit: 10,
    })) as unknown as PageIterator<CursorPage<Item>>) {
      seen.push(item);
    }
