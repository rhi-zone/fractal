...
function openSseStream(frames: readonly string[]): {
...
describe("streaming() — decodeResponse", () => {
  ...
  it("yields plain data frames and returns the event: done payload as the generator's return value", async () => {
    const ext = streaming();
    const res = closedSseStream(
      "data: 1\n\n" + "data: 2\n\n" + 'event: done\ndata: {"total":2}\n\n',
    );
    const iterable = (ext.decodeResponse?.(res, testCtx)?.value ?? undefined) as AsyncGenerator<
      unknown,
      unknown,
      undefined
    >;
  ...
  it("cancels the underlying reader when the consumer breaks early", async () => {
    ...
    const iterable = (ext.decodeResponse?.(response, testCtx)?.value ??
      undefined) as AsyncGenerator<unknown, unknown, undefined>;
...
describe("generateClient — streaming() codegen", () => {
  ...
  it("emits Promise<Array<T>> (unchanged) when streaming() is NOT included, even for an x-stream schema", () => {
    ...
    expect(withoutExt).toMatch(
      /readonly generate: \(callOpts\?: CallOptions\) => Promise<GenerateGetOutput>/,
    );
  ...
  it("emits AsyncIterable<T> and a __requestStream call when streaming() is included", () => {
    ...
    expect(withExt).toMatch(
      /readonly generate: \(callOpts\?: CallOptions\) => AsyncIterable<GenerateGetOutput>/,
    );
    expect(withExt).toContain("export type GenerateGetOutput = string");
    expect(withExt).toMatch(
      /generate: \(callOpts\?: CallOptions\): AsyncIterable<GenerateGetOutput> =>/,
    );
    expect(withExt).toMatch(
      /__requestStream\(baseUrl, fetchImpl, headers, "GET", `\/generate`, undefined,/,
    );
  });
});
  ...
  it("a generated client's __requestStream reassembles a real SSE response over real HTTP", async () => {
    ...
    const mod = (await import(pathToFileURL(modulePath).href)) as {
      createClient: (baseUrl: string) => {
        readonly generate: () => AsyncGenerator<unknown, unknown, undefined>;
      };
    };
