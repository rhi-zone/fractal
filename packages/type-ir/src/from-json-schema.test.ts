...
describe("array", () => {
  test("element type", () => {
    expect(fromJsonSchema({ type: "array", items: { type: "integer" } })).toEqual(
  ...
  test("discriminated union: oneOf + discriminator.propertyName -> meta.discriminator", () => {
    const schema = {
      oneOf: [
        ...
        {
          type: "object",
          properties: { type: { const: "square" }, side: { type: "number" } },
          required: ["type", "side"],
        },
      ],
    ...
    expect(fromJsonSchema(schema)).toEqual(
      t(types.union([circle, square]), { discriminator: "type" }),
    );
  });
});
...
describe("enum", () => {
  test("enum members", () => {
    expect(fromJsonSchema({ type: "string", enum: ["a", "b", "c"] })).toEqual(
      t(types.enum(["a", "b", "c"])),
    );
  });
...
describe("nullable", () => {
  test("type array form", () => {
    expect(fromJsonSchema({ type: ["string", "null"] })).toEqual(
  ...
  test("anyOf wrapper form with multiple non-null variants stays a union", () => {
    ...
    expect(fromJsonSchema(schema)).toEqual(
      t(types.union([t(types.string), t(types.integer)]), { nullable: true }),
    );
  });
});
...
describe("metadata passthrough", () => {
  ...
  test("deprecated", () => {
    expect(fromJsonSchema({ type: "string", deprecated: true })).toEqual(
      t(types.string, { deprecated: true }),
    );
  });

  test("default", () => {
    expect(fromJsonSchema({ type: "integer", default: 0 })).toEqual(
  ...
  test("exclusiveMinimum / exclusiveMaximum", () => {
    ...
    expect(fromJsonSchema(schema)).toEqual(
      t(types.integer, { exclusiveMinimum: 0, exclusiveMaximum: 100 }),
    );
  });
  ...
  test("readOnly / writeOnly", () => {
    expect(fromJsonSchema({ type: "string", readOnly: true })).toEqual(
      t(types.string, { readOnly: true }),
    );
    expect(fromJsonSchema({ type: "string", writeOnly: true })).toEqual(
...
describe("unrecognized format fallback", () => {
  test("string format with no known constructor falls back to base type + meta.format", () => {
    expect(fromJsonSchema({ type: "string", format: "ipv4" })).toEqual(
