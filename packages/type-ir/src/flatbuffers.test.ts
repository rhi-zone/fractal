...
describe("map", () => {
  ...
  test("meta.entryName names the entry table", () => {
    expect(toFlatBuffers(t(types.map(t(types.string), int64()), { entryName: "Prop" }))).toBe(
      "[Prop]",
    );
  });
});
...
describe("union", () => {
  test("standalone falls back to meta.unionName or AnyUnion", () => {
    ...
    expect(toFlatBuffers(t(types.union([t(types.string), int32()]), { unionName: "Shape" }))).toBe(
      "Shape",
    );
  });
});
...
describe("toFlatBuffersTable", () => {
  ...
  test("null/void fields are skipped", () => {
    const ref = t(
      types.object({ name: t(types.string), nothing: t(types.void), empty: t(types.null) }),
    );
  ...
  test("array of nested objects hoists a sibling table, referenced as a vector", () => {
    const ref = t(
      types.object({ tags: t(types.array(t(types.object({ label: t(types.string) })))) }),
    );
  ...
  test("top-level tuple lowers to a positional table", () => {
    ...
    // scalar table field).
    expect(rendered).toBe(
      ["table Pair {", "  e0:string (required);", "  e1:int32;", "}"].join("\n"),
    );
  });
