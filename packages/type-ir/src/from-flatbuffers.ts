...
class ParseCursor {
  ...
  expectPunct(value: string): void {
    ...
    if (tok === undefined || tok.kind !== "punct" || tok.value !== value) {
      throw new Error(
        `fromFlatbuffers: expected "${value}", got ${tok === undefined ? "end of input" : JSON.stringify(tok)}`,
      );
    }
  }
  ...
  expectIdentValue(): string {
    ...
    if (tok === undefined || tok.kind !== "ident") {
      throw new Error(
        `fromFlatbuffers: expected identifier, got ${tok === undefined ? "end of input" : JSON.stringify(tok)}`,
      );
    }
    return tok.value;
  }
  ...
  parseField(): RawField {
    ...
    return defaultValue === undefined
      ? { name, typeExpr, attrs }
      : { name, typeExpr, defaultValue, attrs };
  }
...
function buildFieldTypeRef(
  ...
  for (const attr of field.attrs) {
    if (attr.name === "deprecated") meta.deprecated = true;
      ...
      meta.id = typeof attr.value === "number" ? attr.value : Number(attr.value);
...
function buildTableTypeRef(decl: RawTable, known: ReadonlySet<string>): TypeRef {
  ...
  for (const field of decl.fields)
    fields[field.name] = buildFieldTypeRef(field, decl.namespace, known, decl.isStruct);
...
export function fromFlatbuffers(schema: string): Record<string, TypeRef> {
  ...
  for (const decl of rawTables)
    result[qualify(decl.namespace, decl.name)] = buildTableTypeRef(decl, known);
  for (const decl of rawEnums) result[qualify(decl.namespace, decl.name)] = buildEnumTypeRef(decl);
  for (const decl of rawUnions)
    result[qualify(decl.namespace, decl.name)] = buildUnionTypeRef(decl, known);

  if (rootType !== undefined) {
    const resolved = resolveIdent(rootType.name, rootType.namespace, known);
    const existing = result[resolved];
    if (existing !== undefined)
      result[resolved] = { shape: existing.shape, meta: { ...existing.meta, isRootType: true } };
  }
