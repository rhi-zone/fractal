...
describe("collections", () => {
  ...
  test("map with string key -> NSDictionary<NSString *, V> *", () => {
    expect(toObjCType(t(types.map(t(types.string), t(types.number))))).toBe(
      "NSDictionary<NSString *, NSNumber *> *",
    );
  });

  test("nested array of array", () => {
    expect(toObjCType(t(types.array(t(types.array(t(types.integer))))))).toBe(
...
describe("enums", () => {
  ...
  test("NS_STRING_ENUM style", () => {
    ...
    expect(decl).toBe(
      [
        "typedef NSString * Status NS_STRING_ENUM;",
        "extern Status const StatusActive;",
        "extern Status const StatusInactive;",
      ].join("\n"),
    );
  });
...
describe("@interface / @property generation", () => {
  ...
  test("nested object field hoists a named nested class", () => {
    ...
    expect(header.indexOf("@interface PersonAddress")).toBeLessThan(
      header.indexOf("@interface Person :"),
    );
