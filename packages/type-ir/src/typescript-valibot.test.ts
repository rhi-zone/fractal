...
import { toValibot, toValibotDeclaration, toValibotDeclarations } from "./typescript-valibot.ts";
...
describe("formatted types", () => {
  ...
  test("email", () => {
    expect(toValibot(email())).toBe("v.pipe(v.string(), v.email())");
  });
});
...
describe("intersection", () => {
  test("v.intersect([...]) accepts any arity", () => {
    ...
    expect(toValibot(ref)).toBe(
      "v.intersect([v.object({ id: v.string() }), v.object({ createdAt: v.string() })])",
    );
  });
