...
describe("string format unification", () => {
  test("same format preserved", () => {
    const result = fromJsonCorpus(["2026-01-01", "2026-02-02", "2026-03-03"]);
  ...
  test("format + plain string -> string", () => {
    const result = fromJsonCorpus(["2026-01-01", "hello", "2026-02-02"]);
    expect(result.shape.kind).toBe("string");
  });
});
...
describe("object merging", () => {
  ...
  test("field present in some samples -> optional", () => {
    const result = fromJsonCorpus([{ id: 1, name: "a" }, { id: 2 }, { id: 3, name: "c" }]);
    expect(result.shape.kind).toBe("object");
    const fields = (result.shape as { fields: Record<string, TypeRef> }).fields;
    expect(fields.id!.meta.optional).toBeUndefined();
  ...
  test("field type varies across samples -> unified", () => {
    const result = fromJsonCorpus([{ value: 1 }, { value: "hello" }, { value: 2 }]);
    expect(result.shape.kind).toBe("object");
    const fields = (result.shape as { fields: Record<string, TypeRef> }).fields;
    // uint8 + string -> union
...
describe("CFD-style discriminant search (non-enum-typed fields)", () => {
  ...
  test("tag never gets typed enum on its own (ambiguous K/N band for a string field)", () => {
    const plain = fromJsonCorpus(values, {
      ...cfdOnly,
      detectDiscriminatedUnions: false,
      detectCfdDiscriminants: false,
      splitDissimilarObjects: false,
    });
  ...
  test("no false split when only the COMBINATION of two fields determines structure (unary restriction holds)", () => {
    ...
    const result = fromJsonCorpus(composite, {
      detectCfdDiscriminants: true,
      splitDissimilarObjects: false,
    });
    expect(result.meta.discriminator).toBeUndefined();
  });
...
describe("structural union splitting (no discriminant field)", () => {
  test("root-level objects with dissimilar field sets and no discriminant split into variants", () => {
    ...
    for (const v of variants) {
      ...
      expect(keys).toSatisfy(
        (k: string[]) =>
          k.join(",") === "userEmail,userId,userName" || k.join(",") === "items,orderId,total",
      );
    }
  });
  ...
  test("sparse records (some samples with zero fields) do NOT split", () => {
    const result = fromJsonCorpus([{}, {}, { tag: "a" }, { tag: "b" }, { tag: "c" }, { tag: "d" }]);
    expect(result.shape.kind).toBe("object");
    const fields = (result.shape as { fields: Record<string, TypeRef> }).fields;
...
describe("clustering methods", () => {
  ...
  test("single-linkage chains A-B-C into one cluster (no split) despite A and C being fully disjoint", () => {
    const result = fromJsonCorpus(chainingValues, {
      objectSplitThreshold: 0.8,
      objectSplitMinSamples: 5,
      clusteringMethod: "single-linkage",
    });
    expect(result.shape.kind).toBe("object");
  });
  ...
  test("complete-linkage refuses the chained merge, splitting into two clusters", () => {
    const result = fromJsonCorpus(chainingValues, {
      objectSplitThreshold: 0.8,
      objectSplitMinSamples: 5,
      clusteringMethod: "complete-linkage",
    });
    expect(result.shape.kind).toBe("union");
    const variants = (result.shape as { variants: readonly TypeRef[] }).variants;
    expect(variants).toHaveLength(2);
  ...
  test("key-signature splits on exact key-set signature, ignoring objectSplitThreshold entirely", () => {
    ...
    const result = fromJsonCorpus(chainingValues, {
      objectSplitThreshold: 0.99,
      objectSplitMinSamples: 5,
      clusteringMethod: "key-signature",
    });
    expect(result.shape.kind).toBe("union");
    const variants = (result.shape as { variants: readonly TypeRef[] }).variants;
  ...
  test("default clusteringMethod is single-linkage (back-compat: unspecified behaves as before)", () => {
    const withDefault = fromJsonCorpus(chainingValues, {
      objectSplitThreshold: 0.8,
      objectSplitMinSamples: 5,
    });
...
describe("property: no crashes on any JSON corpus", () => {
  test("arbitrary JSON values", () => {
    fc.assert(
      fc.property(fc.array(fc.jsonValue(), { minLength: 1, maxLength: 20 }), (values) => {
...
describe("property: determinism", () => {
  test("same corpus -> same result", () => {
    fc.assert(
      fc.property(fc.array(fc.jsonValue(), { minLength: 1, maxLength: 10 }), (values) => {
      ...
      fc.property(
        fc.array(
          fc.oneof(
            ...
            fc
              .double({ min: -100, max: 100, noDefaultInfinity: true, noNaN: true })
    ...
    fc.assert(
      fc.property(
        fc.tuple(
          ...
          fc.shuffledSubarray(["active", "inactive", "pending", "archived", "deleted"], {
            minLength: 2,
            maxLength: 5,
          }),
          // Number of samples
...
describe("enumMaxMembers", () => {
  ...
  test("the cap does not override the K>=N unique-values rejection", () => {
    ...
    expect(JSON.stringify(fromJsonCorpus(allUnique, { enumMaxMembers: Infinity }))).not.toContain(
      '"enum"',
    );
  });
});
...
describe("stage pipeline", () => {
  ...
  test("a coverage-based generalization is expressible over the same evidence", () => {
    ...
    expect(JSON.stringify(fromJsonCorpus(tight, { stages: [coverageEnum(0.9)] }))).toContain(
      '"enum"',
    );
    // 40 all-distinct values: coverage 0, under the bar
    const open = Array.from({ length: 40 }, (_, i) => ({ s: `v${i}` }));
    expect(JSON.stringify(fromJsonCorpus(open, { stages: [coverageEnum(0.9)] }))).not.toContain(
      '"enum"',
    );
  });
...
describe("grouping rules", () => {
  ...
  test("veto a split the built-ins do make, by path", () => {
    ...
    const out = fromJsonCorpus(rows, {
      duGrouping: noRootSplit,
      cfdGrouping: () => undefined,
      splitGrouping: () => undefined,
    });
  ...
  test("each of the four points is independently replaceable", () => {
    ...
    fromJsonCorpus(rows, {
      duGrouping: (c) => {
        seen.push("du");
        return defaultDetectDU(c);
      },
      dictGrouping: (c) => {
        seen.push("dict");
        return defaultDetectDict(c);
      },
      cfdGrouping: () => {
        seen.push("cfd");
        return undefined;
  ...
  test("grouping rules see the raw samples, not just merged evidence", () => {
    ...
    fromJsonCorpus(rows, {
      duGrouping: (c) => {
        if (c.path.length === 0) sampleCount = c.samples.length;
        return undefined;
      },
    });
