...
// — there is no sonic-specific tag key and, unlike easyjson (go-easyjson.ts),
// no build-time code generator scanning for a directive comment. Sonic's
// speed instead comes from JIT-compiling (via golang-asm) an encoder/decoder
// for each concrete type the first time it's used, optionally warmed ahead
// of time with `sonic.Pretouch(reflect.TypeOf(T{}))` to avoid first-hit JIT
// latency on huge/recursive schemas — a runtime call, not something this
// type-declaration projector emits. The struct/slice/map/const-enum/
// marker-interface-union shape below is therefore identical to
// go-encoding-json.ts's and go-jsoniter.ts's; this file exists as its own
// module so callers can name the sonic variant explicitly, same convention
// go-jsoniter.ts documents for itself.
...
// Same primitive vocabulary as go-encoding-json.ts — sonic decodes into the
// identical Go types encoding/json does, dispatched via `resolve` so
...
const primitiveHandlers: Record<string, Converter> = {
  ...
  uri: leaf("string"),
  email: leaf("string"),
  // sonic decodes RFC 3339 timestamps into time.Time the same way
  // encoding/json does — both datetime and date (type-ir's `Date` domain
  // type, see kinds/date-time.ts) map here, matching go-encoding-json.ts.
  // The caller assembling the emitted source is responsible for
  // `import "time"`, same convention `instance`/`page` use elsewhere in
  // this file for their own imports.
...
// interface, func value) — sonic's `omitempty` handling reads the same
// struct tag encoding/json does, so a nil value of these is already omitted
// without an extra pointer wrapper. Value-kind fields (struct, string-backed
// enum, tuple struct, primitives) have no such nil zero value, so `*T` is
// the only way to distinguish "absent" from "present, zero value" for those.
...
// go-encoding-json.ts's docComment — sonic doesn't change doc-comment
// conventions.
...
// value itself (not an arbitrary ordinal) is what sonic's reflective/JIT
...
// marker interface with an unexported method every variant implements, same
// as go-encoding-json.ts — sonic's JIT-compiled decoder walks a
// polymorphic-interface field via reflection exactly like encoding/json does
// (it has no separate dispatch mechanism of its own), so there's no
// sonic-specific union encoding to diverge into the way go-easyjson.ts's
// json.RawMessage approach does for its generator.
...
/**
 * Lower a TypeRef to idiomatic Go/sonic source: one or more top-level
 * declarations (structs, string-backed enums, marker-interface unions, …),
 * separated by blank lines, in the order they were first referenced. `name`
 * seeds the root declaration's identifier (defaults to "Root"); if the root
 * itself isn't a kind that hoists its own named declaration (e.g. a bare
 * primitive, slice, or map), an explicit `type Name = <expr>` alias is
 * appended so `name` still resolves to something declared, mirroring
 * go-encoding-json.ts's `toGo`. Every declaration here decodes identically
 * under `encoding/json`, jsoniter, and sonic — the caller picks
 * `sonic.Marshal`/`sonic.Unmarshal` (optionally warmed via
 * `sonic.Pretouch`) over the standard library's equivalents.
 */
export function toSonic(ref: TypeRef, name = "Root"): string {
  const ctx: Ctx = { decls: [] }
