...
describe("objects", () => {
  ...
  test("empty object emits defstruct [] and an empty typespec map, no @enforce_keys", () => {
    ...
    expect(out).toBe(
      ["defmodule Empty do", "  @derive Jason.Encoder", "  defstruct []", "", "  @type t :: %__MODULE__{}", "end", ""].join(
        "\n",
      ),
    )
...
describe("unions", () => {
  ...
  test("union of duplicate-rendering variants collapses to a single type", () => {
    ...
    expect(toElixir(ref, "Name")).toBe(["defmodule Name do", "  @type t :: String.t()", "end", ""].join("\n"))
  })
...
test("unknown kind fallback maps to any()", () => {
  ...
  expect(toElixir(ref, "Mystery")).toBe(["defmodule Mystery do", "  @type t :: any()", "end", ""].join("\n"))
})
