...
function buildMssqlChecks(kind: string, meta: Readonly<Record<string, unknown>>): string[] {
  ...
  if (typeof meta.minLength === "number" && stringLike)
    checks.push(`CHECK (LEN({name}) >= ${meta.minLength})`);
  if (typeof meta.maxLength === "number" && stringLike)
    checks.push(`CHECK (LEN({name}) <= ${meta.maxLength})`);
...
export function singleTableInheritanceMssqlLayout(opts?: {
...
export function tablePerVariantMssqlLayout(opts?: {
    ...
    const tables = variants.map(({ name: variantName, ref }) => {
      ...
      if (isA(ref.shape.kind, "object")) {
        ...
        const columns = Object.entries(s.fields).map(([fieldName, fieldRef]) =>
          renderMssqlColumnDef(fieldName, toColumn(fieldRef)),
        );
 ...
 */
export function toMssqlCreateTableFromRef(
  tableName: string,
  ref: TypeRef,
  opts?: MssqlTableOptions,
): string {
  if (isA(ref.shape.kind, "union")) {
    ...
    const variants = s.variants.map((variant, i) => ({
      name: mssqlVariantName(discriminator, variant, i + 1),
      ref: variant,
    }));
  ...
  if (!isA(ref.shape.kind, "object"))
    throw new Error(
      `toMssqlCreateTableFromRef requires an object or union type, got "${ref.shape.kind}"`,
    );
  const s = ref.shape as TypeShape & { kind: "object" };
