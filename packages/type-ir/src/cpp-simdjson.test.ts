...
describe("primitives", () => {
  ...
  test("number maps to double", () => {
    expect(toSimdjson(t(types.number), "Amount")).toBe(
      ["", "using Amount = double;", ""].join("\n"),
    );
  });
  ...
  test("int32 kind maps to int32_t", () => {
    expect(toSimdjson(int32(), "Count")).toBe(
      ["#include <cstdint>", "", "using Count = int32_t;", ""].join("\n"),
    );
  });
...
describe("struct", () => {
  ...
  test("map field deserializes via a get_object() key_value_pair loop", () => {
    ...
    const out = toSimdjson(ref, "Doc");
    expect(out).toContain(
      'for (simdjson::dom::key_value_pair field : element["scores"].get_object())',
    );
    expect(out).toContain(
      "result.emplace(std::string(field.key), field.value.get_double().value());",
    );
  });
});
...
test("tuple maps to std::tuple<...>", () => {
  ...
  expect(toSimdjson(ref, "Pair")).toBe(
    [
      ...
      "#include <tuple>",
      "",
      "using Pair = std::tuple<std::string, double>;",
      "",
    ].join("\n"),
  );
});
...
describe("enum class", () => {
  test("emits enum class plus fromString/fromJson, no toString/toJson", () => {
    ...
    expect(out).toContain(
      "return statusFromString(std::string_view(element.get_string().value()));",
    );
