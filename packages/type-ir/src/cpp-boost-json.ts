...
function emitEnumDecl(
  ...
  lines.push("");
  lines.push(
    `inline void tag_invoke(const boost::json::value_from_tag&, boost::json::value& jv, ${name} value) {`,
  );
...
function emitStructDecl(
  ...
  const toEntries = fieldNames
    .map((fieldName) => `{${quote(fieldName)}, value.${fieldName}}`)
    .join(", ");
  ...
  lines.push("");
  lines.push(
    `inline void tag_invoke(const boost::json::value_from_tag&, boost::json::value& jv, const ${name}& value) {`,
  );
...
export function toBoostJson(ref: TypeRef, name = "Root"): string {
  ...
  const isNamedDecl =
    ref.shape.kind === "object" || ref.shape.kind === "enum" || ref.shape.kind === "interface";
  const typeExpr = bjType(ref, ctx, name);
  const body = isNamedDecl
    ? ctx.decls.join("\n\n")
