...
describe("primitives", () => {
  ...
  test("number maps to double", () => {
    expect(toRapidjson(t(types.number), "Amount")).toBe(
  ...
  test("int32 kind maps to int32_t", () => {
    expect(toRapidjson(int32(), "Count")).toBe(
...
describe("struct", () => {
  test("emits a struct with hand-generated toJson/fromJson", () => {
    ...
    expect(out).toContain(
      'result.id = std::string(v["id"].GetString(), v["id"].GetStringLength());',
    );
    expect(out).toContain(
  ...
  test("map field serializes via AddMember loop, deserializes via MemberBegin/MemberEnd", () => {
    ...
    expect(out).toContain(
      'for (auto it = v["scores"].MemberBegin(); it != v["scores"].MemberEnd(); ++it)',
    );
  });
});
...
describe("enum class", () => {
  test("emits enum class plus toString/fromString/toJson/fromJson", () => {
    ...
    expect(out).toContain(
      "return statusFromString(std::string(v.GetString(), v.GetStringLength()));",
    );
  });
});
