...
function emitEnumDecl(
  ...
  lines.push(
    `  return ${lowerFirst(name)}FromString(std::string(v.GetString(), v.GetStringLength()));`,
  );
  lines.push(`}`);
  ...
  switch (shape.kind) {
    ...
    case "tuple": {
      ...
      const pushes = s.elements
        .map(
          (e, i) =>
            `arr.PushBack(${rjToJson(e, `std::get<${i}>(${expr})`, ctx, hint)}, allocator);`,
        )
        .join(" ");
  ...
  switch (shape.kind) {
    ...
    case "union": {
      ...
      const checks = s.variants
        .map(
          (v) =>
            `if (${rjIsCheck(v, expr)}) return ${variantType}{${rjFromJson(v, expr, ctx, hint)}};`,
        )
        .join(" ");
