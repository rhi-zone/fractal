...
describe("formats", () => {
  ...
  test("unrecognized format falls back to base type + meta.format", () => {
    expect(fromOpenApi30({ type: "string", format: "ipv4" })).toEqual(
      t(types.string, { format: "ipv4" }),
    );
    expect(fromOpenApi30({ type: "string", format: "binary" })).toEqual(
      t(types.string, { format: "binary" }),
    );
    expect(fromOpenApi30({ type: "number", format: "decimal" })).toEqual(
      t(types.number, { format: "decimal" }),
    );
    expect(fromOpenApi30({ type: "integer", format: "unsigned" })).toEqual(
      t(types.integer, { format: "unsigned" }),
    );
  });
});
...
describe("object", () => {
  ...
  test("no required key -> all fields optional", () => {
    ...
    expect(fromOpenApi30(schema)).toEqual(
      t(types.object({ nickname: t(types.string, { optional: true }) })),
    );
  });
  ...
  test("readOnly property -> meta.readonly alongside verbatim meta.readOnly", () => {
    ...
    const fields = (
      result.shape as { kind: "object"; fields: Record<string, ReturnType<typeof t>> }
    ).fields;
...
describe("array", () => {
  test("element type", () => {
    expect(fromOpenApi30({ type: "array", items: { type: "integer" } })).toEqual(
...
describe("allOf -> intersection", () => {
  ...
  test("2.0 (the only polymorphism keyword Swagger 2.0 has)", () => {
    ...
    expect(fromOpenApi20(schema)).toEqual(
      t(types.intersection([t(types.ref("Pet")), t(types.object({ name: t(types.string) }))])),
    );
  });
});
...
describe("union: oneOf / anyOf (3.0)", () => {
  ...
  test("oneOf + discriminator.propertyName -> meta.discriminator", () => {
    ...
    const dog = t(
      types.object({
        petType: t(types.literal("Dog")),
        bark: t(types.boolean, { optional: true }),
      }),
    );
    const cat = t(
      types.object({
        petType: t(types.literal("Cat")),
        meow: t(types.boolean, { optional: true }),
      }),
    );
  ...
  test("oneOf + discriminator.mapping -> meta.discriminatorMapping (preserved, no forward consumer yet)", () => {
    ...
    expect(result.meta.discriminatorMapping).toEqual({
...
describe("literal / enum", () => {
  ...
  test("enum members", () => {
    expect(fromOpenApi30({ type: "string", enum: ["a", "b", "c"] })).toEqual(
      t(types.enum(["a", "b", "c"])),
    );
    expect(fromOpenApi20({ type: "string", enum: ["a", "b", "c"] })).toEqual(
      t(types.enum(["a", "b", "c"])),
    );
  });
});
...
describe("nullable", () => {
  test("3.0 nullable: true", () => {
    expect(fromOpenApi30({ type: "string", nullable: true })).toEqual(
  ...
  test("3.0 nullable on a composite (object) schema", () => {
    ...
    expect(fromOpenApi30(schema)).toEqual(
      t(types.object({ id: t(types.string) }), { nullable: true }),
    );
  });

  test("2.0 x-nullable: true", () => {
    expect(fromOpenApi20({ type: "string", "x-nullable": true })).toEqual(
      t(types.string, { nullable: true }),
    );
  });
...
describe("metadata passthrough", () => {
  test("description / default / example", () => {
    expect(fromOpenApi30({ type: "string", description: "a name" })).toEqual(
      t(types.string, { description: "a name" }),
    );
    expect(fromOpenApi30({ type: "integer", default: 0 })).toEqual(
      t(types.integer, { default: 0 }),
    );
    expect(fromOpenApi30({ type: "string", example: "Fido" })).toEqual(
      t(types.string, { example: "Fido" }),
    );
  });

  test("3.0 native deprecated", () => {
    expect(fromOpenApi30({ type: "string", deprecated: true })).toEqual(
      t(types.string, { deprecated: true }),
    );
  });

  test("2.0 x-deprecated -> canonical meta.deprecated", () => {
    expect(fromOpenApi20({ type: "string", "x-deprecated": true })).toEqual(
      t(types.string, { deprecated: true }),
    );
  });
  ...
  test("numeric / string constraints", () => {
    ...
    expect(fromOpenApi30(numSchema)).toEqual(
      t(types.integer, { minimum: 0, maximum: 100, multipleOf: 5 }),
    );
  });

  test("readOnly / writeOnly", () => {
    expect(fromOpenApi30({ type: "string", readOnly: true })).toEqual(
      t(types.string, { readOnly: true }),
    );
    expect(fromOpenApi30({ type: "string", writeOnly: true })).toEqual(
      t(types.string, { writeOnly: true }),
    );
  });

  test("unrecognized x-* vendor extension travels verbatim into meta", () => {
    expect(fromOpenApi30({ type: "string", "x-nullable-legacy": true })).toEqual(
      t(types.string, { "x-nullable-legacy": true }),
    );
    expect(fromOpenApi20({ type: "string", "x-internal": "yes" })).toEqual(
      t(types.string, { "x-internal": "yes" }),
    );
  });
});
...
describe("nested schemas", () => {
  ...
  test("nested nullable inside object property (3.0)", () => {
    ...
    expect(fromOpenApi30(schema)).toEqual(
      t(types.object({ nickname: t(types.string, { nullable: true }) })),
    );
  });
...
describe("round-trip: fromOpenApi20(toOpenApi20(ref))", () => {
  ...
  test("intersection / literal / enum / ref", () => {
    ...
    expect(fromOpenApi20(toOpenApi20(t(types.literal("active"))))).toEqual(
      t(types.enum(["active"])),
    );
