...
const exprComplexKinds = new Set([
  "union",
  "object",
  "map",
  "intersection",
...
const exprHandlers: Record<string, ExprConverter> = {
  ...
  array: (shape) => {
    ...
    return exprComplexKinds.has(s.element.shape.kind)
      ? `Array<${renderTypeExpr(s.element)}>`
      : `${renderTypeExpr(s.element)}[]`;
  },
  tuple: (shape) => {
  ...
  page: (shape) => {
    ...
    return s.style === "offset"
      ? `OffsetPage<${renderTypeExpr(s.element)}>`
      : `CursorPage<${renderTypeExpr(s.element)}>`;
  },
  map: (shape) => {
    const s = shape as TypeShape & { kind: "map" };
    return s.key.shape.kind === "string"
      ? `Record<string, ${renderTypeExpr(s.value)}>`
      : `Map<${renderTypeExpr(s.key)}, ${renderTypeExpr(s.value)}>`;
  },
  union: (shape) => {
...
function renderFieldsTable(
  ...
  const rows = Object.entries(shape.fields).map(([name, field]) => {
    ...
    if (field.meta.default !== undefined)
      descriptionParts.push(`Default: \`${JSON.stringify(field.meta.default)}\``);
    if (field.meta.deprecated === true || typeof field.meta.deprecated === "string") {
      const reason =
        typeof field.meta.deprecated === "string" ? field.meta.deprecated : "deprecated";
...
function renderMethodsTable(
  shape: TypeShape & { kind: "interface" },
  doc: TypeRefDocument,
  basePath: string,
): string {
...
function renderVariantsSection(
  ...
  for (const variant of shape.variants) {
    if (variant.shape.kind === "ref") {
      ...
      const description =
        targetRef === undefined ? "Unresolved reference." : typeSummary(targetRef);
      parts.push(
        `<LinkCard title=${quote(title)} description=${quote(description)} href=${quote(linkHref(basePath, target))} />`,
      );
    } else {
...
function renderExamplesSection(meta: Readonly<Record<string, unknown>>): string | undefined {
  const examples: unknown[] = Array.isArray(meta.examples)
    ? meta.examples
  ...
  return examples
    .map((example) => `<Code code={${quote(JSON.stringify(example, null, 2))}} lang="json" />`)
    .join("\n\n");
}
...
function renderShapeSection(
  ...
  const kind = resolve(
    shape.kind,
    Object.fromEntries((["object", "interface", "enum", "union"] as const).map((k) => [k, k])),
  );
  switch (kind) {
    case "object":
...
function renderPage(name: string, ref: TypeRef, doc: TypeRefDocument, basePath: string): string {
  ...
  const description =
    typeof ref.meta.description === "string" ? ref.meta.description : `Reference for ${title}.`;
  ...
  if (ref.meta.deprecated === true || typeof ref.meta.deprecated === "string") {
    const reason =
      typeof ref.meta.deprecated === "string" ? ref.meta.deprecated : "This type is deprecated.";
 ...
 */
export function toStarlightReference(
  doc: TypeRefDocument,
