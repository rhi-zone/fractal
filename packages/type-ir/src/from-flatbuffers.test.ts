...
describe("enums", () => {
  test("sequential values with no explicit numbers", () => {
    ...
    expect((color.shape as unknown as { members: string[] }).members).toEqual([
      "Red",
      "Green",
      "Blue",
    ]);
...
describe("vectors", () => {
  ...
  test("vector of a referenced table type", () => {
    ...
    `);
    const fields = (result.Inventory!.shape as unknown as { fields: Record<string, TypeRef> })
      .fields;
...
describe("namespaces", () => {
  ...
  test("a same-namespace unqualified reference resolves correctly", () => {
    const result = fromFlatbuffers(`
      ...
      table Person { address:Address; }
    `);
    const fields = (result["a.b.Person"]!.shape as unknown as { fields: Record<string, TypeRef> })
      .fields;
...
describe("round-trip via toFlatBuffersTable/toFlatBuffersDeclarations", () => {
  ...
  test("an enum TypeRef survives toFlatBuffersDeclarations -> fromFlatbuffers", () => {
    ...
    expect((result.Color!.shape as unknown as { members: string[] }).members).toEqual([
      "Red",
      "Green",
      "Blue",
    ]);
