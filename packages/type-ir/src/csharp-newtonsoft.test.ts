...
describe("enum", () => {
  ...
  test("description and deprecated render doc comment + [Obsolete] above the enum", () => {
    const ref = t(types.enum(["small", "large"]), {
      description: "A size tier.",
      deprecated: true,
    });
    const out = toCSharpNewtonsoft(ref, "Size");
    expect(out).toContain(
...
describe("discriminated unions", () => {
  ...
  test("plain union (no discriminator) tries each variant in order", () => {
    ...
    expect(out).toContain(
      "try { return jObject.ToObject<TextValue>(serializer); } catch (JsonException) { }",
    );
    expect(out).toContain(
      "try { return jObject.ToObject<NumberValue>(serializer); } catch (JsonException) { }",
    );
  });
});
