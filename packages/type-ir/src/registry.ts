...
import {
  toEffectSchemaDeclaration,
  toEffectSchemaDeclarations,
} from "./typescript-effect-schema.ts";
...
import {
  toKotlin as toKotlinJackson,
  toKotlinDeclarations as toKotlinJacksonDeclarations,
} from "./kotlin-jackson.ts";
...
const json = (
  ...
  extensions,
  input: "json",
  run: (source) => run(JSON.parse(source)),
});
  ...
  return typeRefDocument(
    fromStandardSchema({
      "~standard": {
        ...
        validate: (value: unknown) => ({ value }),
        types:
          envelope.sample === undefined
            ? undefined
            : { input: envelope.sample, output: envelope.sample },
      },
    } as Parameters<typeof fromStandardSchema>[0]),
  );
}
...
const importerList: readonly Importer[] = [
  ...
  json("jtd", "./from-jtd", [".jtd.json"], (v) =>
    fromJtdDocument(v as Parameters<typeof fromJtdDocument>[0]),
  ),
...
function viaTwoStage<D>(
  build: (name: string, ref: TypeRef) => D,
  render: (decls: D[]) => string,
): Render {
  return (doc, options) => {
...
const projectorList: readonly Projector[] = [
  ...
  src(
    ...
    "./typescript-native",
    ".ts",
    viaNameFirst(toTypeDeclaration, toTypeDeclarations),
  ),
  src(
    "typescript-zod",
    "./typescript-zod",
    ".ts",
    viaNameFirst(toZodDeclaration, toZodDeclarations),
  ),
  src(
    "typescript-typebox",
    "./typescript-typebox",
    ".ts",
    viaNameFirst(toTypeBoxDeclaration, toTypeBoxDeclarations),
  ),
  src(
    "typescript-io-ts",
    "./typescript-io-ts",
    ".ts",
    viaNameFirst(toIoTsDeclaration, toIoTsDeclarations),
  ),
  src(
    "typescript-yup",
    "./typescript-yup",
    ".ts",
    viaNameFirst(toYupDeclaration, toYupDeclarations),
  ),
  src(
  ...
  src(
    ...
    "./typescript-valibot",
    ".ts",
    viaNameFirst(toValibotDeclaration, toValibotDeclarations),
  ),
  src(
    "typescript-runtypes",
    "./typescript-runtypes",
    ".ts",
    viaNameFirst(toRuntypesDeclaration, toRuntypesDeclarations),
  ),
  src(
  ...
  src(
    ...
    "./typescript-arktype",
    ".ts",
    viaNameFirst(toArkTypeDeclaration, toArkTypeDeclarations),
  ),
  src(
    "jsdoc",
    "./jsdoc",
  ...
  src(
    ...
    "./java-jackson",
    ".java",
    viaNameFirstOnly((name, ref) => toJavaDeclaration(name, ref)),
  ),
  src(
    "java-gson",
    "./java-gson",
    ".java",
    viaNameFirstOnly((name, ref) => toGsonDeclaration(name, ref)),
  ),
  src(
    "java-moshi",
    "./java-moshi",
    ".java",
    viaNameFirstOnly((name, ref) => toMoshiDeclaration(name, ref)),
  ),
  src(
    "java-jsonb",
    "./java-jsonb",
    ".java",
    viaNameFirstOnly((name, ref) => toJsonbDeclaration(name, ref)),
  ),
  src(
    "kotlin-kotlinx",
    "./kotlin-kotlinx",
  ...
  src(
    ...
    ".kt",
    viaNameFirst(
      (name, ref) => toKotlinGson(ref, name),
      (registry) => toKotlinGsonDeclarations(registry),
    ),
  ),
  ...
  src(
    ...
    "./ruby-rbs",
    ".rbs",
    viaRefFirst((ref, name) => toRbsFile(ref, name ?? DEFAULT_ROOT_NAME)),
  ),
  ...
  src(
    ...
    "./sql",
    ".sql",
    viaNameFirstOnly((name, ref) => toCreateTable(toSnakeCase(name), ref)),
  ),
  src(
    "sql-mssql",
    "./sql-mssql",
    ".sql",
    viaNameFirstOnly((name, ref) => toMssqlCreateTableFromRef(toSnakeCase(name), ref)),
  ),
  src(
    "protobuf",
    "./protobuf",
    ".proto",
    viaTwoStage(toProtoMessage, (msgs) => renderProto(msgs)),
  ),
  src(
    "capnp",
    "./capnp",
    ".capnp",
    viaTwoStage(toCapnpStruct, (structs) => renderCapnp(structs)),
  ),
  src(
    "flatbuffers",
    "./flatbuffers",
    ".fbs",
    viaNameFirst(toFlatBuffersTable, toFlatBuffersDeclarations),
  ),
  ...
  files("mkdocs-vanilla-reference", "./mkdocs-vanilla-reference", (doc) =>
...
export const projectorRegistry: Registry<Projector> = defineRegistry(
  projectorList,
  projectorAliases,
);
