...
const arbEnum: fc.Arbitrary<TypeRef> = fc
  .array(fc.stringMatching(/^[a-z][a-z0-9_]{1,10}$/), { minLength: 2, maxLength: 6 })
  .map((members) => {
...
const arbTypeRef: fc.Memo<TypeRef> = fc.memo((depth) => {
  ...
  const arbObject: fc.Arbitrary<TypeRef> = fc
    .tuple(
  ...
  const arbTuple: fc.Arbitrary<TypeRef> = fc
    .array(inner, { minLength: 1, maxLength: 4 })
    .map((els) => t(types.tuple(els)));

  const arbUnion: fc.Arbitrary<TypeRef> = fc
    .array(inner, { minLength: 2, maxLength: 3 })
...
function arbValueForType(ref: TypeRef): fc.Arbitrary<unknown> {
  ...
  switch (shape.kind) {
    ...
    case "number":
      ...
      return fc.double({ min: -1000, max: 1000, noDefaultInfinity: true, noNaN: true }).map((n) => {
    ...
    case "int64":
      return fc.integer({ min: Number.MIN_SAFE_INTEGER, max: Number.MAX_SAFE_INTEGER });
    ...
    case "object": {
      ...
      return fc.record(arbs).chain((rec) => {
        ...
        return fc
          .tuple(
            ...Object.keys(rec).map((k) => (optionalFlags[k] ? fc.boolean() : fc.constant(true))),
          )
          .map((includes) => {
    ...
    case "map": {
      ...
      return fc
        .array(fc.tuple(fc.stringMatching(/^[a-z]{2,6}$/), arbValueForType(value)), {
          minLength: 2,
          maxLength: 5,
        })
        .map((pairs) => {
    ...
    case "union": {
      const variants = (shape as { kind: "union"; variants: readonly TypeRef[] }).variants.filter(
        (v) => v.shape.kind !== "never",
      );
...
function checkIntegerTightness(value: number, inferred: TypeRef): string | null {
  ...
  if (Number.isSafeInteger(value)) {
    ...
    return kind === expected ? null : `value ${value} should infer as ${expected} but got ${kind}`;
  }
...
function arbTypeAndValue(): fc.Arbitrary<{ type: TypeRef; value: unknown }> {
  return typeRefArb
    ...
    .chain((type) => arbValueForType(type).map((value) => ({ type, value })));
}
    ...
    fc.assert(
      fc.property(
        fc.oneof(
          ...
          fc.boolean(),
          fc
            .double({ min: -1000, max: 1000, noDefaultInfinity: true, noNaN: true })
