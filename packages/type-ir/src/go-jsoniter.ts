...
// it reads the exact same `json:"..."` struct tags via reflection at
// runtime (no build-time code generation, no directive comment, no
// jsoniter-specific tag key), so the struct/slice/map/const-enum/
// marker-interface-union shape this file emits is byte-for-byte identical
// to go-encoding-json.ts's. The only thing that actually changes at the
// call site is which package the caller imports for Marshal/Unmarshal —
// `var json = jsoniter.ConfigCompatibleWithStandardLibrary` in place of
// `encoding/json`'s package-level functions — which is outside what a type
// *declaration* projector emits. This file exists as its own module (rather
// than an alias for go-encoding-json.ts) so callers can name the variant
// they're targeting explicitly, matching go-easyjson.ts's/go-sonic.ts's
...
const primitiveHandlers: Record<string, Converter> = {
  ...
  uri: leaf("string"),
  email: leaf("string"),
  // jsoniter's ConfigCompatibleWithStandardLibrary decodes RFC 3339
  // timestamps into time.Time the same way encoding/json does — both
  // datetime and date (type-ir's `Date` domain type, see
  // kinds/date-time.ts) map here, matching go-encoding-json.ts. The caller
  // assembling the emitted source is responsible for `import "time"`, same
  // convention `instance`/`page` use elsewhere in this file for their own
  // imports.
...
// interface, func value) — jsoniter's `omitempty` handling reads the same
...
// value itself (not an arbitrary ordinal) is what jsoniter's reflective
...
// as go-encoding-json.ts — jsoniter's reflective decoder walks a
// polymorphic-interface field exactly like encoding/json does (it has no
// separate dispatch mechanism of its own), so there's no jsoniter-specific
// union encoding to diverge into the way go-easyjson.ts's json.RawMessage
...
/**
 * Lower a TypeRef to idiomatic Go/jsoniter source: one or more top-level
 ...
 * under both `encoding/json` and `jsoniter.ConfigCompatibleWithStandardLibrary`
 * — the caller picks which package's Marshal/Unmarshal to call against it.
 */
