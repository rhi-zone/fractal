...
function issuePaths(issues: readonly StandardSchemaV1.Issue[]): string[] {
  return issues.map((i) =>
    (i.path ?? []).map((seg) => (typeof seg === "object" ? seg.key : seg)).join("."),
  );
}
...
describe("objects", () => {
  ...
  test("valid object with optional property present passes", () => {
    const schema = toStandardSchema(person);
    expect(schema["~standard"].validate({ name: "Ada", age: 30 })).toEqual({
  ...
  test("additionalProperties: false rejects unexpected properties", () => {
    const strict = t(types.object({ name: t(types.string) }), { additionalProperties: false });
  ...
  test("additionalProperties unset (default) allows unexpected properties", () => {
    const schema = toStandardSchema(person);
    expect(schema["~standard"].validate({ name: "Ada", extra: 1 })).toEqual({
      value: { name: "Ada", extra: 1 },
    });
  });
});
...
describe("jsonSchema export", () => {
  ...
  test("draft-2020-12 target returns a valid-looking JSON Schema", () => {
    const schema = toStandardSchema(person);
    const result = schema["~standard"].jsonSchema.input({ target: "draft-2020-12" }) as Record<
      string,
      unknown
    >;
  ...
  test("draft-07 target returns a valid-looking JSON Schema", () => {
    const schema = toStandardSchema(person);
    const result = schema["~standard"].jsonSchema.input({ target: "draft-07" }) as Record<
      string,
      unknown
    >;
    expect(result.type).toBe("object");
  ...
  test("openapi-3.0 target returns an OpenAPI schema object", () => {
    const schema = toStandardSchema(person);
    const result = schema["~standard"].jsonSchema.input({ target: "openapi-3.0" }) as Record<
      string,
      unknown
    >;
    expect(result.type).toBe("object");
