...
describe("shared: enum", () => {
  ...
  test("enum of integer members", () => {
    ...
    expect(toJsonSchema07(ref)).toEqual({ type: "integer", enum: [0, 1, 2] });
  });
});
...
describe("shared: metadata passthrough", () => {
  ...
  test("readOnly / writeOnly", () => {
    expect(toJsonSchema07(t(types.string, { readOnly: true }))).toEqual({
      type: "string",
      readOnly: true,
    });
    expect(toJsonSchema07(t(types.string, { writeOnly: true }))).toEqual({
      type: "string",
      writeOnly: true,
    });
  });
