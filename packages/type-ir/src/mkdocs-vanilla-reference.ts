...
function examplesSection(meta: Readonly<Record<string, unknown>>): string | undefined {
  ...
  const sections = examples.map((example, i) => [`### Example ${i + 1}`, "", exampleBlock(example)].join("\n"))
 ...
 */
export function toMkdocsVanillaReference(doc: TypeRefDocument, options?: { basePath?: string }): Map<string, string> {
