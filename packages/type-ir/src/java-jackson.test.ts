...
describe("primitives — bare type expressions", () => {
  ...
  test("optional integer with optionalStyle: optional wraps in Optional<Integer>", () => {
    expect(
      toJava(withMeta(t(types.integer), { optional: true }), undefined, {
...
describe("pojo style", () => {
  test("object -> final class with private fields, @JsonCreator constructor, getters", () => {
    ...
    expect(out).toContain(
      'public Person(@JsonProperty("id") String id, @JsonProperty("age") int age) {',
    );
...
describe("union -> sealed interface", () => {
  ...
  test("discriminated union (meta.discriminator) adds @JsonTypeInfo + @JsonSubTypes", () => {
    const cat = withMeta(
      t(types.object({ type: t(types.literal("cat")), livesLeft: t(types.integer) })),
      {
        typeName: "Cat",
      },
    );
    const dog = withMeta(
      t(types.object({ type: t(types.literal("dog")), breed: t(types.string) })),
      {
        typeName: "Dog",
      },
    );
...
describe("function", () => {
  test("0-arg non-void -> Supplier<R>", () => {
    expect(toJava(t(types.function([], t(types.string))))).toBe(
  ...
  test("1-arg non-void -> Function<A, R>", () => {
    expect(
      toJava(t(types.function([{ name: "x", type: t(types.string) }], t(types.integer)))),
    ).toBe("java.util.function.Function<String, Integer>");
  });
