...
const handlers: Record<string, Converter> = {
  ...
  object: (shape, _ref, ctxName, ctx) => {
    ...
    const deprecated =
      typeof _ref.meta.deprecated === "string"
        ? _ref.meta.deprecated
        : _ref.meta.deprecated === true
          ? true
          : undefined;
  ...
  union: (shape, _ref, ctxName, ctx) => {
    ...
    const parts = s.variants.map((variant, i) =>
      toPythonType(variant, `${ctxName}Variant${i + 1}`, ctx),
    );
  ...
  function: (shape, _ref, ctxName, ctx) => {
    ...
    const params = s.params.map((param, i) =>
      toPythonType(param.type, `${ctxName}Param${i + 1}`, ctx),
    );
  ...
  interface: (shape, _ref, ctxName, ctx) => {
    ...
    for (const [methodName, methodRef] of Object.entries(s.methods)) {
      ...
      const params = m.params.map(
        (p) => `${p.name}: ${toPythonType(p.type, capitalize(p.name), ctx)}`,
      );
