...
describe("scalar types", () => {
  ...
  test("double", () => {
    expect(fromProtoField({ name: "n", number: 1, type: "TYPE_DOUBLE" }).shape.kind).toBe(
      "float64",
    );
  });
...
describe("field modifiers", () => {
  ...
  test("repeated -> array", () => {
    ...
    expect((field.shape as { element: { shape: { kind: string } } }).element.shape.kind).toBe(
      "string",
    );
  });
});
...
describe("well-known types", () => {
  const wk = (typeName: string) =>
    fromProtoField({ name: "n", number: 1, type: "TYPE_MESSAGE", typeName });
...
describe("fromProtoDescriptor: messages", () => {
  ...
  test("nested message -> ref resolving into defs under a dotted path", () => {
      ...
      messageType: [
        {
          ...
          nestedType: [
            { name: "Address", field: [{ name: "city", number: 1, type: "TYPE_STRING" }] },
          ],
        },
      ],
    };
    ...
    const person = doc.defs.Person!;
    const fields = (
      person.shape as unknown as {
        fields: Record<
          string,
          { shape: { kind: "ref"; target: string }; meta: Record<string, unknown> }
        >;
      }
    ).fields;
  ...
  test("self-referential (recursive) message resolves via ref without infinite inlining", () => {
    ...
    const doc = fromProtoDescriptor(file);
    const fields = (
      doc.defs.Node!.shape as {
        fields: Record<string, { shape: { kind: string; target?: string } }>;
      }
    ).fields;
  ...
  test("repeated scalar -> array", () => {
    const file: ProtoFileDescriptor = {
      messageType: [
        {
          name: "Post",
          field: [{ name: "tags", number: 1, type: "TYPE_STRING", label: "LABEL_REPEATED" }],
        },
      ],
    };
    const doc = fromProtoDescriptor(file);
    const fields = (doc.defs.Post!.shape as { fields: Record<string, { shape: { kind: string } }> })
      .fields;
...
describe("fromProtoDescriptor: map fields", () => {
  test("map<string, int32> -> map TypeRef", () => {
    ...
    const doc = fromProtoDescriptor(file);
    const fields = (
      doc.defs.Widget!.shape as {
        fields: Record<
          string,
          {
            shape: {
              kind: string;
              key: { shape: { kind: string } };
...
describe("fromProtoDescriptor: oneof", () => {
  test("oneof -> union field, each variant tagged with its original proto field name", () => {
    ...
    const doc = fromProtoDescriptor(file);
    const fields = (
      doc.defs.Shape!.shape as unknown as {
        fields: Record<
          string,
          {
            shape: {
              kind: string;
              variants: { shape: { kind: string }; meta: Record<string, unknown> }[];
            };
...
describe("parseProtoText", () => {
  ...
  test("parses map fields", () => {
    ...
    expect(widget.field![0]).toMatchObject({
      name: "props",
      type: "TYPE_MESSAGE",
      typeName: "PropsEntry",
      label: "LABEL_REPEATED",
  ...
  test("parses nested messages", () => {
    ...
    expect(person.field![0]).toMatchObject({
      name: "address",
      type: "TYPE_MESSAGE",
      typeName: "Address",
    });
...
describe("fromProtoText: end-to-end .proto -> TypeRefDocument", () => {
  test("messages, enums, nested types, map, oneof, repeated, well-known types all convert", () => {
    ...
    const fields = (
      account.shape as unknown as {
        fields: Record<
          string,
          { shape: { kind: string; target?: string }; meta: Record<string, unknown> }
        >;
      }
    ).fields;
    const addressRef = fields.address as unknown as {
      shape: { kind: "ref"; target: string };
      meta: Record<string, unknown>;
    };
    ...
    expect(
      (resolvedAddress.shape as unknown as { fields: Record<string, { shape: { kind: string } }> })
        .fields.city!.shape.kind,
    ).toBe("string");
