...
describe("metadata passthrough", () => {
  ...
  test("readOnly / writeOnly", () => {
    expect(toJsonSchema(t(types.string, { readOnly: true }))).toEqual({
      type: "string",
      readOnly: true,
    });
    expect(toJsonSchema(t(types.string, { writeOnly: true }))).toEqual({
      type: "string",
      writeOnly: true,
    });
  });
...
describe("toJsonSchemaDocument", () => {
  ...
  test("every defs entry is converted, not just ones reachable from root", () => {
    ...
    expect(result.$defs).toEqual({
      Unused: { type: "object", properties: { x: { type: "number" } }, required: ["x"] },
    });
  });
});
