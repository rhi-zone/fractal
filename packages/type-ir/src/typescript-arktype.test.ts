  ...
  test("complex variants use type.or", () => {
    const ref = t(
      types.union([
        ...
        t(types.object({ code: t(types.integer) })),
      ]),
    );
    expect(toArkType(ref)).toBe(
      'type.or(type({ id: "string" }), type({ code: "number.integer" }))',
    );
  });
});
...
describe("toArkTypeDeclaration", () => {
  test("emits a const declaration", () => {
    expect(toArkTypeDeclaration("Age", t(types.integer))).toBe(
