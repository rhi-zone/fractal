...
describe("complex types (postgres)", () => {
  test("object", () => {
    expect(toSqlDdl(t(types.object({ name: t(types.string) })))).toEqual({
      type: "JSONB",
      nullable: false,
    });
  });
  ...
  test("map", () => {
    expect(toSqlDdl(t(types.map(t(types.string), t(types.number))))).toEqual({
      type: "JSONB",
      nullable: false,
    });
  });
...
describe("sqlite dialect overrides", () => {
  ...
  test("unknown", () => {
    expect(toSqlDdl(t(types.unknown), { dialect: "sqlite" })).toEqual({
      type: "TEXT",
      nullable: false,
    });
  });
...
describe("mysql dialect", () => {
  test("boolean", () => {
    expect(toSqlDdl(t(types.boolean), { dialect: "mysql" })).toEqual({
      type: "TINYINT(1)",
      nullable: false,
    });
  });
  ...
  test("number", () => {
    expect(toSqlDdl(t(types.number), { dialect: "mysql" })).toEqual({
      type: "DOUBLE",
      nullable: false,
    });
  });
  ...
  test("integer", () => {
    expect(toSqlDdl(t(types.integer), { dialect: "mysql" })).toEqual({
      type: "INT",
      nullable: false,
    });
  });
  ...
  test("string", () => {
    expect(toSqlDdl(t(types.string), { dialect: "mysql" })).toEqual({
      type: "VARCHAR(255)",
      nullable: false,
    });
  });
  ...
  test("datetime", () => {
    expect(toSqlDdl(datetime(), { dialect: "mysql" })).toEqual({
      type: "DATETIME",
      nullable: false,
    });
  });
  ...
  test("duration", () => {
    expect(toSqlDdl(duration(), { dialect: "mysql" })).toEqual({
      type: "VARCHAR(255)",
      nullable: false,
    });
  });
  ...
  test("null", () => {
    expect(toSqlDdl(t(types.null), { dialect: "mysql" })).toEqual({
      type: "TINYINT(1)",
      nullable: false,
    });
  });
  ...
  test("unknown", () => {
    expect(toSqlDdl(t(types.unknown), { dialect: "mysql" })).toEqual({
      type: "JSON",
      nullable: false,
    });
  });
  ...
  test("union", () => {
    expect(
      toSqlDdl(t(types.union([t(types.string), t(types.integer)])), { dialect: "mysql" }),
    ).toEqual({
      type: "JSON",
      nullable: false,
    });
  ...
  test("boolean literal", () => {
    expect(toSqlDdl(t(types.literal(true)), { dialect: "mysql" })).toEqual({
      type: "TINYINT(1)",
      nullable: false,
    });
  });
  ...
  test("number literal", () => {
    expect(toSqlDdl(t(types.literal(1)), { dialect: "mysql" })).toEqual({
      type: "NUMERIC",
      nullable: false,
    });
  });
  ...
  test("string literal", () => {
    expect(toSqlDdl(t(types.literal("active")), { dialect: "mysql" })).toEqual({
      type: "TEXT",
      nullable: false,
    });
  });
  ...
  test("ref", () => {
    expect(toSqlDdl(t(types.ref("User")), { dialect: "mysql" })).toEqual({
      type: "TEXT",
      nullable: false,
    });
  });
...
describe("description metadata → comment", () => {
  ...
  test("mysql uses native inline COMMENT", () => {
    expect(
      toSqlDdl(t(types.string, { description: "the user's handle" }), { dialect: "mysql" }),
    ).toEqual({
      type: "VARCHAR(255)",
      nullable: false,
...
describe("columnDef renders checks and comments", () => {
  ...
  test("appends comment after checks", () => {
    expect(columnDef("age", toSqlDdl(int32({ minimum: 0, description: "age in years" })))).toBe(
  ...
  test("mysql: comment renders as native COMMENT clause", () => {
    expect(
      columnDef(
        "name",
        toSqlDdl(t(types.string, { description: "display name" }), { dialect: "mysql" }),
      ),
...
describe("union roots", () => {
  ...
  describe("default layout (single-table-inheritance)", () => {
    ...
    test("plain (undiscriminated) union: falls back to a bare 'kind' discriminator column", () => {
      ...
      expect(toCreateTable("Plain", plainUnion)).toBe(
        "CREATE TABLE Plain (\n  kind TEXT NOT NULL\n);",
      );
    });
    ...
    test("one CREATE TABLE per variant, each with proper NOT NULL constraints", () => {
      expect(
        toCreateTable("ApiResponse", discriminatedUnion, {
          unionLayout: tablePerVariantSqlLayout(),
        }),
      ).toBe(
  ...
  describe("custom layout function", () => {
    test("a caller's own SqlUnionLayout is honored — no closed set of built-in strategies", () => {
      ...
      expect(
        toCreateTable("ApiResponse", discriminatedUnion, { unionLayout: countingLayout }),
  ...
  describe("baseTablePerVariantSqlLayout", () => {
    ...
    test("a field shared by every variant is promoted onto the base table, not duplicated per child", () => {
      ...
      expect(
        toCreateTable("ApiResponse", union, { unionLayout: baseTablePerVariantSqlLayout() }),
      ).toBe(
        "CREATE TABLE ApiResponse (\n" +
          "  id SERIAL PRIMARY KEY,\n" +
          "  type TEXT NOT NULL,\n" +
    ...
    test("plain union falls back to positional variant names and a single 'value' column per child", () => {
      const plainUnion = t(types.union([t(types.string), t(types.integer)]));
      expect(
        toCreateTable("Plain", plainUnion, { unionLayout: baseTablePerVariantSqlLayout() }),
      ).toBe(
        "CREATE TABLE Plain (\n  id SERIAL PRIMARY KEY,\n  kind TEXT NOT NULL\n);\n\n" +
