...
const handlers: Record<string, Converter> = {
  ...
  object: (shape, meta, selfName) => {
    ...
    const fields = Object.entries(s.fields).map(([name, field]) => {
      ...
      const wrapped =
        field.meta.optional === true ? `Type.Optional(${readonlyExpr})` : readonlyExpr;
  ...
  tuple: (shape, meta, selfName) => {
    ...
    return call(
      "Type.Tuple",
      [`[${s.elements.map((el) => toTypeBox(el, selfName)).join(", ")}]`],
      [],
      meta,
    );
  },
  ...
  union: (shape, meta, selfName) => {
    ...
    return call(
      "Type.Union",
      [`[${s.variants.map((v) => toTypeBox(v, selfName)).join(", ")}]`],
      [],
      meta,
    );
  },
  ...
  enum: (shape, meta) => {
    ...
    return call(
      "Type.Union",
      [`[${s.members.map((m) => `Type.Literal(${JSON.stringify(m)})`).join(", ")}]`],
      [],
      meta,
    );
  },
  ...
  intersection: (shape, meta, selfName) => {
    ...
    return call(
      "Type.Intersect",
      [`[${s.members.map((m) => toTypeBox(m, selfName)).join(", ")}]`],
      [],
      meta,
    );
  },
...
export function toTypeBox(ref: TypeRef, selfName?: string): string {
  ...
  const expr =
    converter === undefined ? "Type.Unknown()" : converter(ref.shape, ref.meta, selfName);
