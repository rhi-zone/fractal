...
describe("object classes", () => {
  ...
  test("array of objects: element becomes its own top-level class inside a BuiltList", () => {
    const withItems = t(
      types.object({ items: t(types.array(t(types.object({ sku: t(types.string) })))) }),
    );
    const out = toBuiltValue(withItems, "Order");
    expect(out).toContain(
      "abstract class OrderItemsItem implements Built<OrderItemsItem, OrderItemsItemBuilder> {",
    );
...
describe("unions (no native built_value support)", () => {
  test("discriminated union: variant classes plus a manual-dispatch comment", () => {
    ...
    expect(out).toContain(
      "abstract class ShapeVariant0 implements Built<ShapeVariant0, ShapeVariant0Builder> {",
    );
    expect(out).toContain(
      "abstract class ShapeVariant1 implements Built<ShapeVariant1, ShapeVariant1Builder> {",
    );
