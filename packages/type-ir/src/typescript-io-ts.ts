...
function constraintNotes(meta: Readonly<Record<string, unknown>>, kind: string): string[] {
  ...
  if (typeof meta.exclusiveMinimum === "number" && numberLike)
    notes.push(`exclusiveMinimum: ${meta.exclusiveMinimum}`);
  ...
  if (typeof meta.exclusiveMaximum === "number" && numberLike)
    notes.push(`exclusiveMaximum: ${meta.exclusiveMaximum}`);
  ...
  if (typeof meta.minLength === "number" && lengthConstrainable)
    notes.push(`minLength: ${meta.minLength}`);
  if (typeof meta.maxLength === "number" && lengthConstrainable)
    notes.push(`maxLength: ${meta.maxLength}`);
  if (typeof meta.pattern === "string" && stringLike)
    notes.push(`pattern: ${JSON.stringify(meta.pattern)}`);
  if (typeof meta.multipleOf === "number" && numberLike)
    notes.push(`multipleOf: ${meta.multipleOf}`);
  if (typeof meta.description === "string")
    notes.push(`description: ${JSON.stringify(meta.description)}`);
...
export function toIoTs(ref: TypeRef): string {
  ...
  const { code, note } =
    converter === undefined ? { code: "t.unknown", note: undefined } : converter(ref.shape);
  ...
  const notes = [
    ...(note !== undefined ? [note] : []),
    ...constraintNotes(ref.meta, ref.shape.kind),
  ];
