// packages/type-ir/src/realistic-corpora.ts — @rhi-zone/fractal-type-ir/realistic-corpora
//
// Phase 4 of the JSON-inference evaluation rigor plan (see
// inference-eval.ts's module doc for Phases 1-3): `inference-eval.ts`'s
// evaluation harness runs inference BACKWARDS from a known-good schema —
// `generateCorpus` synthesizes JSON from a `TypeRef`, `fromJsonCorpus`
// re-infers it, `scoreInference` grades the round trip. Every case scored
// so far (`defaultLabeledCases` in inference-eval.ts) is therefore
// conditioned on the GENERATOR's own distributional assumptions (uniform or
// profile-shaped presence/enum/dict sampling) — a clustering method that
// wins against `generateCorpus` output could still lose against documents a
// human actually wrote, because real APIs correlate field presence, order
// enum values non-randomly, and nest structure in ways no synthetic
// generator was told to reproduce.
//
...
// `defaultLabeledCases` cannot provide, because there the "ground truth"
// and the "samples" are generated by the same code path.
//
...
// this follows.
//
// HONEST LIMITATION (matches the approved design's own framing): these
// corpora are researcher-constructed to RESEMBLE real-world structure, not
// literally real-world data pulled from production traffic. They cannot
// substitute for a true live-fetched validation corpus — that remains an
// open, user-decision-gated question (licensing/provenance of any fetched
// dataset, and the tractability of hand-authoring ground truth at a scale
// large enough to be statistically meaningful). This phase is the "offline,
...
// `tryDetectDU`'s DU pass only fires from an object FIELD position, not a
// root-level array of already-heterogeneous top-level values (see
// inference-eval.ts's NOTE by `discriminatedUnionSchema` for the same
// constraint). Each sample below is `{ id, created, livemode, event: {...} }`
// so the discriminated payload sits at that nested `event` field, matching
