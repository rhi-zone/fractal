...
export function toProtoField(ref: TypeRef): ProtoField {
  ...
  const base =
    converter === undefined ? { type: "google.protobuf.Any" } : converter(ref.shape, ref.meta);
...
export function toProtoMessage(name: string, ref: TypeRef): ProtoMessage {
  ...
  for (const [fieldName, fieldRef] of Object.entries(shape.fields)) {
    ...
    if (isA(fieldRef.shape.kind, "object")) {
      ...
      fields.push({
        name: fieldName,
        field: { type: nestedName, repeated: false, optional, ...deprecated, ...description },
        number,
      });
    } else if (
      ...
      fields.push({
        name: fieldName,
        field: { type: enumName, repeated: false, optional, ...deprecated, ...description },
        number,
      });
...
export function toProtoUnionMessage(name: string, ref: TypeRef): ProtoMessage {
  ...
  for (const variant of shape.variants) {
    ...
    if (isA(variant.shape.kind, "object")) {
      ...
      const tag =
        tagShape !== undefined &&
        tagShape.kind === "literal" &&
        typeof (tagShape as TypeShape & { kind: "literal" }).value === "string"
      ...
      oneofFields.push({
        name: fieldName,
        field: { type: messageName, repeated: false, optional: false },
        number,
      });
    } else {
      ...
      oneofFields.push({
        name: positionalName,
        field: { ...field, optional: false, repeated: false },
        number,
      });
    }
    number++;
  ...
  const message: ProtoMessage = {
    name,
    fields: [],
    oneofs: [{ name: oneofName, fields: oneofFields }],
  };
  if (nestedMessages.length > 0) message.nestedMessages = nestedMessages;
  if (typeof ref.meta.description === "string") message.description = ref.meta.description;
  return message;
...
export function toProtoService(name: string, ref: TypeRef): ProtoService {
  ...
  for (const [methodName, methodRef] of Object.entries(shape.methods)) {
    ...
    const resultType = isStreaming
      ? (returnType.shape as TypeShape & { kind: "stream" }).element
      : returnType;
    messages.push({
      name: responseType,
      fields: isVoid
        ? []
        : [{ name: "result", field: toProtoField(resultType as TypeRef), number: 1 }],
    });
    ...
    rpcs.push({
      ...
      requestType,
      responseType,
      ...(isStreaming ? { responseStreaming: true } : {}),
    });
  }
...
function renderService(service: ProtoService, depth: number): string[] {
  ...
  for (const rpc of service.rpcs) {
    const responseType =
      rpc.responseStreaming === true ? `stream ${rpc.responseType}` : rpc.responseType;
