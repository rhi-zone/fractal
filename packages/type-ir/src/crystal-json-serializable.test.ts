...
import {
  ...
  toCrystalType,
} from "./crystal-json-serializable.ts";
...
describe("map", () => {
  test("string keyed", () => {
    expect(toCrystalType(t(types.map(t(types.string), t(types.integer))))).toBe(
      "Hash(String, Int32)",
    );
  });
});
...
describe("tuple", () => {
  test("mixed elements", () => {
    expect(toCrystalType(t(types.tuple([t(types.string), t(types.integer)])))).toBe(
      "Tuple(String, Int32)",
    );
  });
});
...
describe("union", () => {
  test("two variants", () => {
    expect(toCrystalType(t(types.union([t(types.string), t(types.integer)])))).toBe(
      "String | Int32",
    );
  });
