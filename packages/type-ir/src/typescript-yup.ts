...
function runtimeGuard(ref: TypeRef): string {
  ...
  if (kind === "object" || kind === "instance" || kind === "map")
...
function withMeta(expr: string, meta: Readonly<Record<string, unknown>>, kind: string): string {
  ...
  if (typeof meta.pattern === "string" && stringLike)
    result += `.matches(${regexLiteral(meta.pattern)})`;
...
const handlers: Record<string, Converter> = {
  ...
  union: (shape) => {
    ...
    if (s.variants.every((v) => v.shape.kind === "literal")) {
      const values = s.variants.map((v) =>
        JSON.stringify((v.shape as TypeShape & { kind: "literal" }).value),
      );
