...
describe("leaf field types", () => {
  ...
  test("half_float -> number with float16 format (no native float16 kind)", () => {
    expect(fromElasticsearchField({ type: "half_float" })).toEqual(
      t(types.number, { format: "float16" }),
    );
  });
...
describe("date", () => {
  ...
  test("date with format -> datetime with format in meta", () => {
    expect(fromElasticsearchField({ type: "date", format: "yyyy-MM-dd" })).toEqual(
...
describe("geo types", () => {
  ...
  test("geo_shape -> unknown with esType meta", () => {
    expect(fromElasticsearchField({ type: "geo_shape" })).toEqual(
      t(types.unknown, { esType: "geo_shape" }),
    );
  });
});
...
describe("flattened", () => {
  test("flattened -> unknown with esType meta", () => {
    expect(fromElasticsearchField({ type: "flattened" })).toEqual(
      t(types.unknown, { esType: "flattened" }),
    );
  });
});
...
describe("unrecognized field type", () => {
  test("degrades to unknown with esType recorded", () => {
    expect(fromElasticsearchField({ type: "completion" })).toEqual(
      t(types.unknown, { esType: "completion" }),
    );
  });
});
