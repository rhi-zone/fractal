  ...
  if (desc.name === "List") {
    ...
    const element =
      elementDesc !== undefined
        ? convertTypeDesc(elementDesc, registry, selfPath)
        : t(types.unknown);
  ...
  for (const m of members) {
    ...
    if (m.kind === "group") {
      fields[m.name] = withMeta(
        t(types.object(convertMembers(m.members, registry, selfPath))),
        memberMeta(m),
      );
      continue;
    }
    ...
    const variants = m.variants.map((v) =>
      withMeta(fromCapnpField(v, registry, selfPath), { capnpFieldName: v.name }),
    );
...
export function fromCapnp(schema: string): Record<string, TypeRef> {
  ...
  for (const [name, entry] of registry) {
    result[name] =
      entry.kind === "struct"
        ? structToTypeRef(entry.decl, registry, name)
        : enumToTypeRef(entry.decl);
  }
  return result;
}
