...
import { toHaskell, toHaskellModule, toHaskellType } from "./haskell-aeson.ts";
...
describe("toHaskell: object -> record", () => {
  test("simple record with required and optional fields", () => {
    ...
    expect(out).toContain(
      "toJSON = genericToJSON defaultOptions { fieldLabelModifier = personFieldLabel }",
    );
    expect(out).toContain("instance FromJSON Person where");
    expect(out).toContain(
      "parseJSON = genericParseJSON defaultOptions { fieldLabelModifier = personFieldLabel }",
    );
  });
...
describe("toHaskell: doc comments and deprecation", () => {
  ...
  test("enum decl gets doc comment and pragma", () => {
    const ref = withMeta(t(types.enum(["active", "inactive"])), {
  ...
  test("union decl gets doc comment and pragma", () => {
    const ref = withMeta(t(types.union([t(types.string), t(types.integer)])), {
      description: "A value.",
    });
...
describe("toHaskell: default name", () => {
  test('falls back to meta.typeName then "T"', () => {
    ...
    expect(toHaskell(t(types.object({ id: t(types.string) }), { typeName: "widget" }))).toContain(
      "data Widget = Widget",
    );
  });
});
