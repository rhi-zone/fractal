...
describe("type form", () => {
  ...
  test("unrecognized format is kept as plain string metadata, not guessed", () => {
    expect(fromJtd({ type: "string", metadata: { format: "ipv4" } })).toEqual(
...
describe("elements form", () => {
  ...
  test("metadata.stream recovers stream", () => {
    expect(fromJtd({ elements: { type: "int32" }, metadata: { stream: true } })).toEqual(
      t(types.stream(int32())),
    );
  });

  test("metadata.tuple recovers a single-element tuple (lossy — toJtd only kept the first member)", () => {
    expect(fromJtd({ elements: { type: "string" }, metadata: { tuple: true } })).toEqual(
      t(types.tuple([t(types.string)])),
    );
  });
});
...
describe("values form", () => {
  test("map with string keys", () => {
    expect(fromJtd({ values: { type: "float64" } })).toEqual(
      t(types.map(t(types.string), float64())),
    );
  });
});
...
describe("nullable", () => {
  test("type form + nullable", () => {
    expect(fromJtd({ type: "string", nullable: true })).toEqual(
      t(types.string, { nullable: true }),
    );
  });

  test("elements form + nullable", () => {
    expect(fromJtd({ elements: { type: "int32" }, nullable: true })).toEqual(
      t(types.array(int32()), { nullable: true }),
    );
  });
});
...
describe("escape hatches", () => {
  ...
  test("intersection recovers a single-member intersection (lossy — toJtd only kept the first member)", () => {
    expect(fromJtd({ type: "string", metadata: { intersection: true } })).toEqual(
      t(types.intersection([t(types.string)])),
    );
  });

  test("function degrades to a placeholder zero-param/unknown-return function", () => {
    expect(fromJtd({ metadata: { function: true } })).toEqual(
      t(types.function([], t(types.unknown))),
    );
  });
});
