...
import { toYup, toYupDeclaration, toYupDeclarations } from "./typescript-yup.ts";
...
describe("leaf types", () => {
  ...
  test("never", () => {
    expect(toYup(t(types.never))).toBe(
      "yup.mixed().oneOf([] as const) /* no native never type in Yup */",
    );
  });
});
...
describe("map", () => {
  test("lossy object fallback", () => {
    ...
    expect(toYup(ref)).toBe(
      "yup.object() /* lossy: Yup has no native record/map type; key/value types not preserved */",
    );
  });
});
