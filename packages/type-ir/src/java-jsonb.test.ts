...
describe("primitives — bare type expressions", () => {
  ...
  test("optional integer with optionalStyle: optional wraps in Optional<Integer>", () => {
    expect(
      toJsonb(withMeta(t(types.integer), { optional: true }), undefined, {
...
describe("function", () => {
  test("0-arg non-void -> Supplier<R>", () => {
    expect(toJsonb(t(types.function([], t(types.string))))).toBe(
  ...
  test("1-arg non-void -> Function<A, R>", () => {
    expect(
      toJsonb(t(types.function([{ name: "x", type: t(types.string) }], t(types.integer)))),
