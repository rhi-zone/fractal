...
//
// Branded refinement-tag types — for consumers authoring TS source that the
// api-tree extractor reads. Intersecting a base type with one or more of
// these tags (`string & MinLength<2> & MaxLength<100>`) round-trips through
// extraction as the base kind carrying the matching `meta` refinement
// key(s) (`minLength`/`maxLength`/…) — the same keys `compile.ts` already
// validates and every projector (json-schema, effect-schema, sql, …) already
// reads. See packages/api-tree/src/extract.ts's
// `refinementFromIntersection`.
//
...
// `semantic-strings.ts`'s sibling doc comment and
// `packages/api-tree/src/extract.ts`'s `brandNameFromSymbolKeyedProp`), but
// with a STRUCTURED value (`{ minLength: N }`) instead of a single string
// literal, so intersecting several tags merges their value objects instead
// of colliding on one shared string. `RefinementTag` is exported type-only —
// there is no runtime value to export (the `declare const` has no emit) —
// so a consuming module can reference `[RefinementTag]` in a computed
// property type position without importing a value.
//
