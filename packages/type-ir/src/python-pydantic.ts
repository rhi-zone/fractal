...
const handlers: Record<string, Converter> = {
  ...
  object: (shape, ref, ctxName, ctx) => {
    ...
    for (const [fieldName, fieldRef] of Object.entries(s.fields)) {
      ...
      const annotated =
        kwargs.length > 0
          ? (ctx.pydanticImports.add("Field"), ctx.typingImports.add("Annotated"), true)
          : false;
  ...
  tuple: (shape, _ref, ctxName, ctx) => {
    ...
    const parts = s.elements.map((element, i) =>
      toPydanticType(element, `${ctxName}${i + 1}`, ctx),
    );
  ...
  union: (shape, ref, ctxName, ctx) => {
    ...
    const parts = s.variants.map((variant, i) =>
      toPydanticType(variant, `${ctxName}Variant${i + 1}`, ctx),
    );
  ...
  function: (shape, _ref, ctxName, ctx) => {
    ...
    const params = s.params.map((param, i) =>
      toPydanticType(param.type, `${ctxName}Param${i + 1}`, ctx),
    );
  ...
  interface: (shape, _ref, ctxName, ctx) => {
    ...
    for (const [methodName, methodRef] of Object.entries(s.methods)) {
      ...
      const params = m.params.map(
        (p) => `${p.name}: ${toPydanticType(p.type, capitalize(p.name), ctx)}`,
      );
...
function renderDecl(decl: Decl): string[] {
  ...
  if (decl.configArgs.length > 0)
    lines.push(`    model_config = ConfigDict(${decl.configArgs.join(", ")})`);
...
export function toPydantic(ref: TypeRef, name = "Root"): string {
  ...
  const pydanticNames = [
    ...(ctx.needsBaseModel ? ["BaseModel", ...ctx.pydanticImports] : ctx.pydanticImports),
  ].sort();
