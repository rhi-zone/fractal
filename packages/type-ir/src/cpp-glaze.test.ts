...
describe("primitives", () => {
  ...
  test("int32 kind maps to int32_t", () => {
    expect(toGlaze(int32(), "Count")).toBe(
...
describe("struct", () => {
  test("emits a plain struct plus a glz::meta<T> specialization, no per-field glue", () => {
    ...
    expect(out).toContain(
      'static constexpr auto value = glz::object("id", &User::id, "age", &User::age);',
    );
  });
...
describe("enum class", () => {
  test("emits enum class plus a glz::meta<T> using glz::enumerate, no toString/fromString helpers", () => {
    ...
    expect(out).toContain(
      'static constexpr auto value = glz::enumerate(Active, "active", Inactive, "inactive");',
    );
