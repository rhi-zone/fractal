...
describe("sqlite flexible typing", () => {
  ...
  test("INTEGER PRIMARY KEY AUTOINCREMENT", () => {
    const result = fromSql("CREATE TABLE t (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT);", {
      dialect: "sqlite",
    });
    const fields = (
      result.t!.shape as { fields: Record<string, { meta: Record<string, unknown> }> }
...
describe("round-trip against sql.ts", () => {
  test("basic table round-trips through toCreateTable", () => {
    const result = fromSql(
      "CREATE TABLE t (name TEXT NOT NULL, age INTEGER NOT NULL, active BOOLEAN NOT NULL);",
    );
    const ref = result.t!;
  ...
  test("uuid/datetime/date round-trip to their dedicated SQL types", () => {
    const result = fromSql(
      "CREATE TABLE t (id UUID NOT NULL, created_at TIMESTAMP NOT NULL, day DATE NOT NULL);",
    );
