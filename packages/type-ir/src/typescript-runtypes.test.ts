...
import {
  toRuntypes,
  toRuntypesDeclaration,
  toRuntypesDeclarations,
} from "./typescript-runtypes.ts";
...
describe("intersection", () => {
  test("uses native R.Intersect(...) for any arity", () => {
    ...
    expect(toRuntypes(ref)).toBe(
      "R.Intersect(R.Record({ id: R.String }), R.Record({ createdAt: R.String }))",
    );
  });
...
describe("constraints", () => {
  ...
  test("regex with forward slash in pattern", () => {
    ...
    expect(toRuntypes(ref)).toBe(
      'R.String.withConstraint((v) => /a\\/b/.test(v) || "must match a/b")',
    );
  });
});
