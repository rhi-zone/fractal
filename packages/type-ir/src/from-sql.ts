...
function mapSqlType(name: string, args: string[], dialect: SqlDialect | undefined): TypeMapping {
  switch (name) {
    ...
    case "CLOB": {
      ...
      return {
        ref: t(types.string, len !== undefined && !Number.isNaN(len) ? { maxLength: len } : {}),
      };
    }
    ...
    default:
      return dialect === "sqlite"
        ? { ref: sqliteAffinity(name) }
        : { ref: t(types.unknown, { sqlType: name }) };
  }
}
...
const TABLE_CONSTRAINT_RE =
  /^(?:CONSTRAINT\s+\S+\s+)?(PRIMARY\s+KEY|FOREIGN\s+KEY|UNIQUE|CHECK)\b/i;
...
function parseColumnConstraints(
  ...
  while (rest.length > 0) {
    ...
    if ((m = /^NOT\s+NULL\b/i.exec(rest)) !== null) {
      ...
      (m = /^GENERATED\s+(?:ALWAYS|BY\s+DEFAULT)\s+AS\s+IDENTITY\b/i.exec(rest)) !== null
    ) {
      meta.autoincrement = true;
      ...
      (m =
        /^ON\s+(?:UPDATE|DELETE)\s+(?:CASCADE|SET\s+NULL|SET\s+DEFAULT|RESTRICT|NO\s+ACTION)/i.exec(
          rest,
        )) !== null
    ) {
...
function parseColumnListItem(
  ...
  if (constraintMatch !== null) {
    ...
    if (kind === "FOREIGN KEY") {
      ...
      const refMatch =
        /REFERENCES\s+(?:"([^"]+)"|`([^`]+)`|\[([^\]]+)\]|([\w.]+))\s*(?:\(([^)]+)\))?/i.exec(
          afterKeyword.slice(consumed),
        );
      if (refMatch !== null) {
        ...
        for (const col of cols)
          table.foreignKeys.set(
            col,
            refCol !== undefined ? { table: refTable, column: refCol } : { table: refTable },
          );
      }
      return;
    }
  ...
  const typeMatch =
    /^([A-Za-z_][\w]*(?:\s+(?:PRECISION|VARYING|ZONE|TIME|WITH|WITHOUT))*)\s*(\([^)]*\))?((?:\s*\[\s*\])*|\s+ARRAY)?/i.exec(
      afterName,
    );
  const typeText =
    typeMatch !== null ? (typeMatch[0] ?? "").trim() : (afterName.split(/\s+/)[0] ?? "");
