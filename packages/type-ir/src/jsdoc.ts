...
function fieldSuffix(field: TypeRef, optional: boolean, readonly = false): string {
  const description =
    typeof field.meta.description === "string" ? field.meta.description : undefined;
...
export function toJsDocTypedef(
  name: string,
  ref: TypeRef,
  options: JsDocDeclarationOptions = {},
): string {
  ...
  if (mode === "interface") {
    const lines =
      s === undefined
        ? []
        : Object.entries(s.fields).map(([fieldName, field]) => propertyLine(fieldName, field));
  ...
  if (mode === "class") {
    const lines =
      s === undefined
        ? []
        : Object.entries(s.fields).map(([fieldName, field]) => paramLine(fieldName, field));
    return [
      "/**",
  ...
  if (isObject && s !== undefined) {
    const lines = Object.entries(s.fields).map(([fieldName, field]) =>
      propertyLine(fieldName, field),
    );
    return [
      "/**",
  ...
  if (ref.meta.deprecated === true) {
    return [
      "/**",
      ` * @typedef {${toJsDocType(ref)}} ${name}${description}`,
      " * @deprecated",
      " */",
    ].join("\n");
  }
...
export function toJsDocTypedefs(
