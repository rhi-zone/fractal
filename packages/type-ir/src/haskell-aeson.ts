...
function buildRecordDecl(name: string, ref: TypeRef, outerDecls: string[]): string {
  ...
  const decorated = [
    ...haddockComment(ref.meta),
    ...deprecatedPragma(name, ref.meta),
    dataDecl,
  ].join("\n");
...
function buildDiscriminatedUnionDecl(
  ...
  const ctorLines = ctors.map((c) =>
    c.fields.length === 0
      ? c.ctorName
      : `${c.ctorName} { ${c.fields.map((f) => `${f.hsField} :: ${f.hsType}`).join(", ")} }`,
  );
  const dataDecl = [
    `data ${name}`,
    `  = ${ctorLines.join("\n  | ")}`,
  ...
  const fromJsonInstance = [
    ...
    ...ctors.map((c) => {
      ...
      const applicative = c.fields
        .map((f, i) =>
          i === 0 ? `${c.ctorName} <$> o .: ${quote(f.jsonName)}` : `<*> o .: ${quote(f.jsonName)}`,
        )
        .join(" ");
...
function buildPlainUnionDecl(
  ...
  const ctors = variants.map((variant, index) => {
    ...
    if (kind === "literal") {
      ...
      base =
        typeof value === "string"
          ? capitalize(sanitizeIdent(value))
          : capitalize(sanitizeIdent(String(value)));
...
export function toHaskell(ref: TypeRef, name?: string): string {
  const typeName = capitalize(
    name ?? (typeof ref.meta.typeName === "string" ? ref.meta.typeName : "T"),
  );
  const decls: string[] = [];
  const kind = ref.shape.kind;
