...
describe("mergeRegistries", () => {
  ...
  it("rejects conflicting ids rather than silently picking a winner", () => {
    expect(() =>
      mergeRegistries(defineRegistry([a]), defineRegistry([{ id: "a", value: 9 }])),
    ).toThrow(/conflicting registry id when merging: a/);
  });
...
describe("lookup", () => {
  it("names the kind in the error so a typo is diagnosable", () => {
    expect(() => lookup(defineRegistry([a]), "output format", "zz")).toThrow(
      /unknown output format: zz/,
    );
  });
