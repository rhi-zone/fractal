...
export function fromJsonSchema(schema: JsonSchema): TypeRef {
  ...
  if (typeof className === "string") {
    ...
    return withMeta(
      t(types.instance(className, typeof declFile === "string" ? declFile : "")),
      meta,
    );
  }
  ...
  if (Array.isArray(schema.anyOf)) {
    ...
    if (nullable) {
      if (rest.length === 1)
        return withMeta(fromJsonSchema(rest[0] as JsonSchema), { ...meta, nullable: true });
  ...
  if (Array.isArray(schema.allOf)) {
    return withMeta(
      t(types.intersection((schema.allOf as JsonSchema[]).map(fromJsonSchema))),
      meta,
    );
  }
