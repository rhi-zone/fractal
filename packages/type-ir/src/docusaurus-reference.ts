...
const linkedHandlers: Record<string, LinkedConverter> = {
  ...
  tuple: (shape, doc, refs) => {
    ...
    return [
      ...code("["),
      ...join(
        s.elements.map((el) => renderLinkedType(el, doc, refs)),
        ", ",
      ),
      ...code("]"),
    ];
  ...
  ref: (shape, doc, refs) => {
    ...
    refs.add(s.target);
    return isDefined(s.target, doc)
      ? [{ link: s.target, href: linkHref(s.target) }]
      : code(s.target);
  },
  ...
  function: (shape, doc, refs) => {
    ...
    const params = s.params.map((p) => [
      ...code(`${p.name}: `),
      ...renderLinkedType(p.type, doc, refs),
    ]);
    return [
      ...code("("),
      ...join(params, ", "),
      ...code(") => "),
      ...renderLinkedType(s.returnType, doc, refs),
    ];
  },
  interface: (shape, doc, refs) => {
    ...
    const methods = Object.entries(s.methods).map(([name, methodRef]) => {
      ...
      return [
        ...
        ...join(params, ", "),
        ...code("): "),
        ...renderLinkedType(m.returnType, doc, refs),
      ];
    });
...
function fieldsTable(
  ...
  const rows = Object.entries(fields).map(([name, field]) => {
    ...
    if (field.meta.default !== undefined)
      bits.push(`default: \`${JSON.stringify(field.meta.default)}\``);
...
function renderPage(name: string, ref: TypeRef, doc: TypeRefDocument, index: number): string {
  ...
  const resolvedKind =
    kind in linkedHandlers ? kind : ancestors(kind).find((a) => a in linkedHandlers);
  ...
  if (referenced.length > 0) {
    ...
    for (const refName of referenced) {
      ...
      const summary =
        target === undefined
          ? "Unresolved reference — no matching entry in defs."
          : (description(target.meta) ?? "");
      const trailer = isDefined(refName, doc)
        ? ` — [${refName}](${linkHref(refName)})`
        : " _(unresolved)_";
      lines.push(
        `- <TypeRef name={${JSON.stringify(refName)}} summary={${JSON.stringify(summary)}} />${trailer}`,
      );
    }
    lines.push("");
  }
 ...
 */
export function toDocusaurusReference(
  doc: TypeRefDocument,
