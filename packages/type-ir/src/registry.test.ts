...
describe("importers", () => {
  ...
  it("rejects corpus ingestion of a non-corpus importer", () => {
    expect(() => ingestCorpus("graphql", ["type A { b: String }"])).toThrow(/not a corpus importer/)
  })
...
describe("convert", () => {
  it("crosses format families", () => {
    const out = convert("json-schema", "python-pydantic", JSON.stringify({ type: "object", properties: { id: { type: "string" } } }))
