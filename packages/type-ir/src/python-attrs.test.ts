...
describe("primitives", () => {
  test("boolean", () => {
    expect(toAttrs(t(types.boolean), "Flag")).toBe(
      "from __future__ import annotations\n\nFlag = bool\n",
    );
  });

  test("number maps to float", () => {
    expect(toAttrs(t(types.number), "Amount")).toBe(
      "from __future__ import annotations\n\nAmount = float\n",
    );
  });

  test("integer maps to int", () => {
    expect(toAttrs(t(types.integer), "Count")).toBe(
      "from __future__ import annotations\n\nCount = int\n",
    );
  });

  test("string maps to str", () => {
    expect(toAttrs(t(types.string), "Name")).toBe(
      "from __future__ import annotations\n\nName = str\n",
    );
  });
  ...
  test("null maps to None", () => {
    expect(toAttrs(t(types.null), "Nothing")).toBe(
      "from __future__ import annotations\n\nNothing = None\n",
    );
  });
...
describe("validation constraints", () => {
  ...
  test("multipleOf has no built-in attrs validator, so it surfaces as an unmodeled-metadata stub", () => {
    ...
    expect(out).toContain(
      "def _validate_amount(instance: object, attribute: object, value: object) -> None:",
    );
  ...
  test("description and deprecated become attrs.field metadata kwargs", () => {
    ...
    expect(out).toContain(
      'legacyId: str = attrs.field(metadata={"description": "old identifier", "deprecated": True})',
    );
  });
  ...
  test("field-level readonly becomes attrs.field(on_setattr=attrs.setters.frozen)", () => {
    ...
    expect(toAttrs(ref, "Record")).toContain(
  ...
  test("unrecognized meta keys surface as a standalone validator function wired via validator=", () => {
    ...
    expect(out).toContain(
      "def _validate_email(instance: object, attribute: object, value: object) -> None:",
    );
...
test("ref renders as the bare target name", () => {
  expect(toAttrs(t(types.ref("User")), "Alias")).toBe(
    "from __future__ import annotations\n\nAlias = User\n",
  );
});
...
test("unknown kind fallback maps to Any", () => {
  ...
  expect(toAttrs(ref, "Mystery")).toBe(
    "from __future__ import annotations\nfrom typing import Any\n\nMystery = Any\n",
  );
});
