  ...
  test("discriminated union: oneOf + native OAS 3.0 discriminator.propertyName", () => {
    ...
    expect(toOpenApi30(ref)).toEqual({
      oneOf: [
        {
          type: "object",
          properties: { type: { enum: ["circle"] }, radius: { type: "number" } },
          required: ["type", "radius"],
        },
        {
          type: "object",
          properties: { type: { enum: ["square"] }, side: { type: "number" } },
          required: ["type", "side"],
        },
      ],
...
describe("metadata passthrough", () => {
  ...
  test("readOnly / writeOnly", () => {
    expect(toOpenApi30(t(types.string, { readOnly: true }))).toEqual({
      type: "string",
      readOnly: true,
    });
    expect(toOpenApi30(t(types.string, { writeOnly: true }))).toEqual({
      type: "string",
      writeOnly: true,
    });
  });
