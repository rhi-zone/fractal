...
const repository = t(
  types.object({
    ...
    stargazersCount: t(types.integer, {
      deprecated: "renamed to `stargazers_count` (snake_case) in API v4; this field will be removed in v5",
  ...
  }),
  {
    ...
    example: { name: "fractal", owner: { login: "rhi-zone", id: 42 }, visibility: "public", stargazersCount: 12 },
  },
)
...
const issueTracker = t(
  types.interface({
    listIssues: t(
      types.function([{ name: "repo", type: t(types.ref("Repository")) }], t(types.array(t(types.ref("WebhookEvent"))))),
...
describe("sphinx-reference structural validity (bar A, RST-specific)", () => {
  ...
  it("every page filename is the kebab-case def name plus .rst", () => {
    expect([...pages.keys()].sort()).toEqual(
      ["repository.rst", "owner.rst", "visibility.rst", "webhook-event.rst", "issue-tracker.rst"].sort(),
    )
  })
})
...
describe("sphinx-reference — cross-page link/label agreement", () => {
  it("every :ref: target used anywhere resolves to a real `.. _label:` declared on some emitted page", () => {
    const declaredLabels = new Set(
      [...pages.values()].map((content) => /^\.\. _([a-z0-9-]+):/.exec(content)?.[1]).filter((l): l is string => l !== undefined),
    )
