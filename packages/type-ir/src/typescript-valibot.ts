...
function constraintActions(meta: Readonly<Record<string, unknown>>): string[] {
  ...
  if (typeof meta.description === "string")
    actions.push(`v.description(${quote(meta.description)})`);
  return actions;
}
...
const handlers: Record<string, Converter> = {
  ...
  union: (shape, meta) => {
    ...
    if (typeof meta.discriminator === "string") {
      return {
        schema: `v.variant(${quote(meta.discriminator)}, [${variants.join(", ")}])`,
        actions: [],
      };
    }
...
export function toValibot(ref: TypeRef): string {
  ...
  const { schema, actions: baseActions } =
    converter === undefined
      ? { schema: "v.unknown()", actions: [] }
