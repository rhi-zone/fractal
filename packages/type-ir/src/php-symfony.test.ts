...
describe("arrays and maps", () => {
  test("array of scalars", () => {
    expect(toPhpType(t(types.array(t(types.string))))).toEqual({
      type: "array",
      doc: "array<string>",
    });
  });
...
describe("discriminated union -> #[DiscriminatorMap] hierarchy", () => {
  ...
  test("plain (non-discriminated) union degrades to a @phpstan-type alias", () => {
    ...
    expect(toSymfonyDiscriminatedUnion("StringOrInt", ref)).toBe(
  ...
  test("toSymfony dispatches union TypeRefs to toSymfonyDiscriminatedUnion", () => {
    const ref = withMeta(
      t(
        types.union([
          withMeta(t(types.object({ type: t(types.literal("a")) })), { typeName: "A" }),
        ]),
      ),
      { discriminator: "type" },
    );
