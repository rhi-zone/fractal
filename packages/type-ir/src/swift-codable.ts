...
function structDecl(name: string, ref: TypeRef): string {
  ...
  for (const [fieldName, fieldRef] of Object.entries(s.fields)) {
    ...
    codingKeyLines.push(
      swiftName === fieldName
        ? `        case ${swiftName}`
        : `        case ${swiftName} = ${quote(fieldName)}`,
    );
  }
...
function plainUnionDecl(name: string, ref: TypeRef, variants: readonly TypeRef[]): string {
  ...
  for (const c of cases)
    lines.push(`        case .${c.caseName}(let value): try container.encode(value)`);
  lines.push("        }", "    }");
  if (ctx.nested.length > 0) {
    lines.push("");
    for (const decl of ctx.nested) lines.push(...indentLines(decl, 4));
...
function discriminatedUnionDecl(
  ...
  for (const c of cases) {
    lines.push(
      `        case ${quote(c.tag)}: self = .${c.caseName}(try ${c.typeName}(from: decoder))`,
    );
  }
  lines.push(
  ...
  for (const c of cases)
    lines.push(`        case .${c.caseName}(let value): try value.encode(to: encoder)`);
  lines.push("        }", "    }");
  if (ctx.nested.length > 0) {
    lines.push("");
    for (const decl of ctx.nested) lines.push(...indentLines(decl, 4));
