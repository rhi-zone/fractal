...
describe("primitives", () => {
  test("boolean", () => {
    expect(toSonic(t(types.boolean), "Flag")).toBe("type Flag = bool")
  })

  test("number maps to float64", () => {
    expect(toSonic(t(types.number), "Amount")).toBe("type Amount = float64")
  })

  test("string", () => {
    expect(toSonic(t(types.string), "Name")).toBe("type Name = string")
  })

  test("bare integer maps to int", () => {
    expect(toSonic(t(types.integer), "Count")).toBe("type Count = int")
  })

  test("int32/int64 preserve width", () => {
    expect(toSonic(int32(), "Id32")).toBe("type Id32 = int32")
    expect(toSonic(int64(), "Id64")).toBe("type Id64 = int64")
  })

  test("float32 preserves width", () => {
    expect(toSonic(float32(), "Ratio")).toBe("type Ratio = float32")
  })

  test("uuid degrades to string", () => {
    expect(toSonic(uuid(), "Id")).toBe("type Id = string")
  })

  test("datetime maps to time.Time", () => {
    expect(toSonic(datetime(), "CreatedAt")).toBe("type CreatedAt = time.Time")
  })

  test("bytes maps to []byte", () => {
    expect(toSonic(bytes(), "Payload")).toBe("type Payload = []byte")
  })

  test("null and unknown map to interface{}", () => {
    expect(toSonic(t(types.null), "Nothing")).toBe("type Nothing = interface{}")
    expect(toSonic(t(types.unknown), "Anything")).toBe("type Anything = interface{}")
  })

  test("void and never map to struct{}", () => {
    expect(toSonic(t(types.void), "Nada")).toBe("type Nada = struct{}")
    expect(toSonic(t(types.never), "Impossible")).toBe("type Impossible = struct{}")
  })
})

test("array becomes a slice", () => {
  expect(toSonic(t(types.array(t(types.string))), "Names")).toBe("type Names = []string")
})

test("map becomes a Go map", () => {
  const ref = t(types.map(t(types.string), t(types.number)))
  expect(toSonic(ref, "Scores")).toBe("type Scores = map[string]float64")
})

describe("struct with json tags", () => {
  test("required field gets a plain json tag", () => {
    const ref = t(types.object({ name: t(types.string) }))
    expect(toSonic(ref, "User")).toBe('type User struct {\n\tName string `json:"name"`\n}')
  })
  ...
  test("optional slice field is not pointer-wrapped (nil already means absent)", () => {
    ...
    expect(toSonic(ref, "Post")).toBe('type Post struct {\n\tTags []string `json:"tags,omitempty"`\n}')
  })
...
describe("enum", () => {
  test("renders as a string type plus a const block", () => {
    ...
    expect(toSonic(ref, "Status")).toBe(
      [
        "type Status string",
        "",
        "const (",
...
describe("service interface", () => {
  test("interface kind maps to a native Go interface", () => {
    ...
    expect(toSonic(ref, "Account")).toBe(
...
test("default root name is Root", () => {
  expect(toSonic(t(types.string))).toBe("type Root = string")
})

test("sonic output is identical to encoding/json output (drop-in compatibility)", async () => {
  ...
  expect(toSonic(ref, "User")).toBe(toGo(ref, "User"))
})
