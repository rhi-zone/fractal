...
describe("primitives", () => {
  test("boolean", () => {
    expect(toPydantic(t(types.boolean), "Flag")).toBe(
      "from __future__ import annotations\n\nFlag = bool\n",
    );
  });

  test("number maps to float", () => {
    expect(toPydantic(t(types.number), "Amount")).toBe(
      "from __future__ import annotations\n\nAmount = float\n",
    );
  });

  test("integer maps to int", () => {
    expect(toPydantic(t(types.integer), "Count")).toBe(
      "from __future__ import annotations\n\nCount = int\n",
    );
  });

  test("string maps to str", () => {
    expect(toPydantic(t(types.string), "Name")).toBe(
      "from __future__ import annotations\n\nName = str\n",
    );
  });

  test("bytes maps to bytes", () => {
    expect(toPydantic(bytes(), "Blob")).toBe(
      "from __future__ import annotations\n\nBlob = bytes\n",
    );
  });

  test("null maps to None", () => {
    expect(toPydantic(t(types.null), "Nothing")).toBe(
      "from __future__ import annotations\n\nNothing = None\n",
    );
  });
...
describe("validation constraints", () => {
  ...
  test("description and deprecated become Field kwargs", () => {
    ...
    expect(out).toContain(
      'legacyId: Annotated[str, Field(description="old identifier", deprecated=True)]',
    );
  });
...
describe("unions", () => {
  ...
  test("discriminated union renders Annotated[Union[...], Discriminator(...)]", () => {
    ...
    expect(out).toContain("from typing import Annotated");
    expect(out).toContain(
      'Shape = Annotated[Union[ShapeVariant1, ShapeVariant2], Discriminator("kind")]',
    );
  });
...
test("ref renders as the bare target name", () => {
  expect(toPydantic(t(types.ref("User")), "Alias")).toBe(
    "from __future__ import annotations\n\nAlias = User\n",
  );
});
...
test("unknown kind fallback maps to Any", () => {
  ...
  expect(toPydantic(ref, "Mystery")).toBe(
    "from __future__ import annotations\nfrom typing import Any\n\nMystery = Any\n",
  );
});
