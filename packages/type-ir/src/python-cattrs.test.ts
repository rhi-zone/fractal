...
describe("validation constraints (identical to python-attrs.ts)", () => {
  ...
  test("field-level readonly becomes attrs.field(on_setattr=attrs.setters.frozen)", () => {
    ...
    expect(toCattrs(ref, "Record")).toContain(
...
describe("collections", () => {
  ...
  test("tuple", () => {
    expect(toCattrs(t(types.tuple([t(types.string), t(types.integer)])), "Pair")).toContain(
      "Pair = tuple[str, int]",
    );
  });

  test("dict with string key", () => {
    expect(toCattrs(t(types.map(t(types.string), t(types.integer))), "Counts")).toContain(
      "Counts = dict[str, int]",
    );
  });
});
