...
describe("merged importer registry", () => {
  ...
  it("spans both halves under one id space", () => {
    ...
    expect(merged.entries.length).toBe(importerRegistry.entries.length + typescriptImporterRegistry.entries.length)
  })
  ...
  it("reports an input-shape mismatch by name instead of failing inside the importer", () => {
    expect(() => ingestFrom(merged, "typescript", "type User { id: ID! }")).toThrow(/needs a file and symbol/)
    expect(() => ingestFrom(merged, "graphql", { file: fixture, symbol: "Sample" })).toThrow(/needs source text/)
  })
