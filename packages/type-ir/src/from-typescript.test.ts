...
function programFromSource(source: string): {
  ...
  host.getSourceFile = (fileName, languageVersionOrOptions, ...rest) =>
    fileName === FILE
      ? sourceFile
      : originalGetSourceFile(fileName, languageVersionOrOptions, ...rest);
  host.writeFile = () => {};
...
describe("objects", () => {
  ...
  it("JSDoc description and @default flow into field meta", () => {
    ...
    const fields = (ref.shape as { fields: Record<string, { meta: Record<string, unknown> }> })
      .fields;
...
describe("arrays and tuples", () => {
  ...
  it("tuple elements", () => {
    ...
    const elements = (ref.shape as unknown as { kind: "tuple"; elements: { shape: unknown }[] })
      .elements;
...
describe("unions", () => {
  ...
  it("mixed-literal union → union of literals", () => {
    ...
    const variants = (
      ref.shape as unknown as {
        kind: "union";
        variants: { shape: { kind: string; value: unknown } }[];
      }
    ).variants;
...
describe("intersections", () => {
  ...
  it("brand tag on a string base promotes to the semantic-string kind", () => {
    const ref = typeRefOf(`type X = string & { readonly __brand: "uuid" }`, "X");
...
describe("Promise/AsyncIterable unwrapping", () => {
  ...
  it("AsyncIterable<T> field lowers to types.stream", () => {
    ...
    const fields = (ref.shape as { fields: Record<string, { shape: unknown }> }).fields;
    expect(fields.s!.shape).toEqual({
      kind: "stream",
      element: { shape: { kind: "number" }, meta: {} },
    });
  });
});
...
describe("recursion and sharing", () => {
  ...
  it("SharingRegistry extracts a reused, big-enough named type to defs", () => {
    ...
    const fields = (
      xBody.shape as { fields: Record<string, { shape: { kind: string; target: string } }> }
    ).fields;
    ...
    const rootFields = (
      roots.root!.shape as { fields: Record<string, { shape: { kind: string; target: string } }> }
    ).fields;
  ...
  it("finalizeSharedDefs inlines a def used only once", () => {
    ...
    const fields = (roots.root!.shape as { fields: Record<string, { shape: { kind: string } }> })
      .fields;
...
describe("createExtractorProgram", () => {
  it("produces a Program whose checker resolves types from a real file", () => {
    const program = createExtractorProgram(
      `${import.meta.dir}/__fixtures__/from-typescript.fixture.ts`,
    );
    const checker = program.getTypeChecker();
