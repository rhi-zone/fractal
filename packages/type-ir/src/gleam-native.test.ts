...
describe("containers", () => {
  ...
  test("2-tuple -> #(a, b)", () => {
    expect(toGleamType(t(types.tuple([t(types.string), t(types.number)])))).toBe("#(String, Float)")
  })
...
describe("function types", () => {
  ...
  test("thisType prepends as a leading parameter", () => {
    const ref = t(types.function([{ name: "x", type: t(types.integer) }], t(types.string), t(types.ref("Account"))))
...
describe("records (object)", () => {
  test("object -> single-constructor custom type with labelled fields", () => {
    ...
    expect(toGleam(ref, "Person")).toBe(["pub type Person {", "  Person(name: String, age: Int)", "}"].join("\n"))
  })

  test("empty object -> nullary constructor", () => {
    expect(toGleam(t(types.object({})), "Empty")).toBe(["pub type Empty {", "  Empty", "}"].join("\n"))
  })
  ...
  test("nested object field hoists a sibling record declaration before the main one", () => {
    ...
    expect(gleam.indexOf("pub type PersonAddress {")).toBeLessThan(gleam.indexOf("pub type Person {"))
  })
...
describe("enum", () => {
  test("closed string set -> custom type of nullary constructors", () => {
    ...
    expect(toGleam(ref, "Status")).toBe(["pub type Status {", "  Active", "  Inactive", "}"].join("\n"))
  })
...
describe("interface", () => {
  test("renders a record of function-typed fields", () => {
    ...
    expect(gleam).toBe(["pub type Account {", "  Account(deposit: fn(Float) -> Nil)", "}"].join("\n"))
  })
})
...
describe("intersection", () => {
  ...
  test("first member being an unnamed object still degrades to Dynamic (object has no naming context here)", () => {
    const ref = t(types.intersection([t(types.object({ id: t(types.string) })), t(types.object({ createdAt: t(types.string) }))]))
