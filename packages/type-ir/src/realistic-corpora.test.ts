// Runs `fromJsonCorpus` against realistic-corpora.ts's hand-typed sample
// arrays and scores the result against each corpus's hand-authored ground
// truth via `scoreInference`. See realistic-corpora.ts's module doc for why
// this is a different validation angle than inference-eval.test.ts's
// generate-then-infer suite (there, both the ground truth and the samples
// come from the same generator; here, a human wrote both, independently, to
// look like real API traffic).
//
// These tests report the actual F1 achieved rather than asserting a
// threshold tuned to make them pass — the point of this corpus is to find
// out what score realistic-but-not-generator-shaped data gets, not to lock
// in a number. Assertions below are limited to structural sanity (finite
// scores in [0, 1]) plus a `console.log` of the full report so the actual
// numbers are visible in test output.
