...
function programFromSource(source: string): { checker: ts.TypeChecker; sourceFile: ts.SourceFile } {
  ...
  host.getSourceFile = (fileName, v, ...rest) =>
    fileName === FILE ? sourceFile : original(fileName, v, ...rest);
  host.writeFile = () => {};
...
function typeOfConst(
  src: string,
  name: string,
): { type: ts.Type; checker: ts.TypeChecker; loc: ts.Node } {
...
function refFor(
  body: string,
  name = "schema",
  options?: Parameters<typeof fromStandardSchemaType>[3],
) {
...
describe("fromStandardSchemaType", () => {
  ...
  test("recovers union branches and enum members no single sample shows", () => {
    ...
    expect([...(ref.shape as { members: readonly string[] }).members].sort()).toEqual([
      "archived",
      "draft",
      "published",
    ]);
