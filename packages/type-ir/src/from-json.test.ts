...
const inferOne = (value: unknown, config?: CorpusInferConfig) => fromJsonCorpus([value], config);
...
describe("arrays vs tuples", () => {
  ...
  test("heterogeneous array -> tuple", () => {
    expect(inferOne([1, "a", true])).toEqual(
      t(types.tuple([uint8(), t(types.string), t(types.boolean)])),
    );
  });
...
describe("objects", () => {
  ...
  test("nested object", () => {
    expect(inferOne({ user: { id: 1 } })).toEqual(
      t(types.object({ user: t(types.object({ id: uint8() })) })),
    );
  });
});
...
describe("array of objects: shape merging", () => {
  test("shared fields across all elements are required", () => {
    ...
    expect(inferOne(value)).toEqual(
      t(types.array(t(types.object({ id: uint8(), name: t(types.string) })))),
    );
  });
  ...
  test("conflicting field type across elements becomes a union", () => {
    ...
    expect(inferOne(value)).toEqual(
      t(types.array(t(types.object({ id: t(types.union([uint8(), t(types.string)])) })))),
    );
  });
  ...
  test("array of objects below threshold stays a tuple of raw element types", () => {
    ...
    expect(inferOne(value)).toEqual(
      t(types.tuple([t(types.object({ id: uint8() })), t(types.object({ id: uint8() }))])),
    );
  });
});
...
describe("custom leaf heuristics", () => {
  test("a heuristic can override default inference", () => {
    ...
    expect(inferOne(42, { leafHeuristics: [moneyHeuristic] })).toEqual(
      t(types.number, { semantic: "money" }),
    );
  });
