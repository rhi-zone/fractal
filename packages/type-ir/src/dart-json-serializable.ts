...
function toJsonExpr(ref: TypeRef, fieldExpr: string, ctx: Ctx, hint: string): string {
  ...
  if (kind === "array" || kind === "stream" || kind === "page") {
    ...
    if (needsCustom(s.element)) {
      ...
      return nullable
        ? `${fieldExpr}?.map((e) => ${elemExpr}).toList()`
        : `${fieldExpr}.map((e) => ${elemExpr}).toList()`;
    }
    return fieldExpr;
  }
...
function emitClass(name: string, ref: TypeRef, ctx: Ctx, extendsName?: string): string {
  ...
  for (const [fieldName, fieldRef] of Object.entries(shape.fields)) {
    ...
    toJsonFields.push(
      nullable
        ? `    if (${dartName} != null) ${quoteDart(fieldName)}: ${toExpr},`
        : `    ${quoteDart(fieldName)}: ${toExpr},`,
    );
  }
  ...
  const lines: string[] = [];
  lines.push(
    `${dartDocComment(ref.meta)}${dartDeprecatedAnnotation(ref.meta)}class ${name}${extendsClause} {`,
  );
  ...
  for (const { variant, variantName } of variantInfo) {
    const objectRef: TypeRef =
      variant.shape.kind === "object"
        ? variant
        : { shape: { kind: "object", fields: { value: variant } }, meta: {} };
