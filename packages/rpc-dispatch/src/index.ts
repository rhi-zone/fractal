...
// The unified transport core. Two halves of one RPC boundary:
//
...
//                  `evaluate` and http's path-walk into a single descent.
//
// Dependency direction: http / rpc / ipc → rpc-dispatch → core. Adapters
// (HTTP, IPC, in-process …) provide a `Transport` to `clientOver` and call a
// `Dispatcher` on the server side; the wire format is the adapter's concern.
