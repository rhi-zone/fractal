...
export function parseJwt(token: string): ParsedJwt {
  ...
  try {
    ...
    throw new JwtParseError(
      `malformed header/payload: ${err instanceof Error ? err.message : String(err)}`,
    );
  }
  if (
...
export function checkClaims(claims: JwtClaims, options: ClaimCheckOptions = {}): void {
  ...
  if (options.issuer !== undefined && claims.iss !== options.issuer) {
    throw new JwtClaimError(
      `issuer mismatch: expected "${options.issuer}", got "${String(claims.iss)}"`,
    );
  }

  if (options.audience !== undefined && !audienceMatches(claims.aud, options.audience)) {
    throw new JwtClaimError(
      `audience mismatch: expected one of ${JSON.stringify(options.audience)}, got ${JSON.stringify(claims.aud)}`,
    );
  }
}
