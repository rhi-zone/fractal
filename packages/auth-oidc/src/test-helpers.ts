...
export async function makeSignedJwt(
  ...
  const publicJwkRaw = (await crypto.subtle.exportKey("jwk", keyPair.publicKey)) as Record<
    string,
    unknown
  >;
...
function signSpecFor(alg: string): {
  readonly importParams: RsaHashedKeyGenParams | EcKeyGenParams;
