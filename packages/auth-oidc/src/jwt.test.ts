...
describe("checkClaims", () => {
  ...
  it("tolerates expiry within clockToleranceSec", () => {
    expect(() =>
      checkClaims({ exp: Date.now() / 1000 - 5 }, { clockToleranceSec: 30 }),
    ).not.toThrow();
  });

  it("throws JwtClaimError on nbf in the future", () => {
    expect(() =>
      checkClaims({ exp: Date.now() / 1000 + 3600, nbf: Date.now() / 1000 + 60 }),
    ).toThrow(JwtClaimError);
  });

  it("throws JwtClaimError on issuer mismatch", () => {
    expect(() =>
      checkClaims(
        { exp: Date.now() / 1000 + 3600, iss: "https://evil.example.com" },
  ...
  it("passes on issuer match", () => {
    expect(() =>
      checkClaims(
        { exp: Date.now() / 1000 + 3600, iss: "https://auth.example.com" },
        { issuer: "https://auth.example.com" },
      ),
    ).not.toThrow();
  });
  ...
  it("throws JwtClaimError on audience mismatch (single value)", () => {
    expect(() =>
      checkClaims({ exp: Date.now() / 1000 + 3600, aud: "other-api" }, { audience: "my-api" }),
    ).toThrow(JwtClaimError);
  });
  ...
  it("passes when aud is an array containing the expected audience", () => {
    expect(() =>
      checkClaims(
        { exp: Date.now() / 1000 + 3600, aud: ["other-api", "my-api"] },
        { audience: "my-api" },
      ),
    ).not.toThrow();
  });
  ...
  it("passes when expected audience is any-of a list", () => {
    expect(() =>
      checkClaims(
        { exp: Date.now() / 1000 + 3600, aud: "my-api" },
        { audience: ["my-api", "other-api"] },
      ),
    ).not.toThrow();
  });

  it('throws JwtClaimError when audience is required but the claim has no "aud" at all', () => {
    expect(() => checkClaims({ exp: Date.now() / 1000 + 3600 }, { audience: "my-api" })).toThrow(
      JwtClaimError,
    );
  });
});
...
describe("verifyJwt (full pipeline)", () => {
  it("returns claims on a fully valid token", async () => {
    ...
    const claims = await verifyJwt(token, publicJwk, {
      issuer: "https://auth.example.com",
      audience: "my-api",
    });
  ...
  it("throws on an invalid signature", async () => {
    ...
    const { publicJwk: wrongKey } = await makeSignedJwt({
      sub: "user-2",
      exp: Date.now() / 1000 + 3600,
    });
