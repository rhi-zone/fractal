...
describe("resolveJwksUri", () => {
  ...
  it("throws JwksFetchError when the discovery fetch fails", async () => {
    ...
    await expect(resolveJwksUri({ issuer: "https://auth.example.com" }, fetchImpl)).rejects.toThrow(
      JwksFetchError,
    );
  });
  ...
  it("throws JwksFetchError when the discovery document has no jwks_uri", async () => {
    const fetchImpl = async (): Promise<Response> =>
      jsonResponse({ issuer: "https://auth.example.com" });
    await expect(resolveJwksUri({ issuer: "https://auth.example.com" }, fetchImpl)).rejects.toThrow(
      JwksFetchError,
    );
  });
...
describe("createJwksCache", () => {
  it("fetches once and caches within the TTL", async () => {
    ...
    const cache = createJwksCache("https://auth.example.com/jwks.json", {
      fetchImpl,
      ttlMs: 60_000,
    });
    await cache.getJwks();
