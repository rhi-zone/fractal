...
- `./apply-validation` — `createApplyValidation`, `applyValidation` (per call site), `isApplyValidationWrapped`, `assertValidationCoverage`, `UncoveredLeafError`, `ValidatorMap` — the keyed, call-site-anchored validation mechanism: `applyValidation(key, projectedTree)` wires generated validators directly onto a leaf's handler (structurally, over either a `Node`'s own `handler` or an `HttpRoute`-style `methods` record), shared by HTTP/MCP/CLI/GraphQL. Permissive by default (an unknown key or uncovered leaf just passes through); `assertValidationCoverage` is the opt-in LOUD build-mode check, throwing `UncoveredLeafError` naming every uncovered leaf at once. `isApplyValidationWrapped` lets a projector's own fallback coercion/validation step skip a leaf this mechanism already covers.
