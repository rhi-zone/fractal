...
- `./build` — `buildValidatorModuleSource`, `stubValidatorModuleSource`, `writeValidatorModule`, `writeStubValidatorModule`, `wrapValidators`, `isValidatorWrapped`, `GeneratedEntry` — the build orchestrator backing the CLI above: wires `./extract`/`./tree`'s extraction to `@rhi-zone/fractal-type-ir`'s AOT validator compiler, and `wrapValidators` wires the resulting generated validators directly onto a `Node` tree's leaf handlers (the single mechanism shared by HTTP/MCP/CLI's `validators` options)
