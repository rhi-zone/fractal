...
async function readCarryForward(entryFile: string, outFile: string): Promise<WireApplyValidationCarryForwardState> {
...
describe("IR-keyed build cache (v3) — Tier 2 leaf-level carry-forward", () => {
  it("1. precision win: a comment/implementation-only edit inside one leaf's body triggers Tier-1 miss but changes ZERO leaves' fingerprints", async () => {
    ...
    const incremental = buildWireApplyValidationModuleSourceIncremental(entryFile, { outFile, program, prior })
  ...
  it("2. transitive type change: an edit reached only through an import changes exactly the reaching leaf; the unrelated leaf is untouched", async () => {
    ...
    expect(incremental.leafArtifacts["bundle\u0000leafOne identity"]).not.toEqual(prior.leafArtifacts["bundle\u0000leafOne identity"])
    ...
    expect(incremental.leafArtifacts["bundle\u0000leafTwo identity"]).toEqual(leafTwoFragmentBefore as never)
  })
  ...
  describe("5. determinism regression — defs/sharing + declarationFile paths, not just an inline-object leaf", () => {
    it("leaf fingerprints are byte-stable across two independent Program builds over a fixture that exercises shouldShare/defs", () => {
      ...
      const { types: typesA } = extractRouteTypeRefs(FIXTURE, { shouldShare: defaultShouldShare, program: programA })
      const programB = createExtractorProgram(FIXTURE)
      const { types: typesB } = extractRouteTypeRefs(FIXTURE, { shouldShare: defaultShouldShare, program: programB })

      expect(Object.keys(typesA).sort()).toEqual(Object.keys(typesB).sort())
      for (const key of Object.keys(typesA)) {
        const fpA = computeLeafFingerprint(FIXTURE, { input: typesA[key]!.input, output: typesA[key]!.output })
        const fpB = computeLeafFingerprint(FIXTURE, { input: typesB[key]!.input, output: typesB[key]!.output })
        expect(fpB).toBe(fpA)
      }
    })
    ...
    it("declarationFile absolute paths are canonicalized away — two different checkout roots with the same relative layout fingerprint identically", () => {
      ...
      const leafA = { input: { shape: { kind: "ref", target: "Widget" }, meta: { typeName: "Widget", declarationFile: declA } } }
      const leafB = { input: { shape: { kind: "ref", target: "Widget" }, meta: { typeName: "Widget", declarationFile: declB } } }

      expect(computeLeafFingerprint(entryFileA, leafA)).toBe(computeLeafFingerprint(entryFileB, leafB))
      ...
      const leafC = { input: { shape: { kind: "ref", target: "Widget" }, meta: { typeName: "Widget", declarationFile: declC } } }
