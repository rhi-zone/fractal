...
Commands:
  build <entry> -o <output>          Build the validator module (skip if cached)
...
/** One artifact kind's build function — `buildValidatorModuleSource` (with
 * its extra `outFile`-anchored import-resolution arg) for `build`/`watch`/
 * `check`, `buildSchemaModuleSource` for the `-schema` commands. Both take
 * `(entryFile, program)` once curried here, so `runBuild`/`runCheck`/
 * `runWatch` below are shared across both artifact kinds — same cache
 * contract, same CLI plumbing, no second divergent code path. */
...
const VALIDATOR_BUILDER: ArtifactBuilder = (entryFile, outFile, program) =>
  buildValidatorModuleSource(entryFile, outFile, undefined, program)
