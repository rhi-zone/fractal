...
// No `@ts-nocheck`: the generated module's `compileWireEntryFragment`/
// `compileConstraintsFn` output (type-ir/src/compile.ts) emits typed guards —
// each entry's `check` is cast to `(value: unknown) => value is T`, so the
// file typechecks clean under `strict`, and a consumer narrows `.value` off
 ...
 * same "compiles and runs before codegen has ever produced anything"
 * convention `build`/`watch`/`check` always had.
 *
 * Phase D (docs/design/wire-profiles-and-staged-validation.md) retired the
 * separate 2-arg-only build path this used to sit alongside as `build-wire`/
 * `watch-wire`/`check-wire` (a distinct set of subcommands, disambiguating two
 * coexisting codegen mechanisms) — with one mechanism left, `build`/`watch`/
 * `check` themselves now run it: EVERY `applyValidation(key, tree, protocol?)`
 ...
 * (phase D) isn't exposed as a CLI flag here — a pre-existing CLI-surface
 * gap, not something this retirement introduces. */
