...
describe("cli.ts — build/watch/check cache behavior end-to-end", () => {
  ...
  it("build now engages Tier 2: cache metadata carries real leafFingerprints/leafArtifacts, not empty objects (audit §6#1)", async () => {
    const outFile = path.join(TMP_DIR, "out-tier2.ts");
    const built = await runCli(["build", WIRE_FIXTURE, "-o", outFile]);
    expect(built.exitCode).toBe(0);

    const meta = JSON.parse(fs.readFileSync(`${outFile}.cache.json`, "utf8")) as {
      leafFingerprints: Record<string, string>;
      leafArtifacts: Record<string, unknown>;
    };
    expect(Object.keys(meta.leafFingerprints).length).toBeGreaterThan(0);
    expect(Object.keys(meta.leafArtifacts).length).toBeGreaterThan(0);

    // `check` writes its own metadata (keyed to the oxfmt-formatted bytes,
    // per runCheck's doc comment) on a Tier-1 miss it confirms as
    // up-to-date; that write must carry the same non-empty Tier-2 state, not
    // regress to empty leafFingerprints/leafArtifacts the way `check` did
    // before this fix. Reformat `outFile` through oxfmt first (mirroring the
    // real workflow's `oxfmt --write .` step after `build`) — build's own
    // raw bytes generally don't already equal oxfmt's own formatting, so
    // `check`'s formatted-bytes-vs-disk comparison would otherwise report a
    // real (not just cache-key) mismatch and this test would be exercising
    // the "stale" branch instead of the "up to date, refresh metadata" one.
    Bun.spawnSync(["bunx", "oxfmt", "--write", outFile]);
    const check = await runCli(["check", WIRE_FIXTURE, "-o", outFile]);
    expect(check.exitCode).toBe(0);
    expect(check.stdout).toContain("up to date");
    const metaAfterCheck = JSON.parse(fs.readFileSync(`${outFile}.cache.json`, "utf8")) as {
      leafFingerprints: Record<string, string>;
      leafArtifacts: Record<string, unknown>;
    };
    expect(Object.keys(metaAfterCheck.leafFingerprints).length).toBeGreaterThan(0);
    expect(Object.keys(metaAfterCheck.leafArtifacts).length).toBeGreaterThan(0);
  }, 30_000);

  it("build now writes the header through the cached wrapper's own header option, matching what check compares against", async () => {
    const outFile = path.join(TMP_DIR, "out-header-unified.ts");
    const built = await runCli(["build", FIXTURE, "-o", outFile]);
    expect(built.exitCode).toBe(0);
    const source = fs.readFileSync(outFile, "utf8");
    expect(source.startsWith("// @generated by @rhi-zone/fractal-api-tree")).toBe(true);
