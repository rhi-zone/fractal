...
// sharing via shouldShare). Every `applyValidation` call site, 2-arg
// included, compiles through `buildWireApplyValidationModuleSource`'s
// wire-profile pipeline, with an omitted protocol resolving to `"identity"`.
// Also doubles as cache.test.ts's/cache-v3.test.ts's entry file for
// exercising cache.ts's generic mechanics end-to-end against a real
// multi-file extraction closure (tree.fixture.ts's own imports of
// result-reexport.fixture.ts/deployment-meta.fixture.ts) — cache.ts tracks
// the whole `ts.Program`'s file set by default, so those dependencies are
// tracked here too.
//
