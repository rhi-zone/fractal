...
//
// Wraps tree.fixture.ts's exported `tree` in a single `applyValidation` call
// site, so the call-site-anchored codegen (apply-validation-build.ts) has
// something to trace against the SAME extraction edge cases tree.fixture.ts
// exercises (named-type import resolution, builtin/global TS type exclusion,
// a DOM builtin reached through a return type, structural sharing via
// shouldShare) — the regression coverage build.test.ts used to assert
// against the now-deleted `buildValidatorModuleSource` (build.ts, phase 3),
// ported onto `buildApplyValidationModuleSource` instead. Also doubles as
