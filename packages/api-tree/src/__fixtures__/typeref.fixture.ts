...
export function overloadedResultFn(
  a: string | number,
): Result<{ text: string } | { num: number }, string> {
...
type MappedUtility<T> =
  T extends MappedUtilitySchema<infer TEntries> ? { [K in keyof TEntries]: TEntries[K] } : never;
 ...
 * (offset style). */
export const offsetPageFn = (): OffsetPage<number> => ({
  items: [],
  offset: 0,
  total: 0,
  hasMore: false,
