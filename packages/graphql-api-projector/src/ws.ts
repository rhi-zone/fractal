...
function parseClientMessage(data: string): ClientMessage | undefined {
  ...
  switch (parsed.type) {
    ...
    case "subscribe": {
      ...
      if (payload.operationName !== undefined && typeof payload.operationName !== "string")
        return undefined;
  ...
  return (conn: GraphQLWsSender): GraphQLWsConnectionHandler => {
    ...
    const initTimer: ReturnType<typeof setTimeout> | undefined =
      waitTimeout === Infinity
        ...
        : setTimeout(() => {
            if (!acknowledged)
              conn.close(CLOSE_CONNECTION_INIT_TIMEOUT, "Connection initialisation timeout");
          }, waitTimeout);
    ...
    const send = (
      message:
        ...
        | PingMessage
        | PongMessage,
    ): void => conn.send(JSON.stringify(message));
    ...
    const runSubscription = async (msg: SubscribeMessage): Promise<void> => {
      ...
      const iterableOrResult = await server.subscribe(
        msg.payload.query,
