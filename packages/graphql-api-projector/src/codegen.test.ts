...
describe("generateGraphQLClient — structure", () => {
  ...
  it("emits client member call signatures matching hasInput", () => {
    ...
    expect(source).toMatch(
      /readonly rename: \(input: UsersUserIdRenameInput\) => Promise<UsersUserIdRenameOutput>/,
    );
    expect(source).toMatch(
      /readonly createUser: \(input: CreateUserInput\) => Promise<CreateUserOutput>/,
    );
  ...
  it("respects a custom clientName option", () => {
    const named = generateGraphQLClient(tree, {
      types: typesMap,
      namedTypes,
...
describe("generateGraphQLClient — eval end-to-end", () => {
  ...
  it("generated GraphQLClientError carries the transport's errors array", async () => {
    ...
    try {
      ...
      expect((caught as { errors: readonly { message: string }[] }).errors).toEqual([
        { message: "boom" },
      ]);
    } finally {
      await rm(tmpDir, { recursive: true, force: true });
