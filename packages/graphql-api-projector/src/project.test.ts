...
describe("nested Query vs flat Mutation/Subscription", () => {
  it("Query: a branch synthesizes a namespace type; root gets one field pointing at it", () => {
    ...
    const nested = (result.types.UsersQuery!.meta.graphqlFields as { name: string }[]).map(
      (f) => f.name,
    );
  ...
  it("deeper nesting synthesizes a PascalCase-joined namespace type name", () => {
    ...
    expect(result.types.UsersQuery).toBeDefined();
    const usersFields = result.types.UsersQuery!.meta.graphqlFields as {
      name: string;
      argsSDL: string;
      typeSDL: string;
    }[];
...
describe("fallback → named GraphQL argument", () => {
    ...
    const n = api_(
      {},
      {
        fallback: {
          name: "userId",
          subtree: api_({
            delete: op((_: { userId: string }) => null, { tags: { destructive: true } }),
          }),
        },
      },
    );
  ...
  it("a fallback under a nested (query) leaf becomes an ID! arg on the leaf field itself (not the namespace field)", () => {
    const n = api_(
      {},
      {
        fallback: {
          ...
          subtree: api_({
            users: api_({
              profile: op((_: { userId: string }) => ({}), { tags: { readOnly: true } }),
            }),
          }),
        },
      },
    ...
    const outer = result.types.UserIdQuery!.meta.graphqlFields as {
      name: string;
      argsSDL: string;
      typeSDL: string;
    }[];
    expect(outer).toEqual([{ name: "users", argsSDL: "", typeSDL: "UserIdUsersQuery!" }]);
    const inner = result.types.UserIdUsersQuery!.meta.graphqlFields as {
      name: string;
      argsSDL: string;
    }[];
  ...
  // branch (`Object.entries(subtree.children ?? {})`) would see no children
  // and omit the leaf entirely.
  it("a bare op() fallback.subtree becomes a query field nested under the branch namespace, no extra segment beyond the fallback's own name", () => {
    ...
    const booksFields = result.types.BooksQuery!.meta.graphqlFields as {
      name: string;
      argsSDL: string;
      typeSDL: string;
    }[];
...
describe("derived TypeRefs drive args/return SDL", () => {
  it("an object input TypeRef expands to one arg per field; output TypeRef drives the return type", () => {
    ...
    const result = projectGraphQL(n, { types: typesMap });
    const inner = result.types.UsersQuery!.meta.graphqlFields as {
      name: string;
      argsSDL: string;
      typeSDL: string;
    }[];
...
describe("meta.graphql per-projection overrides", () => {
  ...
  it("meta.graphql.name overrides the inferred field name", () => {
    const n = api_({
      list: op((_: unknown) => [], { tags: { readOnly: true }, graphql: { name: "catalogList" } }),
    });
  ...
  it("meta.graphql.description overrides meta.description overrides derived description", () => {
    ...
    const n = api_({
      list: op((_: unknown) => [], {
        ...
        graphql: { description: "gql-specific" },
      }),
    });
    const { mutationFields } = projectGraphQL(n, { types: typesMap });
  ...
  it("meta.tags.deprecated → deprecated:true on the field; meta.graphql.deprecated overrides it", () => {
    ...
    const n2 = api_({
      old: op((_: unknown) => ({}), { tags: { deprecated: true }, graphql: { deprecated: false } }),
    });
  ...
  it("meta.graphql.deprecatedReason is only emitted when deprecated resolves true", () => {
    const n = api_({
      old: op((_: unknown) => ({}), {
        ...
        graphql: { deprecatedReason: "use `new` instead" },
      }),
    });
  ...
  it("meta.graphql.sourceMap is threaded onto the Dispatch entry", () => {
    const n = api_({
      create: op((_: unknown) => ({}), {
        graphql: { sourceMap: { name: { store: "argument", key: "fullName" } } },
      }),
    });
    const { handlers } = projectGraphQL(n);
    expect(handlers.get("create")?.sourceMap).toEqual({
      name: { store: "argument", key: "fullName" },
    });
  });
});
