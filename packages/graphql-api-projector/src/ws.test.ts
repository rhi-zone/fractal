...
async function init(
  conn: ReturnType<typeof mockConn>,
  factory: ReturnType<typeof createWsHandler>,
) {
...
describe("createWsHandler — handshake", () => {
  ...
  it("subscribe before connection_init closes with 4401", async () => {
    ...
    const handler = factory(conn);
    handler.onMessage(
      JSON.stringify({ type: "subscribe", id: "1", payload: { query: "subscription { watch }" } }),
    );
    await tick();
  ...
  it("subscribe streams next messages then a server complete", async () => {
    ...
    handler.onMessage(
      JSON.stringify({
        type: "subscribe",
        id: "sub-1",
        payload: { query: "subscription { watch }" },
      }),
    );
    await tick();
    await tick();
    ...
    expect(nexts.map((m) => (m.payload as { data: { watch: string } }).data.watch)).toEqual([
      "event-0",
      "event-1",
    ]);
  ...
  it("passes variables through to the subscription query", async () => {
    ...
    handler.onMessage(
      JSON.stringify({
        ...
        id: "sub-1",
        payload: {
          query: "subscription($count: Int) { watch(count: $count) }",
          variables: { count: 1 },
        },
      }),
    );
...
describe("createWsHandler — errors", () => {
  ...
  it("an invalid query document sends an error message", async () => {
    ...
    handler.onMessage(
      JSON.stringify({ type: "subscribe", id: "sub-1", payload: { query: "subscription { " } }),
    );
    await tick();
  ...
  it("two subscriptions with different ids stream independently", async () => {
    ...
    handler.onMessage(
      JSON.stringify({
        type: "subscribe",
        id: "a",
        payload: { query: "subscription { watch(count: 1) }" },
      }),
    );
    handler.onMessage(
      JSON.stringify({
        type: "subscribe",
        id: "b",
        payload: { query: "subscription { watch(count: 3) }" },
      }),
    );
    await tick();
    await tick();
